...
CodeGen::CodeGen(HermesCtr ast, std::string_view src_path)
...
std::string CodeGen::str(AnyVal av) const {
    ...
    return std::string(StringView(av.to_offset(), ast_.holder()).view());
}

TinyMapView CodeGen::map_of(AnyVal av) const {
    return TinyMapView(av.to_offset(), ast_.holder());
}

ArrayView CodeGen::arr_of(AnyVal av) const {
    return ArrayView(av.to_offset(), ast_.holder());
}
...
void CodeGen::emit_message_impl(std::ostream& out, TinyMapView node) const {
    ...
    // from_doc()
    out << name << " " << name << "::from_doc(logos::hermes::HermesCtr doc) {\n";
    out << "    " << name << " m;\n";
