...
private:
    ...
    std::vector<logos::writ::TinyMapView> items_;
    ...
    std::string str(logos::writ::AnyVal av) const;

    // Read a TinyMapView from an AnyVal offset.
    logos::writ::TinyMapView map_of(logos::writ::AnyVal av) const;

    // Read an ArrayView from an AnyVal offset.
    logos::writ::ArrayView arr_of(logos::writ::AnyVal av) const;

    // Read CODE field (int32 value).
    int32_t code_of(logos::writ::TinyMapView node) const;

    // Read a string field by raw key code.
    std::string str_field(logos::writ::TinyMapView node, uint8_t key) const;

    // Read a qualified name (TYPE_REF with ITEMS=[ident_node, ...]) → "foo.Bar".
    std::string qualified_name(logos::writ::TinyMapView type_ref_node) const;
    ...
    TypeInfo resolve_type(logos::writ::TinyMapView type_node) const;

    // --- Per-definition emitters ---

    // Header
    void emit_enum_decl    (std::ostream& out, logos::writ::TinyMapView node) const;
    void emit_message_decl (std::ostream& out, logos::writ::TinyMapView node) const;
    void emit_service_decl (std::ostream& out, logos::writ::TinyMapView node) const;

    // Source
    void emit_message_impl (std::ostream& out, logos::writ::TinyMapView node) const;
    void emit_service_impl (std::ostream& out, logos::writ::TinyMapView node) const;

    // Per-field helpers
    void emit_field_getter_decl (std::ostream& out, logos::writ::TinyMapView field) const;
    void emit_field_setter_decl (std::ostream& out, logos::writ::TinyMapView field) const;
    void emit_field_getter_impl (std::ostream& out, std::string_view struct_name,
                                  logos::writ::TinyMapView field) const;
    ...
    std::string rpc_handler_sig(logos::writ::TinyMapView rpc_node) const;
    std::string rpc_client_sig (logos::writ::TinyMapView rpc_node) const;
