...
private:
    ...
    void emit_header() {
        ...
        w.line("#include <logos/hermes2/compat.hpp>");
        ...
        if (!g_.exports.empty()) {
            ...
            for (const auto& e : g_.exports)
                w.fmt("logos::hermes2::Hermes parse_{}();", e);
            w.line();
        }
        ...
        for (const auto& r : g_.rules) {
            w.fmt("logos::hermes2::AnyVal rule_{}();", r.name);
            if (is_memoized(r.name))
                w.fmt("logos::hermes2::AnyVal rule_{}_impl();", r.name);
        }
        ...
        for (const auto& r : g_.rules) {
            ...
            w.fmt("std::unordered_map<size_t, std::pair<logos::hermes2::AnyVal, size_t>> memo_{}_;", r.name);
        }
        if (!g_.prec.empty()) {
            w.line();
            w.line("// Pratt expression parser (generated from %prec).");
            w.line("logos::hermes2::AnyVal pratt_expr(int min_prec = 0);");
            w.line("logos::hermes2::AnyVal pratt_atom();");
        }
        ...
        w.line();
        w.line("logos::hermes2::Hermes doc_;");
    ...
    void emit_source() {
        ...
        w.line("#include <logos/hermes2/schema_codes.hpp>");
        ...
        w.line("using logos::hermes2::AnyVal;");
    ...
    void emit_public_entries(CodeWriter& w) {
        ...
        for (const auto& e : g_.exports) {
            ...
            w.fmt("std::fprintf(stderr, \"parse error in {}: expected {} (near line %u)\\n\",",
                  e, e);
            w.line("             next_line());");
            w.line("return logos::hermes2::Hermes{};  // null doc; caller handles error");
            w.dedent();
            w.line("}");
    ...
    void emit_action(CodeWriter& w, const Action& action,
        ...
        w.fmt("auto* node = logos::hermes2::HermesAccess::raw_tiny_map(doc_, {}).get();", slot_count);
        ...
        for (const auto& field : action.fields) {
            ...
            case int32_t(ast::CAPTURE): {
                ...
                if (idx < captures.size() && !captures[idx].empty()) {
                    w.fmt("node->put({}, {}, logos::hermes2::HermesAccess::arena(doc_)).get();",
            ...
            case int32_t(ast::ARRAY_CAPTURE): {
                ...
                w.fmt("node->put({}, {}.to_anyval(), logos::hermes2::HermesAccess::arena(doc_)).get();",
            ...
            case int32_t(ast::STR_LIT): {
                ...
                w.fmt("node->put({}, AnyVal::from_value({}::{}), logos::hermes2::HermesAccess::arena(doc_)).get();",
                ...
                if (field.name == "CODE") {
                    w.fmt("node->set_schema_type_code(logos::hermes2::schema::ast(static_cast<int32_t>({}::{})));",
                          ast_ns_, expr.value);
                }
                break;
            }
            ...
            case int32_t(ast::INT_LIT): {
                w.fmt("node->put({}, AnyVal::from_value(int32_t({})), logos::hermes2::HermesAccess::arena(doc_)).get();",
                      field_const, expr.int_val);
                if (field.name == "CODE") {
                    w.fmt("node->set_schema_type_code(logos::hermes2::schema::ast(int32_t({})));",
                          expr.int_val);
                }
                break;
            }
            ...
            case int32_t(ast::BOOL_LIT): {
                w.fmt("node->put({}, AnyVal::from_value(uint8_t({})), logos::hermes2::HermesAccess::arena(doc_)).get();",
                      field_const, expr.int_val);
                break;
            }
        ...
        w.fmt("node->put({}::SRC_LINE, AnyVal::from_value(first_line_), logos::hermes2::HermesAccess::arena(doc_)).get();",
              ast_ns_);
        w.line("{");
        w.indent();
