# logos_hermes_gdb.py — gdb decoder/pretty-printer for the Hermes container
# format (self-relative arena values). The compiler's IR (LProgram/AST) and any
# Hermes-backed runtime data use this format.
#
# Load:
#   (gdb) source /path/to/logos/tools/gdb/logos_hermes_gdb.py
# Commands:
#   (gdb) logos-hermes <expr>     # decode the AnyVal word at &<expr> (or an addr)
# Also auto-registers a pretty-printer on `logos::hermes::AnyVal`.
#
# Format (see include/logos/hermes/*.hpp):
...
# ── Type codes (include/logos/hermes/type_codes.hpp) ─────────────────────────
HA_I56, HA_BOOL = 1, 2
...
class HermesDecoder:
    ...
    # ── varint (include/logos/hermes/varint.hpp) ──
...
def _selftest():
    """Build a byte image and decode it — runs without gdb (python logos_hermes_gdb.py)."""
...
try:
    ...
    def _av_lookup(val):
        ...
        if name.endswith("AnyVal") or name.endswith("hermes::AnyVal"):
