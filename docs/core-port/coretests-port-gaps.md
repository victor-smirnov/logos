...
| CP-cm-16 | Trait-static dispatch through enum-typed tparam: `C::from_iter(self)` with `C = Result<...>` can't synthesise cloned-spec callee name | Open (surfaced 2026-05-17 trying to add `impl<T,E> FromIterator<Result<T,E>> for Result<Vec<T>,E>` for `result::test_collect`) | mono_clone.cpp:2289 prefix-rewrite path handles `<tparam>__method` callsites by looking up the subst-bound concrete type and rewriting the prefix to its `cname`. The `cname` lookup at line 2304 has a Struct branch (concrete_struct_name) and a `type_str` fallback for everything else. For an enum-typed C the fallback produces a non-mangled string (e.g. literal `Result<Vec<i32>, i32>`) that doesn't compose into the canonical `<base>__<arg1>__<arg2>` form `instantiate_enum_templates` uses for clones, so the rewritten callee name doesn't match any emitted spec. Adding an Enum branch produced the right callee shape but `instantiate_enum_templates`'s size-match (`fn.type_params.size() == tmpl->type_params.size()`) skipped the impl method (impl has [T, E, I] = 3 tparams, Result has [T, E] = 2 — mismatch) so no spec was cloned either way. Both ends need updating. Workaround: free fn `vec_collect_results(input: Vec<Result<T,E>>) -> Result<Vec<T>,E>` in stdlib/mem/collections/vec sidesteps trait-static dispatch entirely. | — | Driver test deferred. |
| CP-cm-17 | Vec-as-enum-payload: returning `Result<Vec<T>, E>::Ok(v)` then `.unwrap().length()` returns garbage; empty Vec inside double-frees | Open (surfaced 2026-05-17 trying to port `result::test_collect`) | `fn make() -> Result<Vec<i32>, i32> { return Result::Ok(vec_new::<i32>()); }; make().unwrap().length()` works at top level but goes wrong inside a multi-test test_harness body: garbage values on `length()` (e.g. -685814257) and a double-free on `vec_new::<T>()` empty Vec when passed back through Result. Suspect cause: mlir-gen's GEP into Result's payload area uses a fixed offset that doesn't match Vec's 24-byte {ptr, len, cap} layout in all alignment paths, plus Vec's Drop impl fires twice on the empty-Vec path (no ownership transfer tracking when the enum payload is moved into and out of the variant). | — | Blocks any `Result<Vec<T>, E>` return type pattern that the port catalog will surely revisit (collect, parser-style chains, etc.). |
