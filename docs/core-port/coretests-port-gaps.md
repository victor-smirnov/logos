...
| CP-cm-08b | Tuple `==` for non-primitive fields (struct ✅ / str ◌ / nested tuple ◌) | Partial (2026-05-14) | Routed tuple `==`/`!=` through SL-sl-08's tuple-impl dispatch: stdlib now has `impl<A:Eq, B:Eq> Eq for (A, B)` / `(A, B, C)` / `(A, B, C, D)` blankets that compare element-wise via per-element `Eq::eq`. sema_expr.cpp lower_binop now desugars `tup_a == tup_b` → call to `$tuple$N__eq`, with autoref of lhs/rhs to `&Tuple` since Eq takes `&self`. **Works** for primitive fields (also covered by the CP-cm-08 mlir-gen fast-path) and **for struct fields** (e.g. `(Point, i32) == (Point, i32)` — Eq trait on Point handles the comparison). **Direct `str.eq(&str)` now works** (2026-05-14): make_ref canonicalisation now collapses `&UnsizedSlice<T>` → `Slice<T>` symmetrically for implicit `&self` (was previously asymmetric — explicit `other: &Self` canonicalised, implicit `&self` stayed as `Ref<UnsizedSlice>`). Slice-method dispatch retries with `Ref<Slice>` args flattened, re-lowering UNARY-& AST. Regression test `pass/str_eq_method.logos`. **Still open:** (a) tuple-with-str-field via the Eq blanket — separate mlir-gen Slice-in-aggregate ABI bug: a Slice<u8> stored in a tuple field has 8-byte field layout (just the slice-desc ptr), but Slice-by-value call ABI spills to 8-byte alloca and the callee reads 16 bytes (ptr+len) → stack garbage. (b) Nested tuple fields — bound-check doesn't recognise that `(i32, i32)` satisfies `A: Eq` through the blanket (needs trait-engine recursive-bound handling). | (a) Fix mlir-gen tuple-field Slice layout: either store full 16-byte slice desc inline or auto-load to a 16-byte alloca before the call. (b) Extend `check_type_bounds` to consult tuple-blanket impls recursively. | Driver: tuple.rs `test_partial_eq` with nested 3-tuples. Regression tests `pass/tuple_eq_via_trait.logos`, `pass/str_eq_method.logos`. |
