...
| SL-sl-04 | Result full method surface | **Blocked** on CP-cm-09 (2026-05-13) | Tried `impl<T,E> Result<T,E> { fn is_ok ... }` etc. — sema/mono only substitutes ONE type param in the receiver-enum mangled name. For `Result<i32, i32>::is_ok` the call mangles to `Result__i32__E__is_ok` with literal `E`, MLIR-gen errors "unknown tagged enum". Single-param Option<T> works (SL-sl-03). | Fix CP-cm-09 first, then re-add the impl block per the file's earlier draft. | — |
| CP-cm-09 | Multi-param generic enum method mono | Open | `impl<T,E> Result<T,E> { ... }` methods fail mono: mangling path at sema_expr.cpp:4543 substitutes only the first type-arg of the receiver enum. Visible as `Result__i32__E__method` instead of `Result__i32__i32__method`. | Walk all enum type-params in subst, not just the first. | Surfaced by SL-sl-04 attempt. |
