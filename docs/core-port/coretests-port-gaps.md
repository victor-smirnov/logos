...
| CP-cm-16 | Trait-static dispatch through enum-typed tparam (`C::from_iter(self)` with `C = Result<...>`) | Partial close (2026-05-17) | **Closed for simple shape** `impl<T,E> Trait for Enum<T,E>`: mono_clone.cpp:2289 prefix-rewrite now (a) computes the canonical enum cname `<base>__<arg1>__<arg2>...` for enum-typed receivers (mirroring record_needed_enum), and (b) consults `enum_templates_` in addition to `struct_templates_` when separating receiver-tparams from method-tparams in CP-cm-14's infer-from-args path — without it Result's from_iter<I> sees [T,E,I] as all method-level. **Open for partial-spec shape** `impl<T,E> Trait for Enum<Vec<T>, E>` (e.g. `impl FromIterator<Result<T,E>> for Result<Vec<T>, E>`): the impl-target type pattern (`Result<Vec<T>, E>`) doesn't structurally match the receiver type's args (`Result<Vec<i32>, i32>` has type-args `[Vec<i32>, i32]`) by positional substitution. Mono naively binds the impl's `T` to `Vec<i32>` (positional), but the right answer needs unification: match `Vec<T>` against `Vec<i32>` → T=i32. Same family as struct partial-spec selection. Workaround: free fn `vec_collect_results(Vec<Result<T,E>>) -> Result<Vec<T>,E>` in stdlib/mem/collections/vec. | — | Simple enum trait-static dispatch now works (e.g. `impl<T> MyTrait for Option<T>`); partial-spec impl target pattern is the follow-up. |
| CP-cm-17 | `is_ok` / `is_err` / `is_some` / `is_none` take `self` by value → consumed non-Copy Result/Option | ✅ Closed (2026-05-17) | Two-leg fix landed together. **Leg (a)** stdlib: signatures changed to `&self`. For Copy payloads (i32 etc.) the prior by-value form was benign; for non-Copy (Vec/String/...) `r.is_ok()` consumed `r` and the follow-up `r.unwrap()` read from moved-from data, double-freeing. **Leg (b)** sema_stmt.cpp's `bind_pattern_ref` VariantData branch now filters wildcard `_` payload bindings — without this, `match self { Some(_) => ... }` (the body of is_ok / is_none / etc.) registered `_` in scope, and `collect_drops` at scope end emitted `Vec.drop` / `String.drop` on the payload even when `_` was meant to discard. Mirrors the Tuple branch's existing filter. Now `match` on `&Result<...>` works cleanly through the `_` wildcards inside the discriminant-check bodies. result::test_collect can re-attempt with native `r.is_ok()` shape; the match-based workaround kept in place since it's no less expressive. Regression: `tests/imported/pass/result/test_harness_coretest_result.logos::test_collect`. | — | — |
