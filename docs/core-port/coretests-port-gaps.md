...
| CP-cm-15 | Trait-bound `T` method dispatch from inside enum variant match-binding passes `ptr` where method expects value | Open (surfaced 2026-05-17 trying to add `impl<T: Debug> Debug for Option<T>`) | Inside `impl<T: Debug> Debug for Option<T> { fn dbg(self: Option<T>, …) { match self { Option::Some(v) => v.dbg(buf), … } } }`, mlir-gen emits `func.call op operand type mismatch: expected operand type 'i32', but provided '!llvm.ptr' for operand number 0`. The match-binding `v` of payload-type `T` (T=i32 at instantiation) is bound as a pointer (likely &T into the enum's payload slot), but the method dispatch `v.dbg(buf)` routes to `dbg(self: i32, …)` and passes `v` directly without loading. Same shape works for: (a) `self.0.dbg(buf)` on a tuple field; (b) `match self { Some(v) => return v }` for `unwrap` (return-value codegen knows to load). Method-dispatch codegen needs the analogous load. Blocks `impl Debug for Option/Result` (required for `result::test_fmt_default`), every `impl Trait for OptionLike<T>` where the trait method consumes payload by value. Workaround: rewrite to free-fn form (`fn debug_some<T: Debug>(v: T, buf: &mut String) { v.dbg(buf) }`) — bypasses the enum-payload binding. | — | — |
