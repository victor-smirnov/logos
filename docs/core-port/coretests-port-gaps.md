...
| CP-cm-03 | Bare-variant shorthand for Option/Result (`Some(x)`, `None`, `Ok(x)`, `Err(x)` without `Option::` / `Result::` prefix) | Open | Rust auto-imports `Option::*` and `Result::*` via prelude. Logos requires explicit `Option::Some(x)`. Same root cause as CP-cm-02. | Every coretest using Option/Result literally | Port-time workaround: add `Option::` / `Result::` prefix on every use. |
...
| SL-sl-07 | `ToString` trait + `.to_string()` on primitives | Open | `false.to_string()` returns "false". Rust has blanket `impl<T: Display> ToString for T`. Logos has Display via `fmt(self, &mut String)`; needs `ToString::to_string(&self) -> String` blanket impl. | Add trait + blanket. | — |
| SL-sl-08 | `Debug` for tuples | Open | `impl<A: Debug, B: Debug> Debug for (A, B)` etc. Currently Debug impls only for primitives, str, Ordering. | Add tuple-arity-specific impls in `std.fmt` (or pack-of-impls). | Driver: any assert_eq! comparing tuples. |
