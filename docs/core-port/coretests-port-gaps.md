...
| SL-sl-05 | Iterator adapter completeness | Partial (2026-05-14, updated 2026-05-14 with method-generic mono fix) | Logos already had free-fn adapters (iter_map / iter_filter / iter_take / iter_skip / iter_enumerate / iter_chain / iter_zip / iter_count / iter_fold / iter_for_each / iter_sum_i32/i64). 2026-05-14: trait-default-method shortcuts `.filter` / `.count` / `.for_each` / `.any` / `.all` / `.nth` / `.last`. **2026-05-14 method-generic mono fix**: `.map<MapOut>` and `.fold<Acc>` (when not chained) now work end-to-end. Two-piece fix in mono_scan.cpp: (a) enqueue_method_inst's root-pin/dispatch path skips methods with method-level tparams (only knows struct-level bindings); (b) enqueue_if_needed routes `Struct__method` callees through struct_method_templates_ when not in free-fn templates_, splitting type_args into struct-prefix + method-suffix. **Still open:** chained `.map(...).fold(...)` hits a separate sema receiver-type-args slicing bug (the `fold` dispatch on MapIter<I=…> uses MapIter<i32,…> instead of MapIter<SliceIter<…>,…>); `.find` / `.position` (raw-ptr-copy trait-default crashes emit-module — `baghunt_iter_trait_default_method_emit_segfault.md`); `take_while`, `skip_while`, `rev`, `peekable`, `cycle`, `step_by`, `inspect`, `min`, `max`, `product`, `collect` (needs FromIterator infra). | — | Regressions: `pass/iter_method_shortcuts.logos`, `pass/iter_terminals.logos`, `pass/iter_map_method_generic.logos`. |
