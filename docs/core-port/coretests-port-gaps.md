...
| CP-cm-12 | Mono enum-method specialisation drops method-level type-params (broader scope than originally noted) | Open (2026-05-14, deeper than expected) | **All** generic methods on generic enums with method-level type-params fail mlir-gen — not just `.and<U>` with enum-typed params, but also `.map<U>(fn(T) -> U)` etc. Root cause: `instantiate_enum_templates` in `mono_clone.cpp` (~3817) skips template fns whose `type_params.size()` doesn't match the enum's. So an `impl<T> Option<T> { fn map<U>(...) }` template gets ignored during `Option<i32>` enum-instantiation (its fn has [T,U], enum has [T] — size mismatch). Call site emits `Option__i32__map__g__Option__fn(T) -> U` but no fn with that name is produced. Tried partial fix (clone with struct-prefix subst, keep U in tail type_params) — produces a fn but body retains TypeVar(U) refs and downstream chokes ("unknown tagged enum 'Option__U'"); call-site spec doesn't kick in to substitute U. Existing struct-side `enqueue_method_inst` machinery handles this for structs but its enum equivalent (a worklist + drain loop) is missing entirely. | Implement `enqueue_method_inst` Enum path mirroring the Struct one: extend kind check to allow Enum; index `enum_method_templates_`; drain method worklist substitutes both struct-level and method-level type-params from the call-site's full type_args. | Deep mono refactor. Stdlib `Option::and<U>` / `Result::and<U>` / `or<F>` carry `GAP CP-cm-12` notes. Surprising scope: `.map<U>` was thought to work but actually fails too — no existing test exercises it because `tests/imported/pass/closures/hrtb-on-self-bound.logos` is the only `.map(` user and it's not on Option. |
