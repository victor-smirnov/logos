...
| CP-cm-08b | Tuple `==` for non-primitive fields (struct ✅ / str ✅ / nested tuple ◌) | Partial (2026-05-14) | Routed tuple `==`/`!=` through SL-sl-08's tuple-impl dispatch: stdlib `impl<A:Eq,B:Eq> Eq for (A,B)` / arity 3 / 4. **Direct `str.eq(&str)` (2026-05-14):** make_ref canonicalises `&UnsizedSlice<T>` → `Slice<T>` for implicit `&self`; slice-method dispatch retries with `Ref<Slice>` args flattened. **Tuple-with-str-field via Eq blanket (2026-05-14):** mlir-gen's `EAddrOfTemp` now treats `Slice` / `TraitObject` (already-ptr-shaped values) like Tuple/Struct/Array — returns the value directly instead of spilling to an 8-byte alloca and passing the alloca address (which made callees expecting "ptr to {ptr,i64}" read 16 bytes from an 8-byte slot → stack garbage). Regressions: `pass/str_eq_method.logos`, `pass/tuple_eq_str_field.logos`. **Still open:** Nested tuple fields (e.g. `((i32,i32), bool) == ((i32,i32), bool)`) infinite-recurse into the outer `$tuple$2__eq` spec instead of instantiating a separate `$tuple$2__eq__i32__i32`. Mono's TypeVar-MethodCall path injects the tuple element types as impl type_args, but for an inner method call on `A` (where A is itself a Tuple after subst) the injection seems to lose against the outer spec's type_args. Stack-overflow segfault on the recursion. | Trace mono_clone.cpp:2343-2506 for the inner-tuple case; type_args end up as the outer spec's `(i32,i32),bool` instead of the inner `i32,i32`. | Driver: tuple.rs `test_partial_eq` (nested 3-tuples). Regression tests `pass/tuple_eq_via_trait.logos`, `pass/str_eq_method.logos`, `pass/tuple_eq_str_field.logos`. |
