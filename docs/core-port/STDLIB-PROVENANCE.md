...
11. ⚠️ `core::fmt::{Formatter,Arguments,Write}` — multi-session migration in flight (see `docs/internals/fmt-migration.md`). Sessions 1+2+3 LANDED 2026-05-18..19. Session 1: support types + Formatter helpers + `FmtDisplay`/`FmtDebug` traits + i32. Session 2: all integer primitives + bool + str + `*const u8` + f32/f64 + tuples-1..4. Session 3: `String` Display+Debug, `Ordering` Display+Debug, `Vec<T>` + `Option<T>` + `Result<T,E>` Debug (after mono blanket-bound-recursion fix + is_self_referential refinement 2026-05-19), `FmtLowerHex`/`FmtUpperHex`/`FmtOctal`/`FmtBinary` for every integer primitive, `FmtLowerExp`/`FmtUpperExp` for f32/f64. `()` unit Display+Debug landed 2026-05-19 (PEG `LPAREN RPAREN` → TUPLE_TYPE; sema_collect/sema_decl Void special-case keeps mono target `"void"` in sync). Session 4 pending: metacall rewrite + legacy delete + rename.
