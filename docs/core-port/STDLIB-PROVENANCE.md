...
| `fmt/` | `std/fmt/fmt.logos` + `Display`/`Debug` traits in `mem/string/string.logos` | ⚠️ partial | `bool/test_harness_coretest_bool_to_string.logos` | `format!`/`print!`/`println!`/`eprintln!` metacalls; `Display`/`Debug`/`LowerHex`/`UpperHex`/`Octal`/`Binary`/`LowerExp`/`UpperExp`. No `fmt::Formatter` width/precision DSL parity (`fmt_pad`/`pad_into` is the manual API). `fmt::Write` trait absent. |
...
| `slice/` | `lang/iter/iter.logos` (`SliceIter`) + raw slice indexing via grammar | ⚠️ partial | `iterators/test_harness_coretest_iter_array.logos` | Slice iteration end-to-end. No `slice::sort`/`sort_by`/`binary_search`/`split_at`/`chunks`/`windows`/`SliceIndex` trait/`sort_unstable`. |
| `str/` | `lang/str/str.logos` + `lang/str/split.logos` + `lang/str/utf8.logos` | ⚠️ partial | — | `str_len`/`str_eq`/`str_starts_with`/`str_ends_with`/`str_index_of`/`str_contains`/`str_trim_start`/`str_trim_end`/`str_trim` + `Splitter`/`ByteSplitter` + UTF-8 helpers. Missing: `Chars`/`CharIndices`/`Bytes` iterators, `Pattern` trait, `Matches`/`MatchIndices`, `parse::<T>`. |
| `sync/atomic.rs` + `sync/mod.rs` | `lang/atomic/atomic.logos` + `mem/sync/arc.logos` | ⚠️ partial | — | `AtomicI32`/`AtomicU32`/`AtomicI64`/`AtomicU64`/`AtomicBool`/`AtomicPtr<T>` + `load`/`store`/`fetch_add`/`fetch_sub`/`compare_exchange`. No `Ordering` (memory order) — Logos atomics are sequentially consistent by default. No `AtomicI8`/`AtomicI16` etc. |
...
| File | Logos | Status | Notes |
|---|---|---|---|
| `mod.rs` | `std/fmt/fmt.logos` + `Display`/`Debug` in `mem/string/string.logos` | ⚠️ partial | metacall-based `format!`/`print!`; no `Formatter`/`Arguments`/`Write` trait. |
...
| `arith.rs` | `lang/ops/ops.logos` | ⚠️ partial | `Add`/`Sub`/`Mul`/`Div`/`Rem`/`Neg`; `*Assign` variants absent. |
| `bit.rs` | `lang/ops/ops.logos` | ⚠️ partial | `BitAnd`/`BitOr`/`BitXor`/`Not`/`Shl`/`Shr`; `*Assign` absent. |
| `function.rs` | — | ❌ TODO | `Fn`/`FnMut`/`FnOnce` — major gap (closures use `fn` pointer surface). |
| `deref.rs` | — | ❌ TODO | `Deref`/`DerefMut` traits; ref-coercion is compiler-builtin. |
| `index.rs` | — | ❌ TODO | `Index`/`IndexMut`; method-based indexing on `Vec` etc. |
...
| `control_flow.rs` | — | ❌ TODO | `ControlFlow<B, C>`. |
