...
11. ⚠️ `core::fmt::{Formatter,Arguments,Write}` — multi-session migration in flight (see `docs/internals/fmt-migration.md`). Sessions 1+2+3 LANDED 2026-05-18. Session 1: support types + Formatter helpers + `FmtDisplay`/`FmtDebug` traits + i32. Session 2: all integer primitives + bool + str + `*const u8` + f32/f64 + tuples-1..4. Session 3: `String` Display+Debug, `Ordering` Display+Debug, `Vec<T>` Debug, `FmtLowerHex`/`FmtUpperHex`/`FmtOctal`/`FmtBinary` for every integer primitive, `FmtLowerExp`/`FmtUpperExp` for f32/f64. `Option<T>`/`Result<T,E>` Debug deferred — `trait_engine`'s blanket-bound-resolver ignores type-args, so `Option<Vec<X>>` passes a FmtDebug check that `Vec<X>` fails directly (cascade). Filed as `baghunt-mono-blanket-bound-recursion`. `()` unit deferred (parser). Session 4 pending: metacall rewrite + legacy delete + rename.
