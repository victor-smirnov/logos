...
| `clone.rs` | `lang/clone/clone.logos` | ⚠️ partial | `clone/test_harness_coretest_clone.logos` | `Clone` + `Copy` traits with Rust-shape `fn clone(&self) -> Self`. `CloneToUninit` / `TrivialClone` absent. |
...
| `fmt/` | `mem/fmt/fmt.logos` (Display/Debug/Hex/Oct/Bin/Exp traits + impls + Formatter) + `std/fmt/fmt.logos` (macro shells + dispatchers + format_args_str fallback) | ⚠️ partial | `bool/test_harness_coretest_bool_to_string.logos`, `fmt_formatter_smoke.logos`, `format_macro_*.logos`, `fmt_session3_structs.logos`, `derive_debug_*.logos` | Sessions 1+2+3+4 LANDED 2026-05-18..19. `format!`/`print!`/`println!`/`eprintln!`/`eprint!`/`panic!`/`assert*!` metacalls all lower through sema-resident Formatter pipeline (per-placeholder spec field writes + dispatcher call). `Display`/`Debug` (Rust shape: `fn fmt_*(&self, f: &mut Formatter) -> Result<(), Error>`), `LowerHex`/`UpperHex`/`Octal`/`Binary` for all integer primitives + `LowerExp`/`UpperExp` for f32/f64 + tuple-1..4 Debug + `Option<T>`/`Result<T,E>`/`Vec<T>` Debug + `()` Display+Debug. `Formatter`/`FmtWrite`/`Arguments`/`fmt::Error` in `logos.mem.fmt`. **Method names stay `fmt_display`/`fmt_debug`/`_fmt_lower_hex` etc.** (not plain `fmt`) — Logos's flat per-type method registry collision blocks the canonical rename pending trait-aware mangling. DebugBuilders (`DebugStruct`/`DebugTuple`/`DebugList`/`DebugSet`/`DebugMap`) landed 2026-05-19 — single-line shape; pretty-print (`{:#?}`) deferred. Missing: `Arguments` is a placeholder (rendered String inside), not a deferred spec carrier; full `\n`/`\t`/`\u{…}` escape ladder for Debug-of-str; pretty-print mode. |
| `future/` | `std-new/rt/fiber/future.logos` | 🔁 | — | Logos has fibers (FutureSlot/poll-less); no `async fn` syntax / `Pin` / poll-based Future. |
| `hash/` | `lang/hash/hash.logos` | ⚠️ partial | `sip_hasher_basic.logos` | `Hash` + `Hasher` traits + `FxHasher` + `SipHasher` (SipHash-2-4) + `BuildHasher<H>` trait + `DefaultBuildHasher` → SipHasher (B103). Rust uses SipHash-1-3 for DefaultHasher; we use 2-4 (slightly slower, equally collision-resistant). |
| `hint.rs` | `lang/hint/hint.logos` | ⚠️ partial | `hint_smoke.logos` | `black_box<T>(x) -> T` (identity barrier; optimiser doesn't fold across calls today), `spin_loop()` (no-op stub — backend can later emit `pause` / `yield`), `unreachable_unchecked()` (defensive abort, `!`-return-type not exposed in Logos). `must_use`, `assert_unchecked`, `cold_path`, `likely`/`unlikely` not exposed. |
...
| `iter/` | `lang/iter/iter.logos` (2K+ LOC) | ⚠️ partial | 43 `iterators/test_harness_coretest_iter_*.logos` files + `iter_try_fold.logos` + `iter_once_with_repeat_with.logos` + `fused_iter_marker.logos` + `iter_array_chunks.logos` + `iter_map_windows.logos` | Most adapters ported (map/filter/filter_map/scan/fuse/take/skip/take_while/skip_while/step_by/inspect/peekable/once/empty/repeat/cycle/chain/zip/enumerate/rev/from_fn/repeat_n/once_with/repeat_with/flatten/intersperse/flat_map). `array_chunks::<N>` + `map_windows::<N, R>` landed B104 after closing the const-N array struct-field codegen baghunt. Short-circuit terminals `try_fold` + `try_for_each` (fn-ptr + closure form). `successors_with_seed`/`successors` with capturing closures still partially blocked on [[baghunt-mono-fn-ptr-field-typevar]] (type-verification fixed B104, runtime call-value bug remains). `by_ref_sized` deferred (auto-deref limit). `cloned`/`copied` intentionally omitted (Logos iters yield by value). `DoubleEndedIterator`/`ExactSizeIterator`/`FusedIterator` present; `TrustedLen` absent. |
...
| `num/` | `lang/cmp/ord.logos` + `lang/math/math.logos` + `lang/num/{newtypes,parse,error,float}.logos` + `lang/num/nonzero.logos` | ⚠️ partial | 16 `num/test_harness_coretest_*.logos` (i32/i64/u32/u64 batteries), `num_newtypes.logos`, `num_float_from_str.logos`, `fp_category.logos` | Heavy coverage for integer methods. `Wrapping<T>`/`Saturating<T>` newtypes (B100) for {i8..i64, u8..u64} (Saturating narrowed to i32/i64/u32/u64). `ParseIntError`/`ParseFloatError` + `FromStr` trait + integer/float walkers (`from_str_radix` for i32/i64/u32/u64; hand-rolled float parser for f32/f64) (B100). `FpCategory` enum + `f32/f64_classify`/`is_nan`/`is_infinite`/`is_finite`/`is_normal` + `to_bits`/`from_bits` (B101). Missing: `f128`/`f16` (no compiler support), `NonZero` method surface beyond constructors, full Rust-bit-exact `dec2flt`. |
...
| `option.rs` | `lang/option/option.logos` | ⚠️ partial | 11 `option/test_harness_coretest_*.logos`, `option_result_method_surface.logos` | Surface: `is_some`/`is_none`/`unwrap`/`expect`/`unwrap_or`/`unwrap_or_else`/`map`/`map_or`/`map_or_else`/`ok_or`/`ok_or_else`/`and`/`and_then`/`is_some_and`/`or`/`or_else`/`take`/`replace`/`as_ref`/`as_mut`/`xor`/`filter` + free fns `option_zip`/`option_unzip`/`option_flatten`/`option_unwrap_or_default` + `IntoIterator<T> for Option<T>` (`.into_iter()` through trait). Missing: `as_deref`/`copied`/`cloned`/`get_or_insert*`/`insert` (require Deref/Copy/Clone trait propagation), `flatten`/`zip`/`unzip` as inherent methods (require partial-spec inherent impl support — currently free fn only). |
...
| `ptr/` | `lang/ptr/ptr.logos` (+ compiler builtins `*const T`/`*mut T` syntax + `dst_from_raw_parts` from Phase 1B-14) | ⚠️ partial | `ptr_volatile_unaligned.logos` | `null`/`null_mut`/`read`/`write`/`read_volatile`/`write_volatile`/`read_unaligned`/`write_unaligned`/`copy`/`copy_nonoverlapping`/`swap`/`replace`/`drop_in_place`/`eq`/`addr_eq` + `NonNull<T>` (`new`/`new_unchecked`/`dangling`/`from_ref`/`from_mut`/`as_ptr`/`as_ref`/`as_mut`/`cast`/`add`/`sub`/`offset`). Volatile/unaligned today route through plain deref (x86 tolerates unaligned; optimiser doesn't reorder side-effecting loads). Missing: `without_provenance`/`with_exposed_provenance`/`hash`/`fn_addr_eq`, `Unique<T>`. |
...
| `result.rs` | `lang/result/result.logos` + `lang/iter/iter.logos` (ResultIter) | ⚠️ partial | 4 `result/test_harness_coretest_result*.logos`, `option-result/test_harness_coretest_qmark_from.logos`, `option_result_method_surface.logos`, `result_into_iter.logos` | Surface: `is_ok`/`is_err`/`is_ok_and`/`is_err_and`/`unwrap`/`unwrap_err`/`expect`/`expect_err`/`unwrap_or`/`unwrap_or_else`/`ok`/`err`/`map`/`map_err`/`map_or`/`map_or_else`/`and`/`or`/`and_then`/`or_else`/`as_ref`/`as_mut` + free fns `result_unwrap_or_default<T: Default, E>` / `result_flatten<T, E>` / `result_transpose<T, E>` + `IntoIterator<T> for Result<T, E>` → `ResultIter<T>` (B101 — `.into_iter()` works through the trait; for-loop sugar dispatch needs a separate sema hookup). Missing: `contains`/`contains_err` (Rust deprecated), `flatten`/`transpose` as inherent methods (require partial-spec inherent impl support — currently free fn only), `into_ok` (needs `!`/Never type with infallible-Err proof). |
...
| `str/` | `lang/str/str.logos` + `lang/str/split.logos` + `lang/str/utf8.logos` | ⚠️ partial | `str_pattern.logos`, `num_parse.logos` | `str_len`/`str_eq`/`str_starts_with`/`str_ends_with`/`str_index_of`/`str_contains`/`str_trim_start`/`str_trim_end`/`str_trim` + `Splitter`/`ByteSplitter` + UTF-8 helpers + `Chars`/`CharIndices`/`Bytes`/`Lines` iterators. `Pattern` trait + impls for `str` (substring) and `u8` (byte) + `str_find_pat`/`str_contains_pat`/`str_starts_with_pat` free fns landed 2026-05-19. `parse::<T>` lives in `lang/num/parse.logos` — `FromStr<E>` trait + `i32`/`i64`/`u32`/`u64` `from_str` + `from_str_radix` (`ParseIntError` chain). Missing: char-as-codepoint Pattern impl (needs UTF-8 decode at the haystack scan), `Matches`/`MatchIndices` iter wrappers, `FromStr` for floats. |
| `sync/atomic.rs` + `sync/mod.rs` | `lang/atomic/atomic.logos` + `mem/sync/arc.logos` | ⚠️ partial | atomic_ordering, `atomic_narrow.logos` | `AtomicI8`/`AtomicI16`/`AtomicI32`/`AtomicI64`/`AtomicU8`/`AtomicU16`/`AtomicU32`/`AtomicU64`/`AtomicBool`/`AtomicPtr<T>` + `load`/`store`/`fetch_add`/`fetch_sub`/`compare_exchange`. `Ordering` enum (Relaxed/Acquire/Release/AcqRel/SeqCst) + `_ordered` variants on every Atomic* type (B101). Narrow atomics (I8/I16/U8/U16) widen to 32-bit storage backed by the same asm stubs as I32 — correct on TSO; native 8/16-bit RMW would route through new asm stubs. All variants currently degrade to seq-cst on x86 (TSO + LOCK-RMW already provides it); weaker-memory targets (AArch64/RISC-V) route to dmb/lr-sc as backend codegen. |
...
| `tuple.rs` | per-arity stdlib impls (Eq/Display/Debug for 2/3/4-tuples) + planned variadic-tuple impls | ⚠️ partial | `tuple/test_harness_coretest_tuple_eq.logos` | Tuples are first-class; trait impls hand-written per arity (variadic `impl<A...> Trait for (A...)` planned — needs grammar `(A...)` + sema `$tuple$variadic` mangling + fold-expand operator; see `[[baghunt-variadic-tuple-impl]]`). |
| `ub_checks.rs` | — | ➖ | — | Internal UB-check scaffolding. |
| `unicode/` | `lang/char/char.logos` + `lang/unicode/unicode.logos` | ⚠️ partial | `unicode_basic.logos` | ASCII predicates + Latin-1 Supplement coverage (B103) via `unicode_general_category` + `unicode_is_alphabetic`/`is_lowercase`/`is_uppercase`/`is_numeric`/`is_whitespace`/`is_control` + `GeneralCategory` enum (Lu/Ll/Lo/Nd/Zs/Cc/Po/Sm/Sc/So/Other). Whitespace coverage extended to common Unicode spaces (EM SPACE, IDEOGRAPHIC SPACE, etc.). Full UCD tables (~100K codepoints) deferred — would need an auto-generated property-table module. |
| `unit.rs` | language built-in `()` | ⚠️ partial | `fmt_session3_structs.logos` | `()` Display + Debug impls landed in `mem/fmt/fmt.logos` (2026-05-19). Sema collect + decl special-case `Kind::Void` after `resolve_type(TUPLE_TYPE)` so impl-target mangles to `"void"` (instead of `$tuple$0`). No `Default`/`Clone`/`Copy` etc. impls for `()` yet. |
...
| `builders.rs` | `mem/fmt/fmt.logos` (DebugStruct/Tuple/List/Set/Map) | ⚠️ partial | `fmt_debug_builders.logos` | All five builders ship with single-line shape: `DebugStruct.field(name, &v)` / `finish` / `finish_non_exhaustive`; `DebugTuple.field(&v)` / `finish`; `DebugList.entry(&v)` / `finish`; `DebugSet.entry(&v)` / `finish`; `DebugMap.entry(&k, &v)` / `finish`. Pretty-print (`{:#?}`, multi-line indented) deferred — needs Formatter-level indent state. |
...
11. ~~`core::fmt::{Formatter,Arguments,Write}`~~ — multi-session migration LANDED 2026-05-18..19 (see `docs/internals/fmt-migration.md`). Session 1: support types + Formatter helpers + `Display`/`Debug` traits + i32. Session 2: all integer primitives + bool + str + `*const u8` + f32/f64 + tuples-1..4. Session 3: `String` Display+Debug, `Ordering` Display+Debug, `Vec<T>` + `Option<T>` + `Result<T,E>` Debug (after mono blanket-bound-recursion fix + is_self_referential refinement 2026-05-19), `LowerHex`/`UpperHex`/`Octal`/`Binary` for every integer primitive, `LowerExp`/`UpperExp` for f32/f64. `()` unit Display+Debug landed 2026-05-19 (PEG `LPAREN RPAREN` → TUPLE_TYPE; sema_collect/sema_decl Void special-case keeps mono target `"void"` in sync). Session 4 (2026-05-19): sema-resident format-family lowering rewritten to drive Formatter (per-placeholder field writes + `fmt_display(&arg, &mut __f)` / `fmt_debug(&arg, &mut __f)` / ... free-fn dispatchers); `fmt_pad`/`pad_into` deleted; legacy `mem.string::Display` trait + impls deleted; legacy `std.fmt::Debug` trait + per-primitive `dbg(self, &mut String)` impls deleted; 5 test files + logos_showcase metaprog migrated to FmtDebug shape; traits renamed `FmtX → X`. Method names stay `fmt_display`/`fmt_debug`/etc. distinct pending trait-aware method mangling. `Arguments` still placeholder (sema-resident expands the spec inline; no deferred Arguments object yet — same divergence as Session 1).
