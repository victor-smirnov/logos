...
11. ⚠️ `core::fmt::{Formatter,Arguments,Write}` — multi-session migration in flight (see `docs/internals/fmt-migration.md`). Session 1 LANDED 2026-05-18: support types + `Formatter::pad`/`write_int_dec_{i64,u64}`/`write_int_radix_u64`/`write_bool` helpers + `FmtDisplay`/`FmtDebug` traits with Rust-shape `fn fmt_*(&self, f: &mut Formatter) -> Result<(), Error>` + first migrated impl `FmtDisplay`/`FmtDebug for i32` + `fmt_display_to_string`/`fmt_debug_to_string` renderers. Method names suffixed (`fmt_display`/`fmt_debug`) to dodge name-clash with legacy traits' `fmt` method (Logos mangling doesn't include trait name); rename back to plain `fmt` happens in Session 4 alongside the legacy delete. Trait names `FmtDisplay`/`FmtDebug` similarly dodge cross-pkg B-mv-02 collision with `mem.string::Display` / `std.fmt::Debug`. Sessions 2-4 pending: rest of primitives → struct impls → metacall rewrite + legacy delete.
