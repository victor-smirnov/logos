...
11. ⚠️ `core::fmt::{Formatter,Arguments,Write}` — multi-session migration in flight (see `docs/internals/fmt-migration.md`). Sessions 1+2 LANDED 2026-05-18. Session 1: support types + `Formatter::pad`/`write_int_dec_{i64,u64}`/`write_int_radix_u64`/`write_bool` helpers + `FmtDisplay`/`FmtDebug` traits + i32 proof-of-shape. Session 2: all integer primitives (i8/i16/i32/i64/isize + u8/u16/u32/u64/usize) + bool + str (Display=pad, Debug=quoted) + `*const u8` (Display) + f32/f64 + tuples-of-arity-1..4 (Debug). `()` unit deferred — Logos parser doesn't accept `()` in impl-target position. Sessions 3-4 pending: stdlib structs (Option/Result/Vec/String/Ordering) + Hex/Octal/Binary/Exp traits → metacall rewrite + legacy delete + rename FmtDisplay→Display/fmt_display→fmt.
