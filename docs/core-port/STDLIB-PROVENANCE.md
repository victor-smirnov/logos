...
| `result.rs` | `lang/result/result.logos` | ⚠️ partial | 4 `result/test_harness_coretest_result*.logos`, `option-result/test_harness_coretest_qmark_from.logos`, `option_result_method_surface.logos` | Surface: `is_ok`/`is_err`/`is_ok_and`/`is_err_and`/`unwrap`/`unwrap_err`/`expect`/`expect_err`/`unwrap_or`/`unwrap_or_else`/`ok`/`err`/`map`/`map_err`/`map_or`/`map_or_else`/`and`/`or`/`and_then`/`or_else`/`as_ref`/`as_mut` + free fn `result_unwrap_or_default<T: Default, E>`. Missing: `iter`/`into_iter` (no IntoIterator impl for Result yet), `contains`/`contains_err` (Rust deprecated), `flatten`/`transpose` (blocked by [[baghunt-mono-eager-typevar-default-clone]] on nested-generic free fn signatures), `into_ok` (needs `!`/Never type with infallible-Err proof). |
