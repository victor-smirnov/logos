...
| `result.rs` | `lang/result/result.logos` | ⚠️ partial | 4 `result/test_harness_coretest_result*.logos`, `option-result/test_harness_coretest_qmark_from.logos`, `option_result_method_surface.logos` | Surface: `is_ok`/`is_err`/`is_ok_and`/`is_err_and`/`unwrap`/`unwrap_err`/`expect`/`expect_err`/`unwrap_or`/`unwrap_or_else`/`ok`/`err`/`map`/`map_err`/`map_or`/`map_or_else`/`and`/`or`/`and_then`/`or_else`/`as_ref`/`as_mut` + free fns `result_unwrap_or_default<T: Default, E>` / `result_flatten<T, E>` / `result_transpose<T, E>`. Missing: `iter`/`into_iter` (no IntoIterator impl for Result yet), `contains`/`contains_err` (Rust deprecated), `flatten`/`transpose` as inherent methods (require partial-spec inherent impl support — currently free fn only), `into_ok` (needs `!`/Never type with infallible-Err proof). |
