...
| `ops.rs` (re-exports from `ops/`) | `lang/ops/ops.logos` | ⚠️ partial | `deref_index_user_dispatch.logos` | `Add/Sub/Mul/Div/Rem/Neg/Shl/Shr/BitAnd/BitOr/BitXor/Not` traits + `Fn`/`FnMut`/`FnOnce` (variadic-pack `<A...>`) + `Deref`/`DerefMut`/`Index`/`IndexMut`. `*x` and `a[i]` operator-form dispatched in sema for user-typed structs that impl Deref/Index (mirror of the unary-operator-overload pattern). `*Assign` traits absent; `DerefMut`/`IndexMut` write-side dispatch deferred (write today via explicit `*x.deref_mut() = v`). Method-autoderef-through-Deref-chain (Rust's `box.method()` chasing pointee methods) deferred. No `ControlFlow`, no `Range*` types (Logos has its own `lang/range/RangeI32`/`RangeI64`), no `Try`/`FromResidual` (compiler-builtin in TRY_EXPR). |
