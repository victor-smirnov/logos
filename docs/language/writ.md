...
Every Writ value carries an 8-byte schema type code. Codes 1–128 are reserved for the standard registry (Map, Array, String, Decimal, …); user types get a 56-bit code from the SHA-256 of the type definition. `TypedValue` (tag 106) is the fallback wrapper for *unregistered* types only — standard registered types must use their direct code.
...
`@[expr for x in iter if guard]` produces a document with an `ObjectArray` root; `@{k: v for x in iter}` one with a Map root — the most direct way to build a structured Writ value from a Logos collection. See [Comprehensions](comprehensions.md).
