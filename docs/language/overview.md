...
- **Models are the primary author** for serious systems software. Their strength is the high-compressible side (algorithms, type systems, formal logic, large-scale generation); their weakness is execution — they cannot reliably *run* the algorithms they understand. So Logos offloads exactly that: explicit metaprogramming, verifiable type-level computation, deterministic codegen, persistent data structures. The C++ paradigm makes every load-bearing capability a first-class construct the model can name, manipulate, and verify, instead of work it does implicitly inside its weights. *Premature passivisation* — hiding hard features behind RFCs/macros because they confuse junior humans — is a tax the model does not need.
...
The discourse around AI and programming has whipsawed — from "it will never replace programmers" to "it already replaced them all" and back to a muddy middle (some, not all; and even those, not well). The settling expectation inverts the panic: *more* programmers will be needed, not fewer — both to clean up after code generated on autopilot, and because the frontier of what is worth building widens precisely because models *can* now build it.
...
The shift is as much economic as technical. Programming — corporate and open-source alike — has accumulated a deep reservoir of dissatisfaction with both its process and its output, which the old cost structure left little room to act on: people build what they can, what they enjoy, and what someone will pay for. Subsidised individual subscriptions change the arithmetic on both sides. **Supply side:** far more becomes feasible to attempt *for its own sake* — within one person's span of motivation a model covers more ground, in depth and breadth, than that person could alone, so a heterodox individual with unconventional ideas plus a capable model is a force that did not exist before (Logos is one such idea, materialised — for as long as the subsidised economics hold). **Demand side:** far more people get the program they *need* rather than the one its authors found *pleasant* to write; the authors deserve real gratitude, since the alternative to software written for the love of it is nothing but invasive, audited, penalty-laden EULAs. Progress is, in the end, steered by user *needs* — when they can be met another way, staying relevant means joining the shift, not resisting it. Nothing personal; just the real order of things.
...
Logos's response (axis 4): full compile-time metaprogramming as a load-bearing feature. A *metafunction* is ordinary Logos running at compile time through compiler APIs — normally via higher-level interfaces, dropping to AST level when needed. The C++ contrast is the whole point: template *expressiveness* is kept, but type-level programming as the vehicle for it — partial specialisation, SFINAE, tag dispatch — is replaced by ordinary metafunctions.

Second motivation, the C++-specific half of axis 3: in C++ those same memory mechanisms live only as a *library* on an unaware compiler, which then conservatively assumes the worst about aliasing/lifetimes/side effects — optimisation becomes a guessing game. Lifting them into the language makes the invariants facts the compiler sees, not gentleman's agreements with a generous inliner.
...
Logos borrows no concrete Haskell/Idris features, only the *direction*: the type system will be strengthened deliberately to improve the model reward signal (axis 6), and the language will significantly *grow as a layer on top of* its verification tooling. Near-term: a Datalog engine in the compiler used for resolution; SMT and constraint solvers to follow.
