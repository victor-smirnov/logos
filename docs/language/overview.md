...
The keyword `genos` is intentionally overloaded with the existing parametric-datatype declaration form. Both meanings are the same idea applied at different semantic levels — *parametric form*, classification by shared structure (Greek γένος = "kind, family"). Data-genos parametrises over field types; algorithm-genos parametrises over data types and operational abstractions. Future evolution may unify them syntactically once data-genos grows invariants and refinement clauses; for now the parser disambiguates by body shape.
