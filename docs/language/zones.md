...
## Self-allocation: mutating methods carry the zone allocator

The whole model above is implementable on **stock Rust** — nothing here needs a custom compiler. The cost is purely ergonomic, and it is about *allocation*: a bare zone object cannot grow itself. To get more memory inside its own zone an object needs the zone's allocator (the arena), and it has no way to reach one — it cannot store an allocator reference (that would be a machine pointer, which the zone rule forbids), and it cannot call `malloc` (that allocates in the root zone, not in the zone). So on its own a zone object is half-inert: readable and overwritable in place, but its mutating methods — a `push`, an `insert` — have no source of fresh zone memory.

**Logos closes this in the language.** A mutating method on a zone object implicitly receives — through a **"fat" receiver reference** — a handle to the **allocator associated with its zone**. Through that handle the method allocates more memory *in the same zone* and grows the object (the [Grow](#lifecycle) step: append a segment, or carve a child node). The allocator is threaded by the compiler and never stored: the object's own bytes still hold only self-relative offsets, never the allocator.

**Rust and C++ have no such mechanism** and must add it by hand:

- The zone object is **wrapped** in a value that carries the allocator reference; the wrapper forwards the allocator into each method call.
- In **C++** the wrapper must additionally be **owning** — it holds the zone alive (a refcount / holder) so the zone cannot be freed while a method is mid-call.

This is not a heavy burden — the wrappers are mechanical and can be **generated by metaprogramming**. But the Logos approach pays *nothing*: there is **no wrapper at all**. Because the compiler knows the full zone rule-set first-hand — who may allocate, what borrows what, when a base may move — it threads the allocator directly and is free to apply **extra optimizations out of the box** wherever it can prove them safe (for example, dropping the implicit allocator parameter entirely from methods that never allocate) — optimizations a wrapper-based encoding would hide from it.

**The main advantage is that this makes zone types *lightweight*.** No wrapper, no per-object allocator field, no owning shell — a zone object is just its bytes, and its cost is what the compiler emits, not what an encoding forces. Deep compiler integration thus makes a `#[zoned]` type **lighter than its Rust/C++ equivalent**, which lifts it out of the narrow serialization niche that arena-bound types occupy in those languages and lets it serve as a **general-purpose type**, used broadly. **Writ is the first thing built on that headroom:** it integrates a **lightweight yet high-level, dynamically dispatched object graph** (the `WAny` / `@{…}` substrate) directly into the language and runtime — something that stays a heavyweight library concern when every zone object must drag a wrapper.
...
- **Self-allocation** — a zone object's mutating methods implicitly receive the zone allocator through a fat receiver reference, so the object can grow itself in its own zone; the allocator is compiler-threaded, never stored in the object. No wrapper (Rust/C++ need one — owning, in C++). Wrapperlessness lets the compiler apply extra zone-safe optimizations out of the box, and makes zone types **lighter than in Rust/C++** — light enough to serve as general-purpose types; Writ is the first to exploit this (a lightweight, high-level, dynamically dispatched object graph built into the language/runtime).
