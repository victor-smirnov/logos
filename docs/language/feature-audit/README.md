...
19. ✅ `4cccc066` Lifetime-elision E0106 — reject the AMBIGUOUS case (≥2 input ref lifetimes, no `&self`, elided output ref) `fn h(a:&T,b:&T)->&T`; rules 2/3 (one input / `&self`) + explicit `&'a` elide fine. (0-input `Box::leak`-shape left to annotation/dangling-check.) Tests lifetime_elision_{e0106,ok} (D,A).
...
22. ✅ `##name` str-position antiquot — `HASH HASH IDENT`→`LIT_STR{NAME_VAR}` (grammar, both antiquot blocks) + blob-subst writes the ident text to VALUE (not NAME) when the placeholder node is `LIT_STR`, in BOTH paths (non-repeat `subst_walk`, repeat `expand_cursor_in_subtree`). Consumer: stdlib `derive_debug` rewritten onto the `DebugStruct` builder (`f.debug_struct(##nm).field(##fnames,&self.#fnames)…finish()`) → byte-exact Rust shape `Pt { x: 5, y: 7 }` (was positional `(5, 7, )`). Test core_6_10_derive_debug strengthened to exact-string assert. Same capability unblocks Default/PartialOrd label emit (J).
23. ✅ extern-fn-ptr TYPE `extern "ABI" fn(...)->R` COMPLETE. Grammar `0d1d05df`; identity closed here: ABI tag stored in the FnPtr-unused `struct_name` slot (FN_PTR_TYPE resolve reads VALUE, normalizes — `"Rust"`/no-extern ⇒ `""` so existing FnPtr UIDs unchanged), folded into TypeUID + structural equality + `type_str` diagnostic (`extern "C" fn()` ≠ `fn()`; mismatch now reads distinctly). ABI validated at type position too (rejects non-{C,C-unwind,system,Rust}). cconv: the accepted set is entirely SysV-equivalent on the x86-64-Linux target, so default cconv IS correct and no MLIR attr is needed — a genuinely non-SysV ABI (Win64/fastcall) is REJECTED, not mis-lowered; emitting those would be cross-target codegen (separate feature, not a fnptr-type gap). FnItem→extern-FnPtr stays ABI-lenient (C-callback interop; Logos fn items carry no ABI tag). Tests core_7_extern_c_fnptr (pass, dispatch=42) + extern_abi_fnptr_mismatch (fail) (N,B).
24. ◑ `69356297` intrinsic-level Ordering const-prop DONE. (B) **const-arg specialization ENGINE shipped** `44394fd3` (interproc const-prop on the mono foundation — mono_const_arg.cpp): a `const`-forwarding wrapper called with a literal Ordering gets a per-value spec clone with the literal baked → atomic op carries the precise ordering (acquire/release), proven by IR-snapshot test const_arg_atomic. Body-baking reuses the const-generic path; seed = in-mono registry (AST key space hard-full). **x86 perf gap CLOSED** for stores: disassembly proved the ENTIRE x86-64 ordering gap is `store_ordered` non-seq_cst getting `xchg` instead of `mov` (acquire-load == seq_cst-load == `mov`; RMW always `lock`; only seq_cst-store = `xchg`). So `logos_atomic_store*_ord` with a RUNTIME ordering now branches `SeqCst ? seq_cst-store : release-store` in mlir-gen (release soundly covers Relaxed) — non-seq_cst stores emit plain `mov`, no barrier. Literal orderings stay branch-free (read_ordering_at). Test atomic_store_runtime_ord. Residual (precise orderings EVERYWHERE incl. where x86 codegen coincides — loads/RMW) needs reviving multi-arena body resolution for the const-spec to reach stdlib binary-skeleton bodies; negligible on x86. Doc: docs/internals/const-arg-specialization.md (G,H).
...
28. ✅ `5e9203ad`+`ccf81717`+`b91df6b5`+`53f49675` **Fully-qualified dotted+`::` path — DONE** (3 increments): (1) package-qualified free-fn calls `pkg.path::fn(args)`/`::fn::<T>(args)` (exact-package resolution disambiguates same-named free fns, `logos.lang.mem::replace_ref` vs `logos.lang.ptr::replace`); (2) type-member calls `pkg.path.Type::method(args)` + no-paren `pkg.Enum::Variant`/`pkg.Type::CONST` (last dotted seg = type); (3) type-position `let x: pkg.path.Type`/params + UFCS `<T as pkg.path.Trait>::m`. Mechanism: grammar dotted-prefix alts (call_expr/enum_lit/simple_type) + QUAL_PARTS field + sema `call_pkg_qualifier_` exact-package filter + last-segment-as-type derivation. RESIDUAL: generic qualified type-position `pkg.Type<A>` (`$...` slurps path segs — use `use`+short name) (I,E).
29. ✅ `a83e6ed5` Uninhabited-variant arm elision in exhaustiveness (O).
30. ◑ Nested `#(…)*` repetition — `quote_item!` depth-2 DONE (ragged `Vec<Vec<Ident>>`; outer index pinned into the depth-2 cursor placeholder between the two expansion phases; CSR cursor blob `CursorHdr{count,pods_offset,inner_counts_offset}`; depth array threaded to `logos_qib_pack_cursors`). Fixed a LATENT realloc-dangle (GrowableSingleChunk `doc` frees its chunk mid-copy → new `Arena::reserve` front-loads one grow to an expansion upper-bound). Test quote_nested_repeat (ragged 2+1 → 203), valgrind-clean, depth≥3 graceful error. REMAINING: `quote_expr!` nesting (separate machinery sema_expr.cpp:16287, no consumer — expr-context nesting rare). Doc: docs/internals/quote-nested-repeat-design.md (J).
