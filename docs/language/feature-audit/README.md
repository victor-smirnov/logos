...
23. ◑ `0d1d05df` extern-fn-ptr TYPE grammar `extern "ABI" fn(...)->R` DONE (parses + type-checks + dispatches; ABI parse-transparent — sound on x86-64-SysV where default==C). REMAINING: distinct extern-ABI FnPtr type identity + ABI tag on `Kind::FnPtr` (+UID) + non-default cconv (Windows/sysv64/fastcall) → MLIR cconv attr (N,B).
