...
24. ◑ `69356297` `AtomicUsize`/`AtomicIsize` + intrinsic-level Ordering const-prop (`read_ordering_at`) DONE. REMAINING (PERF-ONLY, sound seq_cst fallback): Ordering through `*_ordered` WRAPPER methods (`load_ordered(ord)` forwards a param VAR_REF → not a literal EnumLit → seq_cst); needs fn-inlining or interproc const-prop, which Logos lacks (G,H).
