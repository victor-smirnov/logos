...
- quote blobs ride the **hermes** AST API since the hermes2 cut-over (`0c4caa0e` fixed the splice-time `HermesStatic` materialization + DAG-safe `collect_param_slots`); user doc `docs/language/reference/macros.md`.

**Match verdict: WARN — blessed §A3 divergence (rename + model shift).** Strictly more general (Turing-complete codegen vs declarative TT matching). Unchanged since v1 in classification; substrate fully re-platformed onto hermes with no surface change.
...
- Splice shims: `logos_emit_item_blob_subst` (`src/compiler/main.cpp:327`), `logos_macro_arg` (`:119`); emitters `stdlib/std/compiler/metaprog/emitter.logos`, `itemlist.logos`. Handlers walk the target item via `meta_*` reflection over the hermes doc (read layer re-ported `996ca163`).
...
- **Items (output):** OK. `ItemList`/`QuoteItemBlob` splice via `logos_emit_item_blob_subst` thunks; `template`/`template_of` declaration-as-data (`logos.peg:582`; `template_of` lowering fixed to the hermes offset model in `0c4caa0e`).
...
- **M (Const eval):** metacall shares the JIT driver with all macro flavors; serialization now hermes-native (`746a42e0`).
