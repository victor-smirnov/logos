...
### moves + variance (57+36 originals; agent direct, 4 gaps linked+run; top-3 re-verified by me)
9 confirmed soundness GAPs. Top-3 re-verified personally (GAP6+GAP2 at runtime, GAP7 in source):
- **GAP6 — conditional `Copy` impl bound UNCHECKED (memory-unsafe, hits STDLIB).** `impl<P: Copy> Copy for MyPin<P>{}` makes every instantiation Copy without checking `P: Copy`. `MyPin<&mut i64>` → Copy → two live `&mut` aliases, second write wins (exit 222, verified). **`stdlib/lang/pin/pin.logos:30` ships exactly `impl<P: Copy> Copy for Pin<P>{}`** — the comment there deliberately narrowed blanket→conditional but the bound isn't enforced, so `Pin<&mut T>` is wrongly Copy in real stdlib. (bug_move_copy_checker_holes)
- **GAP1 — implicit Copy for any drop-free non-owning struct.** Move-checker approximates "Copy" as "needs no drop glue" instead of "nominally impl Copy". `struct P{a:i64,b:i64}` reused twice → compiles (Rust E0382). Same root class as GAP6.
- **GAP2/3/4/5 — move-tracking holes → double-free** (straight-line if/else is sound; these are context-specific): loop back-edge (GAP2, re-verified exit 134 `double free detected in tcache 2`), match-guard re-eval (GAP3), slice/array-pattern destructure through `&[D;4]` (GAP4), conditional-move-in-loop used after (GAP5). All agent linked+ran → double-free.
