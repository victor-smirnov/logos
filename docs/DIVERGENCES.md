...
| B7 | **enum heap-representation** — every enum value is a heap-malloc'd `{tag,payload}` block (heap-ptr repr); the block is never freed → **leak** on every enum that goes out of scope (Copy-payload enums get no drop at all; droppable-payload enums drop the payload but never the block). Also forces a malloc+memcpy "heap-promotion" hack when an enum is stored into a struct field / through `&mut`. | Rust: enums are **by-value inline** `{tag,payload}` (no heap block, no leak, no promotion). | Convert to value-representation (ptr-to-inline-**stack** storage, like Struct; one-level `&Enum`; memcpy on field/element/assign writes). Sema/Mono unchanged (recursive enums already require `*const`/Box indirection via `check_recursive_value_types`). Mostly DELETION of mlir-gen enum special-cases. All-at-once flip of ~15 value/ptr sites, full-suite gated. **Full plan: `docs/memory-management/ANALYSIS.md §8`.** | **high** (pervasive leak; foundational) |
| B8 | **assignment doesn't drop the old value** — `x = y` for a live owning `x` overwrites without dropping `x`'s prior contents → **leak**. | Rust `expr.assign.drop-target`: drop the old value first, *unless the place is uninitialized*. | Blocked on **definite-assignment analysis** (flow-merged init state, dual of `moved_vars_` snapshot/union) or dynamic **drop-flags** — Logos has neither (`sema_stmt.cpp:1670`). Without it, dropping is UB on conditional-init paths (an earlier attempt was reverted as "too aggressive"). Implement the prerequisite, then drop-before-replace in `lower_assign`. | high (leak; needs the analysis feature first) |
