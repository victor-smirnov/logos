...
| B9 | **Box ergonomics stale** — `Box<T>` accessors use raw-pointer self (`get`/`get_mut` over `*mut Box<T>`), not `Deref`/`DerefMut` (`*b` / auto-deref `b.field`); and `Box<dyn Trait>` / `Box<[T]>` (`?Sized`) are not representable by the stdlib struct (rely on the compiler's separate dyn-collapse). NOT a leak — RAII landed (box_free/vec_free removed). | Rust: `*b`, `b.field`/`b.method()` via Deref/DerefMut; `Box<T: ?Sized>` fat pointer. | Add `impl Deref/DerefMut for Box<T>` (needs Deref-coercion / `*b` sugar support — check if the compiler has it) + `?Sized`/fat-ptr Box layout (depends on B2/B3 custom-DST). Ergonomics modernization, not correctness. | low (ergonomics) |
