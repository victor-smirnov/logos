...
| B5 | **Dynamic-slice PATTERNS** — `match slice { [a, b] => …, [h, ..] => … }` over a `&[T]` (runtime-length) scrutinee. (The `&[T;N]`→`&[T]` *coercion* part of this row is now CAUGHT UP — see "Recently caught up".) | Rust slice patterns with fixed prefix + optional rest | Codegen only (sema already accepts + checks: refutable, one `..`, no suffix-after-`..` for dynamic slices — sema_stmt.cpp:3059-3108). Three distinct failures, all in mlir-gen: (a) **match-AS-EXPRESSION** `let r = match s {[a,b]=>…}` → falls to the default arm-dispatch → `arith.cmpi` on the slice `!llvm.ptr` (verify fail); (b) **match-AS-STATEMENT** with block arms → routed through yet another lowering that leaves a stray `arith.constant` (LLVM-translate fail) — note `gen_match` (stmt) is NOT reached, so the path needs tracing; (c) **`&[_, _]` ref-pattern** form → sema "reference pattern requires reference scrutinee, got '&[i64]'" (in Logos `&[T]` is one Slice type, so the faithful pattern is `[_, _]` without the leading `&`). Needed: a `Kind::Slice` scrutinee branch in BOTH match codegens — load len (field 1 of the {ptr,len} desc), gate `len == N` (no rest) / `len >= N` (rest), GEP prefix elements through the data ptr (field 0) for literal-eq tests + bindings. A first stmt-side attempt was reverted (unreachable for the test shapes — sema routes statement slice-matches elsewhere); trace that routing first. Affected (still skipped): `tests/ui/match/match-tuple-slice.rs` (discriminates by slice length — a fixed-array port would be degenerate). | medium |
