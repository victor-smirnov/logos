...
| B5 | `&[T; N]` (sized array ref) does not unify/coerce with `&[T]` (slice) — incl. as tuple/struct fields: `(i64, &[i64;2])` ≠ `(i64, &[i64])` | Rust's unsized coercion `&[T; N]` → `&[T]` | Proper fix is the coercion mechanism, NOT a representation swap (canonicalising `&[T;N]`→Slice in resolve_type breaks `array_from_ref`/`array_from_mut`, which legitimately return a thin `&[T;1]`). Needed: (1) `&arr` produces `Ref(Array<T,N>)` (thin) instead of the current eager Slice; (2) a coercion `Ref(Array<T,N>)` → `Slice<T>` (build fat ptr: elem-ptr + static len N) at flow sites — let/arg/return + structurally inside tuples/structs (mirror `try_coerce_*`). High blast-radius (`&arr` is everywhere) → focused pass. Affected: match-tuple-slice. Slice patterns with rest (`&[_, ..]`, `&[]`) are a separate, not-yet-exercised feature. | medium |
