...
| B2 | **custom-DST tail-slice — read-side DONE (2026-05-29, `1088b703`).** `struct Foo { hdr: H, tail: [T] }`: declaration, fat `&Foo` carrying the tail length, prefix field access (`f.hdr`), tail-as-slice (`f.tail` → `&[T]`), `.len()`, `f.tail[i]`, `dst_from_raw_parts::<Foo>(raw,n)` construction over existing memory, `&Foo ↔ *const u8` casts — verified at runtime (`custom_dst_tail_roundtrip`, exit 110), under `unsafe`. | full custom-DST layout + construction | **Remaining**: owning `Box<Foo>` (heap-alloc `offsetof(tail)+n·size(T)` + DST-aware drop_in_place+dealloc — a new collapse like Box<[T]>; deferred until a concrete consumer validates — HermesString stays Zone-hand-rolled per Victor 2026-05-29). By-value construction stays impossible (unsized — Rust agrees). | medium (read-side done) |
