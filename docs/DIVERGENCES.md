...
| A11 | integer widths | `i{8,16,32,64,128}` / `u…` / `i/usize` | the same set PLUS Hermes-fabric widths `I24/U24/I56/U56` (`sema.hpp:46-138`) for packed datatype/view storage. | addition | Logos-only widths serving the Hermes datatype fabric — not parity items (a Rust port never names them); here so "Logos has extra int kinds" isn't read as a gap. |
