...
- **SEMILATTICE** (min / max — idempotent, *no inverse*): **stateful re-min / re-max on retraction.** Insertion is O(1): `best[key] = meet(best[key], v)` (min) / `join` (max), emit only on strict improvement — this mirrors the batch absorb gate exactly (`emit_rel_push_agg`, `rexpr_walk.logos:3494-3524`, strict `<`/`>` at `:3500`). **Retraction is the hard case**: retracting the current best re-opens the group and requires the *next* best. Two maintenance strategies, chosen per-group:
  - **Multiset of live values per group** (`HashMap<K, multiset<V>>`): retraction removes one occurrence; if it was the extremum, the new extremum is the multiset's next element. O(log n) with an ordered structure; O(n) with a plain re-scan of that group's values only (never the whole relation).
  - **Count-of-best**: keep `best[key]` plus `count_at_best`. Retraction of a non-best value is O(1) (no emit). Retraction of a best value decrements `count_at_best`; only when it hits 0 do we re-scan for the next best. This bounds re-mins to the *frequency of extremum churn*, which is low in the SSSP/reachability workloads (R2). **Correction (review U2): count-of-best does NOT save value storage** — the "re-scan for next best" needs the group's live values, which it does not itself retain, so it must rescan the *upstream integrated arrangement* (`A_int`, §2.3). Its only saving over the multiset is avoiding a maintained ordered structure until churn forces it; it is therefore not an independent storage strategy but a lazy variant of "multiset, materialized on first best-retraction". Commit to one at the R2 slice.

  Slice-1 value column is **i64** (ordered lattice; f64/str rejected, `rexpr_walk.logos:3754`) — same constraint as batch.
...
2. **Retraction / DRed (over-deletion → re-derivation)**: a **negative** EDB delta can invalidate derived facts. The standard DRed three-phase, run as plain loops within the epoch:
   ...
   Under **pure signed-weight Z-set semantics** with `distinct`, DRed is subsumed by weight cancellation *for linear+distinct recursion*: a fact retracted on one path but re-derived on another nets weight ≥ 1 and stays. DRed's explicit three-phase is only needed where `distinct`/antijoin/semilattice-agg break weight-linearity (see §4.3). **Slice-1 defers all recursion**; this section is the design-for target for R1/R2.
...
- **R2 incremental retraction (re-min-in-fixpoint / DRed-for-lattices)** is *designed* but not *resolved to an algorithm choice*: §3.1 offers two maintenance strategies (multiset vs count-of-best) without committing. That commitment is deferred to the R2 implementation slice — legitimately, since slice-1 excludes recursion and aggregates, so the choice is not yet forced. **Flagged, not hand-waved.**
- **DRed vs weight-cancellation boundary** (§4.2/§4.3): the claim that "signed-weight cancellation subsumes DRed for linear+distinct recursion, explicit three-phase only where distinct/antijoin/semilattice-agg break linearity" is *correct in the DBSP literature* but the exact set of RExpr shapes where three-phase DRed is actually required needs enumeration during the recursion slice. Deferred, but bounded.
