...
2. **Retraction / DRed (over-deletion → re-derivation)**: a **negative** EDB delta can invalidate derived facts. The standard DRed three-phase, run as plain loops within the epoch:
   ...
   **STATIC TIER, LANDED (2026-08-08).** The three phases are emitted per recursive SCC beside the cross-epoch insert driver `_i` (`rexpr_walk.logos`, `emit_scc_od_fns`): `_od` (seed — base + recursive bodies at the deleted rows, over the previous total), `_odp` (propagate — recursive bodies only, over the survivors, read == write == `od`), `_cpt` (compact — in-place retain + `pop` to `T ∖ OD`, refusing with `ElError::RetractAbsent` from a read-only pass *before* it writes if `OD ⊄ T`, and handing back the removed rows `OD ∩ T` in `__rm_<m>` plus `|OD|`/`|S|`/`|RD|` latches in `__lat`).
   **PHASE 3 IS `_i`, NOT A FOURTH SIBLING.** Re-derive = the same emitted `_i` re-entered with the SURVIVOR edb in place of the accumulated one and `__tot := S` — i.e. `_cpt`'s own `&mut Vec` handed straight on, since in `_i` the shadow set and the total are the same variable (the plan's "tot = 0, shadow set pre-populated" would empty the shadow set and recompute the whole closure — right answer, no incrementality). `RD` is the suffix `[b0 .. len)` with `b0` captured *after* `_cpt`, because `_cpt` shrinks the vector. `_odp`, by contrast, needed its own sibling: its shadow set is `OD` and starts near-empty, so `_i`'s base bodies would inject every surviving base fact into `OD` — the base bodies are wanted in phase 3 and poison in phase 1b.
   **Completeness under a SHRINKING input**: `_i`'s proof is stated as the hypothesis `T₀ ⊆ lfp(E')`, discharged by monotonicity for the insert caller and by the survivor lemma `S = T ∖ OD ⊆ lfp(PRES)` for phase 3, which follows from the `SOME derivation` form of the `OD` spec. The lemma is the soundness of phases 1+2 *together* and is asserted row-by-row against an independent closure (`pass/wql_incr_rel_od_phase3.logos`, `pass/wql_incr_rel_dred_product_random.logos`) — phase 3 only appends, so a row phase 2 wrongly keeps is unrepairable.
   **NOT YET**: no retraction SURFACE. `incr_retract_eligible` still refuses every rel-backed query; there is no epoch driver, so `|RD|` stays the `-1` "never ran" sentinel and the negative fold of `rm` has no consumer. Aggregate-headed SCCs get no `od` siblings at all.
...
**Slice-8 update (LANDED): `FactStore` — the first real (A) producer.** A mutable in-memory fact-store (`stdlib/std/deem/deem.logos`, `pub struct FactStore`, one per named source relation) now OWNS the U1 clamp and PRODUCES the changelog: `insert`/`retract` maintain a per-fact SET (presence 0/1) keyed on the schema's **scalar** fields (`field_read` at field granularity — whole-Node identity is impossible since `rt_eq(Node,Node)=false` and the engine dedups on `node.raw()`), interning content to ONE **canonical handle** re-emitted every epoch so the engine sees a stable pointer. Insert-of-present / retract-of-absent emit **no** delta; re-insert-after-retract emits `+1`; `take_delta()` moves out the accumulated `±ZBatch` and resets — feeding `IncrJoin::epoch`/`IncrRec::epoch` **unchanged** (drivers `pump_join`/`pump_rec`). This is exactly the `src_apply` clamp the harnesses did by hand, now OWNED by the store, behind the same source-agnostic `ZBatch` seam. Proven end-to-end green: unit deltas (`query_incr_factstore_unit`), a store-driven transitive-closure fuzz battery (`query_incr_factstore_e2e_rec`, incl. cyclic-graph DRed), and a two-store equi-join battery (`query_incr_factstore_e2e_join`), each snapshot-vs-batch-oracle over `build_present()`. Still deferred: **WMap-container mutation instrumentation** (the store is a clean drop-in producer, not yet the container itself) and **(B) CoW/snapshot-diff**.
...
| **R4** | Provenance / witness trees (derivation trees; semiring provenance) | **FIRST CUT DONE (slice-9, TC) / DESIGN-FOR (full)** | **Slice-9 (2026-07-04)** ships the bounded first cut: `IncrRec::explain(a,c)` reconstructs the WITNESS chain (immediate-derivation tree) for a derived path fact of a linear-recursive transitive closure and walks it to base edges. **Design A — on-demand reconstruction, NOT stored-at-derivation**: the derivation site (`qrel_push_out`) sees only the projected `(a,c)` — the midpoint `b` is discarded in the Proj arm — so surfacing it for a stored witness needs an OutTab/exec-signature change threaded through the shared batch+incremental executor (the §10.2 concrete risk) plus a witness remap across DRed COMPACT. Instead `explain` recomputes the *same* witness record (`WitStep{is_base, supp_fid, edge_id, src, dst, a, c}`) post-hoc by a BFS over the current concrete edge set from `a`, restricted to path-fact frontier nodes, reading off a canonical SIMPLE path (visited-set ⇒ terminates on DAGs/diamonds/cycles). Because nothing is stored, **DRed-validity is free**: a retracted fact fails the present-check → `present:false`; a re-derived fact yields a fresh chain; no index-keyed witness dangles across COMPACT's row-reassignment. Cost stated honestly: the witness is *canonical* (BFS-deterministic), not *historically-first* — equivalent for set-semantics why-provenance and path reconstruction. Test `query_incr_prov_e2e` (chain / diamond / DRed retract+re-derive / cycle). **STILL DESIGN-FOR (the full R4)**: per-operator provenance for join/antijoin/aggregate (each operator attaches its rule to derived rows — the operator-signature change of §10.2), MULTI-witness (all alternative derivations, not one canonical), WHY-NOT (absence) provenance, and stored historically-first witness (Design B). |
