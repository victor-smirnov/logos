...
**T4 REFRAMED AND (STATIC HALF) SHIPPED 2026-07-09 (4f0d713c).** The "T4 gates on M4 tag-traverse" premise was WRONG, per the language's own design: native objects are deliberately UNTAGGED — types are known statically or through dyn Trait/TypeId; the tag system is a Writ-style special case. So case 2b's static form needs NO tags: `#[derive_graph_source]` (stdlib metaprog handler) generates the traversal by compile-time reflection — walker + materializer + `impl GraphSource for T` per annotated struct, the same eight-column vocabulary, node id = address, tag = 0. `**`/[*]/{kind}/joins/generic mappings apply to native objects unchanged. Ordering: the pre-scan seeds the natspec BY CONVENTION from the annotation one metacall round before the emitted impl collects; identical re-registration = confirmation (the idempotence discipline spans all three registries, which now ride the sema state snapshot across rounds). f64 leaves: vi = IEEE-754 bits (d9c96eb7), both walkers — bit identity is the honest Eq for floats; the same fix ended the Writ walker's silent f64-value drop. Remaining T4 dynamic half: dyn-Trait fields through the VTABLE + TypeId — the dyn alternative to tags; plus rules-backed impl sugar over the generated base relation if ever wanted (generic mappings already cover the derived-vocabulary case better).
...
**T3 LANDED 2026-07-09.** `mapping M<S: Bound>(g: &S)` — one type param, one source-trait bound; the source param's type must BE the type param (`g: &S`); scalars = later slice. A generic mapping is a PURE rule module: lower_mapping_def registers it and emits nothing (its rule bodies are validated at first consumption, like templates). Instantiation = fusion: `deem!(w: Reach<Chain>)` checks the bound per-trait (every rel of `trait_rels_[bound]` bound in `source_impls_[concrete]` — by trait, not by name accident), substitutes `&S → &Chain` in the ONE place S appears (the emitted signature), and the standard post-substitution natspec supplies the concrete materializers to the spliced rules. Gates: wql_mapping_generic_e2e (ONE closure mapping over TWO unrelated source types, consumer rel on top, consumer-before-mapping), fail wql_mapping_generic_unbound_fail (missing rel binding named at the site).

**Scalar binding LANDED 2026-07-09.** `mapping Limits(g: &Writ, floor: i64)` consumed as `deem!(w: Limits, floor: i64)` — a mapping's scalar params bind at the consumption site by NAME IDENTITY: the consumer declares a param with the same name and type, and the spliced rule bodies resolve the scalar exactly as the mapping wrote it. No EL rename, no new syntax; missing scalar = named error. Works for generic mappings too (`Far<Chain>(g, min_b)` — the T3 source-param-only restriction is lifted); params after the source must be scalars (i64/f64/str/bool). An explicit-binding sugar (`w: Limits(floor = my)`) remains possible later if two instances of one mapping ever need different bindings in one query. Gates: wql_mapping_scalar_e2e (same query, different bound per call; generic + scalar over a user source), fail wql_mapping_consume_scalar_fail.
