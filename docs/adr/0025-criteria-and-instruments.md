...
| §12 | the `direct` stream form — ⚠ **THE RECORDED BLOCKER WAS FALSIFIED AT R-E, TWICE**: the typeof-in-module compiler fix LANDED (sema.cpp typeof arm → demand + `pass/typeof_container_hand_written_state`), AND `direct` never depended on it (the emitter spells the concrete walk type as TEXT from `MacroParams`). The TRUE blocker, found by landing the precondition: `direct`'s state struct holds the emitted `<C>LeafWalk` as a FIELD, and `sema_abi_layout` declines a metaprog-emitted struct field asked from a foreign package (returns {8,8}; `LOGOS_VERIFY_LAYOUT` aborts — sema 8 vs DataLayout 72; the class is named at `sema.cpp` `find_struct_repr_` comment, never reached because no emitted struct in the corpus holds an emitted struct as a field — a permissive defect invisible to a green corpus, third instance). ⚠ AND THE WIN IS SMALLER THAN PRICED: seam-1 is 1 packet at 489 of 502 sites BECAUSE THE SOURCE IS 1 PACKET (SliceStream/no-stream), which `direct` cannot change; the measurable effect is 13 container-walk sites (⌈leaves⌉ packets); the eligible range 276–315 of 606 is a criterion-1 PESSIMIZATION (~−1.2 to −1.4 pp accounted, both sides shrink). **2026-08-18 UPDATE: #62 IS FIXED** (the decline was ROUND-ORDER, not layout: `run_metaprog_dispatch` advanced the delta cursor across an UNCACHED retry round, orphaning every `logos.gen` registration, and `resolve_type`'s metaprog swallow froze the miss into an Error field with no pending — closed by not advancing the cursor + the emitted-type `note_pending_` demand; pair `pass/deem_emitted_struct_field_layout` + `fail/emitted_struct_field_unknown_fail`). **The direct landing then STOPPED at a SECOND, distinct blocker: the #70 class** — `Option<T>::replace` (option.logos, `replace_ref(self, Option::Some(value))`) false-reds "use of moved value" when mono instantiates `Option<T>`/`Result<T,E>` UNCALLED for a `#[borrow_carrying]` T that also holds an owned Drop field — which the direct state struct necessarily is (emitted walk + owned scratch), and the surface must stay `Result` (`pass/deem_pipeline_handle_seam:114` pins the Result-typed container-walk stream). 15-line no-metaprog repro pair `sandbox/s5direct/min2,min3.logos` + green control `min_opt_bc.logos`. **Everything else of direct is PROVEN green end-to-end**: `sandbox/s5direct/probe_bare.logos` — emitted borrow-carrying state struct holding the walk as a FIELD, multi-packet pull (`pk >= 2`), rows+sums equal the cursor oracle, 0 declines under the verifier. The three buffered ground sentences in `rexpr_walk.logos` now name #70, not the fixed decline | **#70 first; then direct is emitter plumbing** (pull_shape pins re-derive UP, seam-1 packet pin re-derives with the stage) |
...
* **`direct`** — ~~blocked on **#62**~~ **2026-08-18: #62 is FIXED and the
  blocker moved ONE deeper — the #70 class** (`Option<T>::replace` false-reds
  under mono's uncalled instantiation for the borrow-carrying state struct;
  the §12 row above carries the full measurement, including `probe_bare`'s
  end-to-end green proof of every other part of the direct shape).
...
that — is blocked on **#62** (2026-08-18: #62 FIXED; the blocker is now **#70**
— see the §12 row; the verdict sentence itself is unchanged because the output
plane still fills to completion). Everything else on the plane is either integrated
...
| **output plane** | **606 + 45 + 1** | ⚠ **UNDECIDED — blocked on ~~#62~~ #70 → `direct`** (08-18: 606→607 with the #62 fixture) |
...
|---|---|---|
| L1 | **~~#62~~ #70 → `direct`** | ~~fix `sema_abi_layout` declining a metaprog-emitted struct FIELD~~ **#62 DONE 2026-08-18** (round-order, not layout — see §12); the step re-opens as **#70 → `direct`**: fix the arg-position borrow-composition false-red (`Option<T>::replace` under uncalled mono instantiation), then `direct` is emitter plumbing — the emitted shape is already proven green end-to-end (`sandbox/s5direct/probe_bare.logos`); success unchanged = the output plane's 607 landings stop being filled to completion and `logos_09_pull_shape`'s pins move UP with a derivation comment |
