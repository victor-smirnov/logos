...
| §12 | the `direct` stream form — ⚠ **THE BLOCKER MOVED A THIRD TIME, AND THIS ONE WAS FOUND BY ACTUALLY LANDING THE EMITTER.** History: R-E falsified the typeof-in-module blocker twice; **#62** (the emitted-struct-FIELD layout decline) closed at `d90a89a4`; **#70** (a nested borrow-forming call in ARGUMENT position depositing no borrow sources) closed at `fb8dc2b7`. With both closed the Result-wrapped constructor was re-probed BEFORE the emitter was touched — `sandbox/s5land/probe_res.logos`: rc 0 under `LOGOS_VERIFY_LAYOUT=1`, `pk >= 2`, rows+key-sums equal the cursor oracle, bite-proven by perturbing `pk < 2` → `pk < 200000` for rc 3 (the probe's own arm). The emitter was then written (`emit_stream_direct`: state struct + inherent `next_batch` + `BatchStream` forward + `Result<<Q>Dx, ElError>` facade, walk field type and constructor read from the prelude's own `rel_itty`/`rel_mat`/`rel_marg` so the pushdown-chosen producer travels) and it **REDS ON A THIRD, DIFFERENT DEFECT**: this file can only deliver a STRUCT through `quote_item!` + `logos_emit_item_blob_subst`, and on that channel a method whose CALLEE RETURNS A BORROW-CARRYING VALUE loses its field-split borrow (task **#74**, D8; ⚠ the round's own control aimed at the wrong side — the verify re-aimed it: the struct's `#[borrow_carrying]` is IRRELEVANT, the quote half reds without it and a quote struct with no borrow-carrying callee is green) — `self.w.next_batch()` loans the WHOLE `self`, so all three `self.sc` uses red with "cannot use 'self' while it is mutably borrowed". **BISECTED TO THE CHANNEL, NOT INFERRED**: the emitted and hand-written bodies were diffed to exactly two deltas (a doc comment, one pair of parentheses round `self.w`), both were removed from the emitter, and the red was unchanged; the identical text on the module-source channel (`Emitter`/`logos_emit_source`) is green IN THE SAME COMPILATION. Minimal repro `sandbox/s5land/REPRO_qroute.logos` — 115 lines, no deem, no container, no Memoria, `HandDx` green and `QuoteDx` red 3×. **ELIGIBILITY RE-DERIVED (not trusted from R-E), swept at `fb8dc2b7` over the 189-fixture corpus / 172 user dumps**: of 503 emitted `_stream` doors, **118** have the plain scan shape direct needs; by source class **107 SliceStream slice params, 4 container walks, 7 other**. Only the container-walk class would be MULTI-PACKET; a `SliceStream` is one packet by construction, so the 107 would republish a seam type and remove no pull. The three buffered ground sentences are rewritten to name THIS blocker with that count; #70 is retired from them. **2026-08-19: THE QUOTE-CHANNEL PREMISE IS REFUTED AND `direct` IS LANDED.** The D8 diagnosis ran the bidirectional control the 08-18 bisection lacked: the SAME text reds on the module-source channel when emitted MULTI-LINE and greens on the quote channel when emitted as ONE line — the discriminator was never the channel, it was physical LINE LAYOUT, because loan liveness is keyed on the source line (`release_dead_borrows`, `lu <= cur_line`) and a single-line module releases every loan before the conflicting use. The 08-18 "green oracle" was un-borrow-checked code (the oracle-silence shape). TWO real defects replaced the premise: (A) the OVER-refusal — `take_ref_borrows`' AddrOfTemp MethodCall branch deposited `take_borrow(bp.root, …)` with `bp.path` DISCARDED, so a field-place receiver × `&mut self` × self-borrowing callee locked the whole root; FIXED by mirroring the two sibling arms' path split (`take_field_borrow` on non-empty path), pair `pass/bc_d8_quote_field_split_admit` + `fail/bc_d8_quote_whole_self_conflict_fail`, path-precise across 7 verifier spellings; (B) the PERMISSIVE line-keyed-liveness hole — **OPEN, #75** (5 stdlib single-line emitters currently exempt from all exclusivity checking; refusing-direction fix, needs its own measured red list). With (A) fixed the reconstructed emitter compiled first build: **`direct` is landed** — 10 doors in the PINNED `wql_*`/`deem_*` population (per plan facts: native + `AD_NONE` + batch, forward arm), each cross-pinned 1:1:1:1:1 (struct : inherent `next_batch` : forward : `Result` facade : `BatchStream` impl); the corpus outside the glob holds ≥22 more doors, UNPINNED — **#76**. The emitted shape matches `EMITTED_SHAPE.gen.logos` byte-for-byte EXCEPT the struct NAME, which gained the walk-type suffix (`ScanAllDxHs…LeafWalk`) because the state type is per (query, SOURCE FAMILY) — two families red as duplicate-struct without it. SliceStream (107 at `fb8dc2b7`; 82 first-reason plan lines on this tree — different quantities, both recorded) REFUSED in ground text: one packet by construction | **LANDED 2026-08-19** — doors pinned; #75 (liveness) and #76 (non-glob doors) are the survivors |
...
⚠ **THE `next_batch` ROW MOVED AT S5-direct (2026-08-19) AND THE ROW'S SHAPE
MOVED WITH IT.** Until §12's `direct` door landed, every `.next_batch()` in the
corpus was a PULL. The direct door emits a third form — `impl BatchStream<…> for
<Q>Dx { fn next(&mut self) { return self.next_batch(); } }` — which is a
DISPATCH to the same object, moves no data, and has no `__bj` loop. Counting it
as a pull would have let the stage inflate criterion 2's own numerator by exactly
the number of doors it landed. So the gate now carries TWO POSITIVE RULES and a
ZERO-PINNED RESIDUAL, and the table row below is the PULL half. Derivation
1018 → 1030: **+1** D7 #62's fixture, **+1** `deem_direct_stream_pull`'s `_run`
body, **+10** one `(self.w).next_batch()` per direct door.

| pinned | value | second, independent count of the same act |
|---|---|---|
| `next_batch()` PULLS | **1030** | `while (__bj < __bn)` headers 1030 · `__bj` zero-seed declarations 1030 |
| `BatchStream` FORWARDS (`return self.next_batch();`) | **10** | == the direct-door count, cross-pinned below |
| `.next_batch()` residual | **0** | pinned at zero: a third consumer spelling must be classified, not absorbed |
| §12 `direct` doors | **10** | 1:1:1:1:1 — `<Q>Dx` structs 10 · inherent `next_batch` 10 · forwards 10 · `Result<…Dx, ElError>` facades 10 · `impl BatchStream … for …Dx` 10 |
...
* **`direct`** — ~~blocked on **#62**~~ ~~then on **#70**~~ ~~then on the
  "quote-item splice channel"~~ **LANDED 2026-08-19.** The channel premise was
  REFUTED by the bidirectional control (line layout, not channel — see the §12
  row); the real over-refusal (#74/D8, the AddrOfTemp path discard) is FIXED,
  and the permissive line-keyed-liveness hole it unmasked is **#75**. 10 doors
  in the pinned population; ≥22 non-glob doors unpinned (**#76**). The emitter itself was
  written and is reverted rather than left half-landed, and the eligible class
  is re-derived and recorded: 118 plain-shape `_stream` doors of 503, of which
  only 4 (container walks) would be multi-packet.
...
that — is blocked on **#62** (2026-08-18: #62 FIXED, then #70 FIXED;
2026-08-19: the "#74 channel" premise REFUTED, the real D8 over-refusal FIXED,
and **`direct` IS LANDED** — 10 doors in the pinned population no longer fill
to completion; the buffered remainder is refused per class with measured
counts, and the un-pinned non-glob doors are #76. The verdict sentence above
is HISTORY as of the arc-close tree; the §12 row carries the current state). Everything else on the plane is either integrated
...
| **output plane** | **606 + 45 + 1** | **`direct` LANDED 08-19** for the container-walk class (10 pinned doors; the 08-18 revert's "channel defect" premise was refuted — line-keyed liveness, #75); buffered remainder refused per class with counts (08-18: 606→607 with the #62 fixture; on the landed tree the pinned query heads read 480+16+5 = 501 = 10 direct + 491 refused) |
...
|---|---|---|
| L1 | ~~**#62 #70 #74 → `direct`**~~ **DONE 2026-08-19** | #62 DONE `d90a89a4` (round-order, not layout — see §12); #70 DONE `fb8dc2b7`; #74/D8 DONE the same round and its filed premise REFUTED (not a quote-channel defect at all — `BorrowChecker::take_ref_borrows`' `AddrOfTemp` MethodCall arm discarded `bp.path`; the module-source half only looked green because loan liveness is keyed on source LINE and it was emitted as one line — that permissive hole is SEPARATE and still open). "Emitter plumbing" was FALSE twice over; recorded so the phrase is not written a third time. **LANDED:** `rexpr_walk::emit_stream_direct` + the `dx_on` decision site in `emit_simple`; the emitted shape matches the hand-proven `sandbox/s5land/EMITTED_SHAPE.gen.logos` byte-for-byte EXCEPT the struct NAME (6 lines: `ScanAllDx` → `ScanAllDxHs…LeafWalk`, the per-(query, source-family) suffix — two families red as duplicate-struct without it); pushdown travels: `__ctr_bfrom_…(m, 30u64)`. **10 doors in the PINNED `wql_*`/`deem_*` population, every one the CONTAINER-WALK class** (the corpus outside the glob holds ≥22 more, UNPINNED — #76) (the plan's `rel_native && rel_node == AD_NONE && rel_batch` forward arm — decided from plan facts, not names); the other classes are REFUSED IN THE GROUND WITH THEIR MEASURED COUNTS — 152 aggregate, 113 join, 82 `deem` slice param (ONE packet by construction: the 107-of-118 class republishes a seam type and removes no pull), 62 surviving `order by`, 51 non-batch source, 18 borrowed element, 7 `limit`, 4 `distinct`, 1 prelude-reading `select`, 1 generic; 1 clause (`where` reading a prelude binding) is UNWITNESSED and said so. **Consumers updated:** `deem_pipeline_handle_seam` seam 1 (one-packet pin → packets concatenated, equivalent-or-stronger on all four axes, plus a NEW seam 1b over a 2000-row container where `pk >= 2` is a fact); `pull_shape_gate` (`next_batch` splits into a two-rule positive partition with a zero-pinned residual + a 1:1:1:1:1 cross-pin per door, loops now correspond to PULLS); `plan_ground_census_gate` (FACT J′ — the door partition over the same 501 query-landing lines, with the `<Q>Dx` artifact cross-pin per fixture); `ctr_access_path_gate` (the filter-retirement pin re-derived from a COUNT into a RULE: every occurrence of the bound is inside an access call, and the access-call count is the door count). **The `_run` LANDING DID NOT MOVE and that is the claim, not an oversight** — S5's rule holds, `direct` replaces the `_stream` DOOR and leaves the 608 `Buffer` landings exactly where they were; what stops being filled to completion is what the STREAMING consumer pulls. Answer oracle: 189 matched fixtures, **0 moved answer rows**, 1 added. **THE ROUND'S OWN VERIFY THEN FOUND TWO MATERIAL PERMISSIVE HOLES AND BOTH ARE CLOSED HERE.** (i) `dx_on` admitted queries whose door DOES NOT COMPILE: any arithmetic in `select`/`where` lowers through the checked tower (`el_add(..)?`), and a direct door's `next_batch` returns `Option<&[E]>` — the `BatchStream` pull protocol has no error channel, so widening it is an ADR 0025 §3 change to every batch source and the honest move is to REFUSE. New cascade clause + ground, pinned by `pass/deem_direct_fallible_buffered` (the one-token mate of the direct fixture: it compiles — that IS the refusal — and its plan line carries the new ground). ⚠ Its first cut scanned for a bare `?` BYTE and so printed the CHECKED-ARITHMETIC ground over `starts_with("a?b", …)`, a query with no arithmetic in it — a FALSE ground, which this file's own rule calls worse than a missing count; the scan now matches the tower's `)?` OUTSIDE string literals, measured on a one-byte twin pair. (ii) The BC fix's own class had a SECOND site: a MATCH SCRUTINEE is a whole-value read, `take_borrow`'s B83 arm refuses those against a live mut FIELD loan, but the scrutinee never reaches `take_borrow` — pair `fail/bc_d8_match_scrutinee_field_loan_fail` + `pass/bc_d8_match_scrutinee_disjoint_admit` (3 legs: disjoint field, loan dead under NLL, CALL scrutinee), both bite-proven. Refusal census now asserted IN CODE per clause (`EXPECT_DXWHY`, 13 first-reason counts + sum), and `pull_shape`'s `dx_forward` stopped being a COPY of `nb_forward` — it reads the trait method header off the artifact, with their correspondence as its own clause |
