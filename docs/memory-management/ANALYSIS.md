...
| **array `[T;N]`** | stack | per-elem drop | move iff elem move; `let s = arr[i]` for droppable T rejected (anti-double-free); diagnostic is more specific than Rust's ("Drop-bearing element …" vs "cannot move out of index"); non-droppable non-Copy structs slip through via auto-Copy ([[ref_divergences_register]] — global policy, not array-specific) | ✅ element borrow tracked (`b1ba5dd3`): `&[mut] arr[i]` registers a whole-array borrow (Rust-conformant — `Index`/`IndexMut` take `&[mut] self`); `arr[i] = v` checks container borrow state | ✅ |
