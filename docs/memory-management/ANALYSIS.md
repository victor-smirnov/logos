...
| **assignment `x=y`** | — | ✅ drop-before-replace (`b9cf9d81`); declared-uninit vars use Rust-style DYNAMIC DROP FLAGS (`33d12014`): exact drop on reassign + scope exit incl. conditional/early-return paths, no garbage-drop UB | source moved (suppress double-free) | assign-while-borrowed ✅ | ✅ full Rust drop semantics (conditional-init divergence CLOSED) |
