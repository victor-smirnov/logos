...
**Variable case fixed.** `mlir_gen_expr` now emits `llvm.intr.{s,u}{add,sub,mul}.with.overflow` + `cond_br` on the overflow bit + `llvm.intr.trap` for every signed/unsigned integer `+`/`-`/`*`. SIGILL on runtime overflow. Float ops, bitwise, comparison, division, and remainder are unchanged. Intentional two's-complement wrap (FNV/SHA/MD5/splitmix64/FxHasher) is opt-in via the `wrapping_add` / `wrapping_sub` / `wrapping_mul` intrinsic family in `std.lang.arith` — declared as generic `pub fn wrapping_op<T>(a: T, b: T) -> T { return a; }` placeholders that mlir-gen intercepts by name (matching the bare name *and* the post-mono `__g__<sig>` form) and emits the silent `arith.{add,sub,mul}i`. Stdlib hot spots (FNV in `std.writ.string` / `std.writ.hashing`, FxHasher in `std.lang.hash`, splitmix64 in `std.math.random`, SHA-256 + MD5 in `std.crypto`) migrated to `wrapping_*`. Lock-in: pass test `wrapping_arith` (covers `wrapping_add` wrap to INT_MIN, `wrapping_mul` u32 wrap, `wrapping_sub` u8 wrap-to-255). Lock-in for the literal-fold case stays: fail tests `lit_arith_i32_overflow` and `lit_arith_i64_overflow`.
**Repro**: `B01/` —
```logos
fn main() -> i32 {
