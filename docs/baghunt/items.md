...
```
**Observed**: Compiles cleanly.
**Expected**: `eval_static_writ_lit` should reject capture nodes per the documented contract ("no captures `${...}`, just static values" — see [attributes.md](../spec/items.md#meta-blocks-meta)). At least error: "meta block requires static values; ${...} captures not allowed".
