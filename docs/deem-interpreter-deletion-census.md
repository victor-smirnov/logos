...
# 2026-09-03k — `is_unowned_move_source`'s FieldRead/TupleIndex arms walk the
#   receiver CHAIN: 2 admit rows -> imported fail (borrowck, nll), +2 native
#   fail + +2 native pass bc_mvchain_* twins. ALL +4 (+4 native; the 2 rows
#   move admit->imported fail, which is +0 to ALL); NOIMPORTED +2 (+4 native,
#   -2 admit tests, which are not `imported`-labelled and their fail ports
#   are); TIERCOMMIT -2 (the two admit tests). Predicted before the pin was
#   read; all three landed on the number.
#   ALL          8977 -> 8981   +4
#   NOIMPORTED   4555 -> 4557   +2
#   TIERCOMMIT    187 ->  185   -2
# 2026-09-03n (the call-result loan round): +6 native fixtures (three fail
#   pairs' halves under tests/logos/fail, three pass halves under
#   tests/logos/pass) and -3 admit tests (three ledger rows closed and moved
#   admit->imported fail, which is +0 to ALL). NOIMPORTED +3 (+6 native, -3
#   admit tests, which are not `imported`-labelled and their fail ports are);
#   TIERCOMMIT -3 (the three admit tests). Predicted before the pin was read;
#   all three landed on the number.
#   ALL          8981 -> 8987   +6
#   NOIMPORTED   4557 -> 4560   +3
#   TIERCOMMIT    185 ->  182   -3
# 2026-09-03p (the `move`-capture deposit): +4 native fixtures (two fail halves
#   under tests/logos/fail, two pass halves under tests/logos/pass) and -1 admit
#   test (one ledger row closed and moved admit->imported fail, +0 to ALL).
#   NOIMPORTED +3 (+4 native, -1 admit test, which is not `imported`-labelled
#   while its fail port is); TIERCOMMIT -1 (the one admit test). Predicted from
#   the shape of the 09-03n entry BEFORE the pin was read; all three landed.
#   ALL          8987 -> 8991   +4
#   NOIMPORTED   4560 -> 4563   +3
#   TIERCOMMIT    182 ->  181   -1
# 2026-09-04a (the written `self:` type of an inherent impl method): +5 native
#   fixtures (2 fail, 3 pass under tests/logos) and -2 admit tests (two ledger
#   rows closed and moved admit->imported fail, +0 to ALL). NOIMPORTED +3 (+5
#   native, -2 admit tests, which are not `imported`-labelled while their fail
#   ports are); TIERCOMMIT -2 (the two admit tests). Predicted from the shape of
#   the 09-03p entry BEFORE the pin was read; all three landed on the number.
#   ALL          8991 -> 8996   +5
...
# 2026-09-09 (the struct-literal region mint). SIX new native fixtures, three
#   fail/pass PAIRS one token apart; and THREE imported `admit` rows closed, so
#   their fixtures moved from tests/imported/admit/ to tests/imported/fail/.
#   The moved three are `imported`-labelled at their new home and were NOT
#   before, so NOIMPORTED takes +6-3 = +3; they WERE tier_commit as admit gates
#   and the fail tier is not, so TIERCOMMIT takes -3.
#   ALL          9005 -> 9011   +6
#   NOIMPORTED   4572 -> 4575   +3
#   TIERCOMMIT    176 ->  173   -3
# 2026-09-04land (the three runtime soundness holes). EIGHTEEN new native
#   fixtures — 17 pass + 1 fail — none of them `imported`, none of them
#   tier_commit, so ALL and NOIMPORTED take +18 and TIERCOMMIT is unmoved.
#   Every one asserts a RUN, and the nine that pin drop glue assert a destructor
#   COUNT on stdout, because an exit code cannot tell one drop from two.
#   ALL          9011 -> 9029   +18
...
# 2026-09-05a (soundness queue: 4 rows closed, 20 -> 17 with one added). The
#   four programs leave tests/soundness/open/ (unglobbed, so no test leaves)
#   and land as native fixtures with their twins: +12 pass + +7 fail, all
#   `bc_recvaot_*` / `bc_patmut_*`, none imported, none tier_commit.
#   Predicted before the pin was read.
#   ALL          9040 -> 9059   +19
#   NOIMPORTED   4604 -> 4623   +19
#   TIERCOMMIT    174 ->  174     0
# 2026-09-06b (soundness queue: 2 rows closed, 4 added, 21 -> 23). The two
#   programs leave tests/soundness/open/ (unglobbed) and land with their twins
#   and the round's counter-examples: +20 pass + +16 fail, all `bc_patmut_*`,
#   none imported, none tier_commit. Predicted from the file count before the
#   pin was read (the gate then said +36).
#   ALL          9059 -> 9095   +36
...
# 2026-09-09b (soundness queue: 5 rows closed, 5 added, 28 -> 28). The five
#   programs leave tests/soundness/open/ (unglobbed) and land with their twins
#   and the round's counter-examples: +51 pass + +24 fail, all `bc_bindmut_*`,
#   none imported, none tier_commit. Predicted from the file count before the
#   pin was read (`ls tests/logos/pass/bc_bindmut_*.logos | wc -l` -> 51,
#   `ls tests/logos/fail/bc_bindmut_*.logos | wc -l` -> 24).
#   ALL          9216 -> 9291   +75
...
# 2026-09-08 — `lifereg.L7` LANDS: TWO `logos_00_bc_admit_*` tests leave the registry
#   (both rows deleted, 94 -> 92), and the two programs land as FOUR imported fixtures
#   — 2 fail halves with the diagnostic pinned in full, 2 one-token pass halves that RUN.
...
#   2026-09-09 (bck.NEW-BCS: E0716 at every door that records a temporary
#   provenance. One bc_admits row closed; three native pairs pin the three
#   doors.)
#   ALL          9495 -> 9501  +6   (+3 native pass, +3 native fail, and the
#                                    moved row: -1 admit gate, +1 imported fail)
...
# 2026-09-14i (harvest 2 — the hand batteries of rounds 2026-09-14e-thruref and 2026-09-14f-thrurefland, second snapshot
#                /home/logos/sandbox/hand-harvest-2026-09-14b, manifest src/compiler/probes/2026-09-14i-harvest2/MANIFEST.tsv).
#                33 native pass fixtures bc_0914{e_thruref,f_thrurefland}_hb_<id>_admit and 41 native fail fixtures
#                bc_0914{e_thruref,f_thrurefland}_hb_<id>_refuse, no pairs, no moves — +74, all native, none tier_commit.
...
# 2026-09-15c (argref PRICING — `&<reference binding>` one level short; no compiler change lands, soundness_queue 200 -> 211).
...
# 2026-09-15e (the soundness queue's rows become ctest tests — one `logos_00_squeue_<id>` per program under
#                tests/soundness/open/, `soundness_queue_gate.sh --one <id>`; `logos_00_soundness_queue` keeps only the roster,
#                `--roster`). No fixture added or moved. 209 rows = 209 new tests, each labelled tier_commit like the bc ledger's
#                logos_00_bc_admit_*. So ALL 10600 -> 10809 (+209), NOIMPORTED 6098 -> 6307 (+209), TIERCOMMIT 118 -> 327 (+209)
#                — PREDICTED 10809 / 6307 / 327 before reconfiguring, then measured by `ctest -N` on the reconfigured build.
#                ⚠ From now on every soundness_queue row opened or closed moves all three numbers by one, as a bc_admits row does.
# 2026-09-15f (consume PRICING — a by-value operand placed into a consuming node is not marked moved; no compiler change lands,
#                soundness_queue 209 -> 219). 10 queue rows opened (10 new logos_00_squeue_<id> tests, each tier_commit) and 4 native fail
#                fixtures (bc_0915e_consume_hb_x0{1,2,3,4}_refuse, the sentence prediction the pricing refuted), no pass fixture, no port
#                moved. So ALL 10809 -> 10823 (+14), NOIMPORTED 6307 -> 6321 (+14), TIERCOMMIT 327 -> 337 (+10) — PREDICTED before the
#                re-glob, then measured by `ctest -N` on the rebuilt, re-globbed build 7eb6735b16576c45 43 before this pin was touched.
# 2026-09-15g (consumeland LANDING — the operator half of consumex: a by-value operator / *Assign operand is marked moved;
#                soundness_queue 219 -> 221). 1 queue row closed (byvalue_operator_operand_not_moved_double_drop, its program moved to
#                pass/bc_0915f_consumeland_operator_operands_moved_admit) and 3 opened (aggregate_literal_temp_place_base_never_dropped_run,
#                match_expr_array_pattern_segfaults_run, operator_trait_rhs_type_param_refused): tier_commit +3 -1. Native fixtures: 44 pass
#                (the closed row's program + 43 hand-battery programs bc_0915f_consumeland_hb_*_admit) and 3 fail (the closed row's twin
...
# 2026-09-15i (aggtemp PRICING — the temporary-owner door for a place-base receiver and the array rows in series behind it; no compiler
#                change, soundness_queue 221 -> 231). 10 queue rows OPENED and none closed (extended_field_base_temp_dropped_at_let_end_run,
#                array_len_builtin_discards_receiver_run, foreach_array_rvalue_elements_never_dropped_run,
#                fnptr_field_call_on_temporary_never_dropped_run, fru_temporary_base_unmoved_field_never_dropped_run,
#                array_repeat_rvalue_noncopy_len3_admits, call_result_index_projection_invalid_mlir_refused,
#                nested_array_literal_index_initializer_lost_refused, ifexpr_arm_extended_field_borrow_refused,
#                array_temp_elem_field_store_refused): each is its own ctest test, all tier_commit, so +10 / +10 / +10. Native fixtures:
#                3 pass (bc_0915g_aggtemp_hb_b0{1,2,3}_admit, the hand programs that REFUTED the arms), none tier_commit. So
#                ALL 10878 -> 10891 (+13), NOIMPORTED 6376 -> 6389 (+13), TIERCOMMIT 339 -> 349 (+10) — PREDICTED before the re-glob,
#                then measured by `ctest -N` on the re-globbed build before this pin was touched.
# 2026-09-15j (aggtemp LANDING — a fresh move-type rvalue in a place-base position owns its temporary, an extending borrow's
#                place chain owns it to the end of the BLOCK, and an array / repeat literal CONSUMES its operands except as a
#                for-each iterable; soundness_queue 231 -> 220). 11 queue rows CLOSED and none opened, so 11 logos_00_squeue_<id>
#                tests (all tier_commit) go away; their programs land as 9 pass and 2 fail native fixtures, plus 12 pass hand-battery
...
# 2026-09-16n (arrstore2 LANDING — the array place write's two guards are doors in SERIES; both ask
#                store_loans_die_in_stmt_ now, the second scoped to the store's own LHS; soundness_queue 234 -> 233).
#                TWO queue rows CLOSED (array_store_value_reads_loan_refused, array_index_store_index_reads_loan_refused)
#                and ONE OPENED (array_field_store_value_reads_field_loan_refused, the struct-FIELD spelling, rowed with
#                its reason), so logos_00_squeue_* goes 234 -> 233, all tier_commit: -1. Native fixtures: 7 pass (the 2
#                closed rows' programs + 5 hand-battery programs whose verdict MOVED base(refused) -> landed(compiles and
#                RUNS exit 0)) and 2 fail (the abuse-direction twins x02/x04, rustc E0506), none tier_commit. So
...
#                ALL 11063 -> 11072 (+9), NOIMPORTED 6561 -> 6570 (+9), TIERCOMMIT 353 -> 354 (+1) — 2026-09-17p-ordcmpland
#                lands EIGHT fixtures (5 pass, 3 fail), DELETES one soundness_queue row (tier 3) and OPENS one (tier 1), so the
#                closed row's logos_00_squeue_<id> test cancels against ONE of the two opened, so the net of the three row edits is +1 in all three registries and the eight fixtures move ALL / NOIMPORTED only. The
...
#                ALL 11076 -> 11077 (+1), NOIMPORTED 6574 -> 6575 (+1), TIERCOMMIT 354 -> 355 (+1) — 2026-09-17-ledger-sync adds a gate
#                ONE test, `logos_00_ledger_sync`, labelled "logos;lint;suite_lint;tier_commit". It is NOT imported and it IS in
...
#                ALL 11082 -> 11084 (+2), NOIMPORTED 6580 -> 6582 (+2), TIERCOMMIT 355 -> 355 (0) — 2026-09-18-array-order lands
#                TWO FIXTURES ADDED for #377: fail/array_relational_refused (sema now refuses `<` on an aggregate the
#                lowering cannot order — it used to reach mlir-gen and crash the MLIR verifier with an `!llvm.ptr`
#                operand on `arith.cmpi`) and pass/array_relational_admitted (its twin: the seven shapes that MUST keep
...
#                fail/struct_ref_pair_eq_without_impl_refused, which registers in the DIAGNOSTICS tier — so the two outer
#                columns cancel to zero and only the tier_commit column moves. RE-DERIVED FROM THE GATE'S OWN PRINTED
...
#                ALL 11100 -> 11101 (+1), NOIMPORTED 6596 -> 6597 (+1), TIERCOMMIT 347 -> 348 (+1) — 2026-09-20 ADR 0028
#                closure captures as an aggregate. ONE tier_commit squeue gate added, for #442
#                generic_closure_id_collides_across_instances: a closure inside a generic function instantiated at two
#                types does not compile (`redefinition of symbol named '__closure_0'`), because mono copies the closure
...
#                ALL 11107 -> 11107 (0), NOIMPORTED 6602 -> 6602 (0), TIERCOMMIT 346 -> 347 (+1) — 2026-09-21,
#                ADR 0028 blocker triage, second batch: ONE pass fixture LEAVES and ONE squeue program ARRIVES, so
#                the outer columns cancel and only tier_commit moves (#449
#                field_retarget_overlapping_mutborrow_admits, was pass/bc_d1r9_f0_retarget_overlap_admit). The
#                fixture pinned a stated RULE that Rust does not have — that retargeting a `&mut` FIELD converges
...
#                ALL 11120 -> 11121 (+1), NOIMPORTED 6565 -> 6564 (-1), TIERCOMMIT 239 -> 235 (-4) — 2026-09-23:
#                trait default templates: BC rows elided-self-lifetime-in-trait-fn (#43) and
#                trait-method-lifetime-suggestion (#46) moved admit -> imported/fail/lifetimes; squeue #101/#102
#                -> fail/trait_default_body_{type,call}_fail; new pass/trait_default_body_template_admit.
#                ALL 11121 (0), NOIMPORTED 6564 -> 6563 (-1), TIERCOMMIT 235 -> 234 (-1) — 2026-09-23: BC row
#                regions-normalize-in-where-clause-list (#77) moved admit -> imported/fail/regions.
#                ALL 11121 -> 11122 (+1), NOIMPORTED 6563 (0), TIERCOMMIT 234 -> 233 (-1) — 2026-09-23: BC row
#                trait-method-return-lifetime-mismatch (#47) admit -> imported/fail; new pass/struct_bound_lifetime_in_signature_admit.
#                ALL 11122 (0), NOIMPORTED 6563 -> 6562 (-1), TIERCOMMIT 233 -> 232 (-1) — 2026-09-23: BC row
#                propagate-fail-to-approximate-longer-no-bounds (#67) admit -> imported/fail/nll.
#                ALL 11122 -> 11123 (+1), NOIMPORTED 6562 (0), TIERCOMMIT 232 -> 231 (-1) — 2026-09-23: BC row
#                dropck-only-error (#40) admit -> imported/fail/dropck; new pass/assoc_bound_eq_self_supertrait_admit.
#                ALL 11123 -> 11124 (+1), NOIMPORTED 6562 (0), TIERCOMMIT 231 -> 230 (-1) — 2026-09-23: BC row
#                issue-74400 (#44) admit -> imported/fail/lifetimes; new pass/fn_bound_hr_param_outer_ret_admit.
#                ALL 11124 -> 11125 (+1), NOIMPORTED 6562 -> 6563 (+1), TIERCOMMIT 230 (0) — 2026-09-23, #461:
#                fail/elided_result_body_source_fail.
#                ALL 11125 (0), NOIMPORTED 6563 -> 6562 (-1), TIERCOMMIT 230 -> 229 (-1) — 2026-09-23: the LAST
#                BC row, anonymous-region-in-apit--closure-param-escapes (#21), admit -> imported/fail/borrowck.
#                ALL 11125 (0), NOIMPORTED 6562 (0), TIERCOMMIT 229 -> 224 (-5) — 2026-09-23: FIVE squeue rows closed
#                (ptr_coercion_*, #212-#216), each landed as a fail fixture that replaces its squeue test.
#                ALL 11125 (0), NOIMPORTED 6562 (0), TIERCOMMIT 224 -> 221 (-3) — 2026-09-23: three squeue rows closed
#                (static_mut_*, #197/#198/#200), landed as fail fixtures.
#                ALL 11125 (0), NOIMPORTED 6562 (0), TIERCOMMIT 221 -> 217 (-4) — 2026-09-23: squeue #240, #245-#247 closed,
#                landed as fail fixtures.
#                ALL 11125 (0), NOIMPORTED 6562 (0), TIERCOMMIT 217 -> 215 (-2) — 2026-09-23: squeue #196, #307 closed.
#                ALL 11125 (0), NOIMPORTED 6562 (0), TIERCOMMIT 215 -> 213 (-2) — 2026-09-23: squeue #184, #199 closed.
#                ALL 11125 -> 11128 (+3), NOIMPORTED 6562 -> 6565 (+3), TIERCOMMIT 213 -> 207 (-6) — 2026-09-23: squeue
#                #125, #132, #148, #176, #267, #305 closed (5 pass + 1 fail fixtures), 3 new pass fixtures.
#                ALL 11128 -> 11129 (+1), NOIMPORTED 6565 -> 6566 (+1), TIERCOMMIT 207 -> 206 (-1) — 2026-09-23: squeue
#                #186 closed (2 fail fixtures).
#                ALL 11129 -> 11130 (+1), NOIMPORTED 6566 -> 6567 (+1), TIERCOMMIT 206 -> 205 (-1) — 2026-09-23: squeue
#                #111 closed (2 pass fixtures).
#                ALL 11130 -> 11131 (+1), NOIMPORTED 6567 -> 6568 (+1), TIERCOMMIT 205 -> 204 (-1) — 2026-09-23: squeue
#                letelse_or_pattern_takes_else closed (2 pass fixtures).
#                ALL 11131 -> 11132 (+1), NOIMPORTED 6568 -> 6569 (+1), TIERCOMMIT 204 -> 202 (-2) — 2026-09-23: squeue
#                #313, #309 closed (3 pass fixtures).
#                ALL 11132 -> 11133 (+1), NOIMPORTED 6569 -> 6570 (+1), TIERCOMMIT 202 -> 200 (-2) — 2026-09-23: squeue
#                #265, #266 closed (3 pass fixtures).
#                ALL 11133 -> 11135 (+2), NOIMPORTED 6570 -> 6572 (+2), TIERCOMMIT 200 -> 198 (-2) — 2026-09-23: squeue
#                #290, #302 closed (4 pass fixtures).
#                ALL 11135 -> 11136 (+1), NOIMPORTED 6572 -> 6573 (+1), TIERCOMMIT 198 -> 197 (-1) — 2026-09-23: squeue
#                index_place_through_refmut_never_drops_old closed (2 pass fixtures).
#                ALL 11136 -> 11138 (+2), NOIMPORTED 6573 -> 6575 (+2), TIERCOMMIT 197 -> 196 (-1) — 2026-09-23: squeue
#                #293 closed (3 pass fixtures).
#                ALL 11138 -> 11139 (+1), NOIMPORTED 6575 -> 6576 (+1), TIERCOMMIT 196 -> 195 (-1) — 2026-09-23: squeue
#                #285 closed (2 pass fixtures).
#                ALL 11139 -> 11141 (+2), NOIMPORTED 6576 -> 6578 (+2), TIERCOMMIT 195 -> 193 (-2) — 2026-09-23: squeue
#                #294, #295 closed (4 pass fixtures).
#                ALL 11141 -> 11142 (+1), NOIMPORTED 6578 -> 6579 (+1), TIERCOMMIT 193 -> 192 (-1) — 2026-09-23: squeue
#                static_array_whole_borrow_reads_garbage closed (2 pass fixtures).
#                ALL 11142 -> 11144 (+2), NOIMPORTED 6579 -> 6581 (+2), TIERCOMMIT 192 (0) — 2026-09-23: squeue #86 closed
#                (2 pass fixtures), #470 opened (its neighbour, the `while` shape).
#                ALL 11144 -> 11145 (+1), NOIMPORTED 6581 -> 6582 (+1), TIERCOMMIT 192 -> 191 (-1) — 2026-09-23: squeue
#                #470 closed (2 pass fixtures).
#                ALL 11145 -> 11148 (+3), NOIMPORTED 6582 -> 6585 (+3), TIERCOMMIT 191 -> 190 (-1) — 2026-09-23: squeue
#                tuple_elem_shared_borrow_through_ref_dangles_run closed (4 pass fixtures).
#                ALL 11148 -> 11149 (+1), NOIMPORTED 6585 -> 6586 (+1), TIERCOMMIT 190 -> 189 (-1) — 2026-09-23: squeue
#                param_ref_struct_cast_after_addr_of_reads_slot_run closed (2 pass fixtures).
#                ALL 11149 -> 11151 (+2), NOIMPORTED 6586 -> 6588 (+2), TIERCOMMIT 189 (0) — 2026-09-23: 2 pass fixtures
#                (if-condition terminating scope, field-base temporaries).
#                ALL 11151 -> 11154 (+3), NOIMPORTED 6588 -> 6591 (+3), TIERCOMMIT 189 -> 188 (-1) — 2026-09-24: squeue
#                #144 closed (2 pass + 2 fail fixtures).
#                ALL 11154 -> 11157 (+3), NOIMPORTED 6591 -> 6594 (+3), TIERCOMMIT 188 -> 186 (-2) — 2026-09-24: squeue
#                #135, #115 closed (5 pass fixtures).
#                ALL 11157 -> 11159 (+2), NOIMPORTED 6594 -> 6596 (+2), TIERCOMMIT 186 -> 184 (-2) — 2026-09-24: squeue
#                #92, #93 closed (4 pass fixtures).
#                ALL 11159 -> 11161 (+2), NOIMPORTED 6596 -> 6598 (+2), TIERCOMMIT 184 -> 183 (-1) — 2026-09-24: squeue
#                #284 closed (2 pass + 1 fail fixtures).
#                ALL 11161 -> 11164 (+3), NOIMPORTED 6598 -> 6601 (+3), TIERCOMMIT 183 -> 182 (-1) — 2026-09-24: squeue
#                #141 closed (4 pass fixtures, the row among them).
#                ALL 11164 -> 11167 (+3), NOIMPORTED 6601 -> 6604 (+3), TIERCOMMIT 182 -> 181 (-1) — 2026-09-24: squeue
...
#                TIERCOMMIT 171 -> 169 (-2) — 2026-09-24: squeue #147 #308 closed (2 rows -> fail fixtures).
#                ALL 11180 -> 11183 (+3), NOIMPORTED 6617 -> 6620 (+3), TIERCOMMIT 169 -> 167 (-2) — 2026-09-24: squeue
#                #129 #79 closed (5 pass fixtures, the 2 rows among them).
#                ALL 11183 -> 11184 (+1), NOIMPORTED 6620 -> 6621 (+1), TIERCOMMIT 167 -> 164 (-3) — 2026-09-24: squeue
#                #84 #80 #123 closed (4 pass fixtures, the 3 rows among them).
#                TIERCOMMIT 164 -> 163 (-1) — 2026-09-24: squeue match_array_nested_array_subpattern_mlirgen_malfunction closed (moved to pass/).
#                ALL 11184 -> 11186 (+2), NOIMPORTED 6621 -> 6623 (+2), TIERCOMMIT 163 -> 160 (-3) — 2026-09-24: squeue
#                #299 #81 #94 closed (5 pass fixtures, the 3 rows among them).
#                ALL 11186 -> 11187 (+1), NOIMPORTED 6623 -> 6624 (+1), TIERCOMMIT 160 -> 159 (-1) — 2026-09-24: squeue
#                #114 closed (2 pass fixtures, the row among them).
#                ALL 11187 -> 11188 (+1), NOIMPORTED 6624 -> 6625 (+1), TIERCOMMIT 159 -> 157 (-2) — 2026-09-24: squeue
#                #116 #131 closed (3 pass fixtures, the 2 rows among them).
#                ALL 11188 -> 11189 (+1), NOIMPORTED 6625 -> 6626 (+1) — 2026-09-24: pass/tuple_in_tuple_place_assign.
#                ALL 11189 -> 11192 (+3), NOIMPORTED 6626 -> 6629 (+3), TIERCOMMIT 157 -> 155 (-2) — 2026-09-24: squeue
#                #85 #96 closed (4 pass + 1 fail fixtures, the 2 rows among them).
#                ALL 11192 -> 11193 (+1), NOIMPORTED 6629 -> 6630 (+1), TIERCOMMIT 155 -> 153 (-2) — 2026-09-24: squeue
#                #280 #282 closed (3 pass fixtures, the 2 rows among them).
#                ALL 11193 -> 11195 (+2), NOIMPORTED 6630 -> 6632 (+2), TIERCOMMIT 153 -> 150 (-3) — 2026-09-24: squeue
#                #264 #268 #277 closed (5 pass fixtures, the 3 rows among them).
#                ALL 11195 -> 11198 (+3), NOIMPORTED 6632 -> 6635 (+3), TIERCOMMIT 150 -> 149 (-1) — 2026-09-24: squeue
#                #122 closed (4 pass fixtures, the row among them).
#                ALL 11198 -> 11202 (+4), NOIMPORTED 6635 -> 6639 (+4), TIERCOMMIT 149 -> 148 (-1) — 2026-09-24: squeue
#                #306 closed (5 pass fixtures, the row among them).
#                ALL 11202 -> 11210 (+8), NOIMPORTED 6639 -> 6647 (+8), TIERCOMMIT 148 -> 147 (-1) — 2026-09-24: squeue
#                #120 #121 closed, let_array_named_rest_refused rowed (#472); 6 new pass fixtures, 2 new fail fixtures
#                (fnparam/closure E0005); fail/fnparam_array_drop_rest_unbound moved to pass (net 0).
#                ALL 11210 -> 11213 (+3), NOIMPORTED 6647 -> 6650 (+3), TIERCOMMIT 147 -> 146 (-1) — 2026-09-24: squeue
#                #118 closed (3 pass fixtures, the row among them) + fail/match_pattern_duplicate_binding_e0416.
#                ALL 11213 -> 11215 (+2), NOIMPORTED 6650 -> 6652 (+2), TIERCOMMIT 146 -> 144 (-2) — 2026-09-24: squeue
#                #159 #146 closed as fail fixtures (net 0); + spec/fail expr_diag__deref-non-pointer, pass/fnitem_array_elem_static_return_ok.
#                ALL 11215 -> 11216 (+1), NOIMPORTED 6652 -> 6653 (+1), TIERCOMMIT 144 -> 142 (-2) — 2026-09-24: squeue
#                #117 #451 closed (2 pass fixtures, net 0) + fail/match_deref_at_byvalue_move_e0507.
#                ALL 11216 -> 11217 (+1), NOIMPORTED 6653 -> 6654 (+1), TIERCOMMIT 142 -> 141 (-1) — 2026-09-24: squeue
#                #260 closed as a fail fixture (net 0) + pass/refcell_push_outliving_refs_ok.
#                ALL 11217 -> 11219 (+2), NOIMPORTED 6654 -> 6656 (+2) — 2026-09-24: fail/vec_iter_mut_store_block_local_e0597,
#                pass/vec_iter_mut_store_outliving_ok.
#                ALL 11219 -> 11221 (+2), NOIMPORTED 6656 -> 6658 (+2), TIERCOMMIT 141 -> 140 (-1) — 2026-09-24: squeue
#                #244 closed as a fail fixture (net 0) + fail/elided_ptr_regions_generic_call_refused, pass/ptr_same_region_compare_ok.
#                ALL 11221 -> 11223 (+2), NOIMPORTED 6658 -> 6660 (+2), TIERCOMMIT 140 -> 139 (-1) — 2026-09-24: squeue
#                #263 closed (3 pass fixtures, the row among them).
#                ALL 11223 -> 11227 (+4), NOIMPORTED 6660 -> 6664 (+4), TIERCOMMIT 139 -> 134 (-5) — 2026-09-24: squeue
#                #458 #126 #109 #145 #300 closed (9 pass fixtures, the 5 rows among them).
#                ALL 11227 -> 11231 (+4), NOIMPORTED 6664 -> 6668 (+4), TIERCOMMIT 134 -> 128 (-6) — 2026-09-24: squeue
#                #301 #90 #279 #183 #209 #283 closed (10 pass fixtures, the 6 rows among them);
#                + pass/refbind_over_ref_scrutinee_deref_coerced_call (+1 ALL, +1 NOIMPORTED).
#                ALL 11232 -> 11234 (+2), NOIMPORTED 6669 -> 6671 (+2), TIERCOMMIT 128 -> 125 (-3) — 2026-09-24: squeue
#                #207 #185 #208 closed (5 pass fixtures, the 3 rows among them).
#                ALL 11234 -> 11235 (+1), NOIMPORTED 6671 -> 6672 (+1), TIERCOMMIT 125 -> 122 (-3) — 2026-09-24: squeue
#                #107 #211 #222 closed (4 pass fixtures, the 3 rows among them).
#                ALL 11235 -> 11243 (+8), NOIMPORTED 6672 -> 6680 (+8), TIERCOMMIT 122 -> 118 (-4) — 2026-09-24: squeue
#                #204 #206 #139 #152 #154 #155 closed (10 pass fixtures, the 6 rows among them; 2 fail
#                fixtures); rows #473 #474 opened (+2 ALL, +2 NOIMPORTED, +2 TIERCOMMIT).
#                ALL 11243 -> 11256 (+13), NOIMPORTED 6680 -> 6693 (+13), TIERCOMMIT 118 -> 107 (-11) — 2026-09-24:
#                squeue #156 #238 #201 #202 #128 #273 #241 #242 #203 #190 #189 closed (10 pass + 1 fail
#                fixtures are the rows; + 6 pass, 7 fail).
#                ALL 11256 -> 11258 (+2), NOIMPORTED 6693 -> 6695 (+2), TIERCOMMIT 107 -> 106 (-1) — 2026-09-24: squeue
#                #314 closed (3 pass fixtures, the row among them).
#                ALL 11258 -> 11260 (+2), NOIMPORTED 6695 -> 6697 (+2), TIERCOMMIT 106 -> 104 (-2) — 2026-09-25: squeue
#                #140 #133 closed (3 pass fixtures, the 2 rows among them; 1 fail).
#                ALL 11260 -> 11262 (+2), NOIMPORTED 6697 -> 6699 (+2), TIERCOMMIT 104 -> 103 (-1) — 2026-09-25: squeue
#                #89 closed (its program is a fail fixture now; + 1 pass, 1 fail).
#                ALL 11262 -> 11265 (+3), NOIMPORTED 6699 -> 6702 (+3), TIERCOMMIT 103 -> 100 (-3) — 2026-09-25: squeue
#                #454 #106 #278 closed (#454's program is a fail fixture, the other two pass; + 2 pass, 1 fail).
#                ALL 11265 -> 11266 (+1), NOIMPORTED 6702 -> 6703 (+1), TIERCOMMIT 100 -> 99 (-1) — 2026-09-25: squeue
#                #108 closed (2 pass fixtures, the row among them).
#                ALL 11266 -> 11268 (+2), NOIMPORTED 6703 -> 6705 (+2), TIERCOMMIT 99 -> 98 (-1) — 2026-09-25: squeue
#                #150 closed (3 pass fixtures, the row among them).
#                ALL 11268 -> 11271 (+3), NOIMPORTED 6705 -> 6708 (+3), TIERCOMMIT 98 -> 97 (-1) — 2026-09-25: squeue
#                #472 closed (3 pass fixtures, the row among them; 1 fail).
#                ALL 11271 -> 11273 (+2), NOIMPORTED 6708 -> 6710 (+2), TIERCOMMIT 97 -> 96 (-1) — 2026-09-25: squeue
#                #127 closed (3 pass fixtures, the row among them).
#                ALL 11273 -> 11275 (+2), NOIMPORTED 6710 -> 6712 (+2), TIERCOMMIT 96 -> 95 (-1) — 2026-09-25: squeue
#                #288 closed (2 pass fixtures, the row among them; 1 fail).
#                ALL 11275 -> 11277 (+2), NOIMPORTED 6712 -> 6714 (+2), TIERCOMMIT 95 -> 94 (-1) — 2026-09-25: squeue
#                #153 closed (its program is a fail fixture now; + 1 pass, 1 fail).
#                ALL 11277 -> 11278 (+1), NOIMPORTED 6714 -> 6715 (+1), TIERCOMMIT 94 -> 93 (-1) — 2026-09-25: squeue
#                #149 closed (2 pass fixtures, the row among them).
#                ALL 11278 -> 11280 (+2), NOIMPORTED 6715 -> 6717 (+2), TIERCOMMIT 93 -> 91 (-2) — 2026-09-25: squeue
...
#                ALL 11287 -> 11289 (+2), NOIMPORTED 6724 -> 6726 (+2), TIERCOMMIT 87 -> 86 (-1) — 2026-09-25: squeue
#                #194 closed (1 pass fixture is the row; + 2 fail).
#                ALL 11289 -> 11292 (+3), NOIMPORTED 6726 -> 6729 (+3), TIERCOMMIT 86 -> 84 (-2) — 2026-09-25: squeue
...
#                ALL 11296 -> 11299 (+3), NOIMPORTED 6733 -> 6736 (+3), TIERCOMMIT 81 -> 80 (-1) — 2026-09-25: squeue
#                #297 closed (its program is a pass fixture now; + 2 pass, 1 fail).
#                ALL 11299 -> 11300 (+1), NOIMPORTED 6736 -> 6737 (+1), TIERCOMMIT 80 -> 81 (+1) — 2026-09-25: squeue
#                #475 opened (struct_lit_const_promotion_refused, its open-shelf program).
#                ALL 11300 -> 11305 (+5), NOIMPORTED 6737 -> 6742 (+5), TIERCOMMIT 81 -> 80 (-1) — 2026-09-25: squeue
#                #475 closed (a pass fixture now; + 1 pass, 1 fail) and i128_literal_promotion_refused opened (+1
#                open-shelf); #91 closed (a fail fixture now; + 1 fail, 1 pass).
#                ALL 11305 -> 11306 (+1), NOIMPORTED 6742 -> 6743 (+1), TIERCOMMIT 80 -> 79 (-1) — 2026-09-25: squeue
#                #476 closed (a pass fixture now; + 1 pass).
#                ALL 11306 -> 11308 (+2), NOIMPORTED 6743 -> 6745 (+2), TIERCOMMIT 79 -> 78 (-1) — 2026-09-25: squeue
...
#                (+ 1 pass, 1 fail).
#                ALL 11315 -> 11317 (+2), NOIMPORTED 6752 -> 6754 (+2), TIERCOMMIT 77 -> 76 (-1) — 2026-09-25: squeue
#                #270 closed (a pass fixture now; + 1 pass, 1 fail).
#                ALL 11317 -> 11319 (+2), NOIMPORTED 6754 -> 6756 (+2), TIERCOMMIT 76 -> 75 (-1) — 2026-09-25: squeue
#                #274 closed (a pass fixture now; + 1 pass, 1 fail).
#                ALL 11319 -> 11323 (+4), NOIMPORTED 6756 -> 6760 (+4), TIERCOMMIT 75 -> 73 (-2) — 2026-09-25: squeue
#                #239 #298 closed (both pass fixtures now; + 3 pass, 1 fail).
#                ALL 11323 -> 11326 (+3), NOIMPORTED 6760 -> 6763 (+3), TIERCOMMIT 73 -> 70 (-3) — 2026-09-25: squeue
#                #100 #88 #136 closed (pass fixtures now), #440 (a fail fixture now); + 2 pass; impl_fn_return_env_leaks
#                opened (+1 open-shelf).
#                ALL 11326 -> 11328 (+2), NOIMPORTED 6763 -> 6765 (+2), TIERCOMMIT 70 -> 69 (-1) — 2026-09-25: squeue
#                #137 closed (a pass fixture now; + 1 pass, 1 fail).
#                ALL 11328 -> 11334 (+6), NOIMPORTED 6765 -> 6771 (+6), TIERCOMMIT 69 -> 67 (-2) — 2026-09-25: squeue
#                #193 #205 closed (local type inference; pass fixtures now, -2 open-shelf); + 4 pass, 2 fail.
#                ALL 11334 -> 11338 (+4), NOIMPORTED 6771 -> 6775 (+4), TIERCOMMIT 67 -> 68 (+1) — 2026-09-25: squeue
#                #473 closed (a pass fixture now); + 2 pass; debug_through_generic_ref_unresolved and
#                array_impl_compound_generic_elem_refused opened (+2 open-shelf).
#                ALL 11338 -> 11339 (+1), NOIMPORTED 6775 -> 6776 (+1), TIERCOMMIT 68 -> 67 (-1) — 2026-09-25: squeue
#                #478 closed (a pass fixture now); + 1 pass.
#                ALL 11339 -> 11342 (+3), NOIMPORTED 6776 -> 6779 (+3), TIERCOMMIT 67 -> 66 (-1) — 2026-09-25: squeue
#                #477 closed (a pass fixture now); + 2 pass, 1 fail.
#                ALL 11342 -> 11343 (+1), NOIMPORTED 6779 -> 6780 (+1), TIERCOMMIT 66 -> 65 (-1) — 2026-09-25: squeue
#                #474 closed (a pass fixture now); + 1 pass.
#                ALL 11343 -> 11345 (+2), NOIMPORTED 6780 -> 6782 (+2), TIERCOMMIT 65 (±0) — 2026-09-25: squeue
#                #479 closed (a pass fixture now); + 1 pass; array_impl_nested_generic_elem_refused opened.
#                ALL 11345 -> 11346 (+1), NOIMPORTED 6782 -> 6783 (+1), TIERCOMMIT 65 -> 64 (-1) — 2026-09-25: squeue
#                #480 closed (a pass fixture now); + 1 pass.
#                ALL 11346 -> 11358 (+12), NOIMPORTED 6783 -> 6795 (+12), TIERCOMMIT 64 -> 71 (+7) — 2026-09-25: the
#                discovery battery's findings: 8 squeue rows opened, #486 closed with them (a pass fixture now); + 2 pass, 2 fail.
#                ALL 11358 -> 11360 (+2), NOIMPORTED 6795 -> 6797 (+2), TIERCOMMIT 71 -> 69 (-2) — 2026-09-26: squeue
#                #481 #482 closed (pass fixtures now); + 2 pass.
#                ALL 11360 -> 11363 (+3), NOIMPORTED 6797 -> 6800 (+3), TIERCOMMIT 69 -> 66 (-3) — 2026-09-26: squeue
#                #483 #487 #488 closed (pass fixtures now); + 3 pass.
#                ALL 11363 -> 11364 (+1), NOIMPORTED 6800 -> 6801 (+1), TIERCOMMIT 66 -> 65 (-1) — 2026-09-26: squeue
#                #485 closed (a fail fixture now); + 1 pass.
#                ALL 11364 -> 11369 (+5), NOIMPORTED 6801 -> 6806 (+5), TIERCOMMIT 65 -> 66 (+1) — 2026-09-26: the
#                second discovery battery: + 4 pass; iterator_trait_item_assoc_type_refused opened (+1 open-shelf).
#                ALL 11369 -> 11378 (+9), NOIMPORTED 6806 -> 6815 (+9), TIERCOMMIT 66 -> 71 (+5) — 2026-09-26: the
#                third discovery battery: + 4 pass; five squeue rows opened (+5 open-shelf).
#                ALL 11378 -> 11379 (+1), NOIMPORTED 6815 -> 6816 (+1), TIERCOMMIT 71 -> 70 (-1) — 2026-09-26: squeue
...
#                ALL 11388 -> 11394 (+6), NOIMPORTED 6825 -> 6831 (+6), TIERCOMMIT 68 -> 70 (+2) — 2026-09-26: four
#                pass fixtures (operators on reference Self, literal rhs, identity From, recursive Rc/Weak node type) and
#                two squeue rows (Ord max/min default, heterogeneous PartialEq<Rhs>).
#                ALL 11394 -> 11401 (+7), NOIMPORTED 6831 -> 6838 (+7), TIERCOMMIT 70 -> 72 (+2) — 2026-09-26: four
#                pass fixtures (refutable aggregate in a payload, unit variant in a tuple element, nested-variant non-Copy
#                return, `@` string arm), one fail (tuple scrutinee E0004), two squeue rows (inference gaps).
#                ALL 11401 -> 11404 (+3), NOIMPORTED 6838 -> 6841 (+3), TIERCOMMIT 72 -> 71 (-1) — 2026-09-26: squeue
#                #498 #499 closed (two pass fixtures now), let_tuple_literal_width_from_later_use_refused opened, two
#                fail fixtures (tuple / array value widening is E0308).
#                ALL 11404 -> 11406 (+2), NOIMPORTED 6841 -> 6843 (+2), TIERCOMMIT 71 -> 71 — 2026-09-26: squeue #492
#                #496 closed (two pass fixtures now), bare_self_param_shorthand_refused and
#                ref_param_address_taken_return_yields_slot opened.
REGISTRY-ALL         11406
REGISTRY-NOIMPORTED  6843
REGISTRY-TIERCOMMIT  71
