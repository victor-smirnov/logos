...
- [How Models Behave](models.md) — facts about LLMs in isolation, under two complementary lenses. *Dynamical:* iterated-map dynamics, attraction basins, in-distribution vs out-of-distribution behavior. *Information-theoretic:* compressibility-split tasks, the determinism gap, deterministic components as offload / cache / guardrail, and model memory as a compressed forward-only program — point-query-only, non-invertible, non-self-enumerable. Plus what cannot be assumed.
...
- [Coding Tasks](coding-tasks.md) — what changes when the task is code: uniform unknown-unknown gaps, local-syntactic competence vs global-semantic correctness, the failure mode humans do not have — and the development methodology this forces (external corpus as mandatory index, gap-discovery dynamics, instance-coverage vs class-generality and the generalizing pass), with how Logos materializes it. This is the bridge from the model-behavior pillar to concrete methodology.
