...
Specification of trait declaration, resolution, dispatch, coherence, associated items, generic parameters, inference, variance, and monomorphization; extracted from the grammar, sema, mono, and codegen layers of the reference implementation (`src/compiler/`, `tools/peg_gen_cpp/grammars/logos.peg`) — each `### ` heading is the rule's permanent linkable id.
...
`[pub] [auto|unsafe] trait NAME [<params>] [: super (+ super)*] { items }` defines a trait; `auto` and `unsafe` are mutually-exclusive optional leading modifiers; the supertrait list after `:` is `+`-separated. Generic params, supertraits, both, or neither may be present, independently of the modifier.
...
### `trait.decl.explicit-instantiation` — Explicit trait (genos) specialization declaration

`[pub] genos TypeRef;` (trait_kw type_ref SEMI, no body) binds attribute metadata to a logical-family (trait) specialization; implementing eidos inherit the specialization via `impl`.

```logos
#[type_code=N] pub genos Array<i32>;
```

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L1139-L1146`
...
### `trait.bound.assoc-type-eq` — Associated-type equality clause in bound args

Inside a trait bound's `<...>` argument list, each bound_arg is either a positional type_or_lt_arg or an associated-type equality clause `Name = Type` (ASSOC_EQ_BIND: NAME=assoc-type name, TYPE=rhs type AST); the two forms may be mixed in one comma-separated list (e.g. `Iterator<Item = T>`-style constraints).

```logos
Iterator<Item = i32>
```

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L276`, `tools/peg_gen_cpp/grammars/logos.peg#L3188-L3193`
...
### `trait.bound.fn-family` — Fn-family parenthesized trait-bound sugar

`IDENT(T1, T2, ...) [-> R]` and `IDENT() [-> R]` are parenthesized trait-bound forms (e.g. `Fn(T)->R`, `FnMut(T)`, `FnOnce()`); the argument types populate PARAMS (parallel to the angle-bracket TYPE_PARAMS slot) and an optional `-> R` populates RET_TYPE. Available both with and without a leading HRTB binder.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3159-L3166`, `tools/peg_gen_cpp/grammars/logos.peg#L3175-L3182`
...
`for<'a, 'b, ...> Trait<...>` (HRTB) is accepted as a bound prefix on every trait_bound alternative, but Logos does not track lifetimes structurally: the binder is parsed into a disposable head and dropped, so `for<'a> Fn(&'a T)` is semantically identical to `Fn(&T)`.
...
### `trait.bound.hrtb-for-binder` — `for<'a, 'b>` higher-ranked binder on a trait bound

A TRAIT_BOUND may carry a `for<'a, 'b, …>` higher-ranked lifetime binder list, represented as an HRTB_BINDERS sub-node whose ITEMS are LIFETIME_PARAM entries.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L75`

### `trait.bound.relaxed-sized` — Relaxed bound syntax `?IDENT`

`? IDENT` is grammatically accepted as a relaxed trait_bound (TRAIT_BOUND with RELAXED=true) for any IDENT, opting out of an implicit bound on the type parameter (intended use: `?Sized`).

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3152-L3156`
...
A trait item may also be an associated type `type NAME [<params>] [: Bound (+ Bound)*] [= T] ;` (a non-empty `<params>` list makes it a GAT — generic associated type) or an associated const `const NAME: T [= expr] ;`, both with an optional default value/type.

```logos
type Item;
type Item: Ord = i32;
type Wrap<U>: Iterator<Item = U>;
const N: usize = 0;
```

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L980-L991`
...
**Related:** `trait.supertrait.blanket-satisfies`, `trait.supertrait.self-bound-satisfies`
...
### `trait.impl.negative-impl-syntax` — `impl !Trait for Type {}` negative implementation

`impl !Trait for Type {}` is parsed as an IMPL_BLOCK carrying an IS_NEGATIVE marker, denoting a negative trait implementation (an explicit opt-out of Trait for Type).

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L74`
...
A trait method is `[unsafe] fn (NAME|new|null) [<params>] (params) [-> T] [where ...] (block | ';')`: a body-bearing form supplies a default implementation, a `;`-terminated form declares a required (abstract) method. `where` may follow the signature, before the block or the terminating `;`; on a default body it gates per-impl synthesis of that default (skipped for an impl where the bound doesn't hold for the impl's concrete type).
...
*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L906-L979`
...
### `trait.dyn.auto-trait-and-lifetime-bounds` — dyn Trait + Send/Sync + 'a auxiliary bounds

A trait-object type may carry auxiliary bounds after the principal trait: `+ Send`/`+ Sync`-style auto-trait bounds (AUTO_TRAIT_BOUND, NAME=bound ident) and `+ 'a` lifetime bounds (AUTO_LIFE_BOUND, NAME=lifetime label). Both collect into the dyn_type's ITEMS array alongside type-args; `resolve_type` buckets bounds vs args by CODE. Auto-trait bounds are folded into the TraitObject TypeRef's const_val bits and enforced at unsize-coercion sites; lifetime bounds are currently only logged, not enforced, pending region-inference wiring.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L315-L316`
...
### `trait.genos.declaration` — genos declaration is a trait with is_genos=true

A `genos` declaration (GENOS_DEF) is sema-represented as an ordinary trait definition with the flag `is_genos=true` set, i.e. it shares the trait-definition data shape rather than introducing a separate item kind.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L265`
...
### `generic.param.const-param` — Const generic parameter forms

type_param accepts `const IDENT ... : Type` (variadic const param, IS_VARIADIC=true), `const # IDENT : Type` (const param bound to a name-variable, NAME_VAR), and `const IDENT : Type` (fixed-name const param) — all produce CONST_PARAM.
...
### `generic.param.default` — Type parameter with default type

`IDENT : trait_bound (+ trait_bound)* = Type` and `IDENT = Type` supply a default TYPE on a type_param, with or without trait bounds respectively.

```logos
T: Default = i32
```

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3234-L3237`
...
### `generic.param.name-var` — Name-variable type parameter

`# IDENT : trait_bound (+ trait_bound)*` and bare `# IDENT` are type_param forms bound to a name-variable (NAME_VAR) rather than a fixed identifier, with or without trait bounds respectively — a metaprogramming-facing parameter-naming mechanism.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3230-L3233`

### `generic.param.repeat-group` — Parameter-pack repeat-group type param

`#(type_param), *` is a REPEAT_GROUP type_param form wrapping an inner type_param with a repetition marker (OP=1).

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3207-L3208`
...
### `generic.param.type-outlives-bound` — Type parameter with lifetime-outlives bound

`IDENT : lifetime_param (+ lifetime_param)*` is a type_param form (TYPE_PARAM) whose ITEMS are all LIFETIME_PARAM nodes, distinguished from the ordinary trait-bound TYPE_PARAM form (whose ITEMS are TRAIT_BOUND nodes) by the element code; must be tried before the trait-bound alternative so the leading `'` resolves the choice.

```logos
T: 'a + 'b
```

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3210-L3217`

### `generic.param.type-param-list` — Generic type-parameter list

`<type_param, ...>` is a generic parameter list; the same production also serves as a specialization-pattern argument list (e.g. `<i32>`, `<*T>`, `<[T;4]>`) — the grammar does not distinguish a type-var parameter from a concrete specialization arg; that classification is done in sema.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3131-L3135`
...
### `generic.param.variadic` — Variadic type parameter (pack)

`IDENT ... : trait_bound (+ trait_bound)*` and bare `IDENT ...` are variadic type_param forms (TYPE_PARAM, IS_VARIADIC=true), with or without trait bounds respectively.
...
### `generic.arg.type-arg-list` — Generic instantiation type-argument list

`<type_or_lt_arg, ...>` (type_arg_list) is a comma-separated list of type or lifetime arguments used for generic instantiation in type position (e.g. `Vec<i32>`); trailing comma allowed.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3244-L3245`
...
### `generic.where.clause` — `where` clause syntax

An item-level generic bound list is written `where Pred (, Pred)*` where each Pred is a where_pred.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L1317-L1319`
...
### `generic.where.ref-subject` — Reference-typed where-clause subject

A where-clause predicate's subject may be a reference type: `&T: Bound (+ Bound)*`, `&mut T: Bound (+ Bound)*`, or `for<'a> &'a T: Bound (+ Bound)*` (an associated-type subject `T::Item: Bound` is likewise accepted syntactically). Such a predicate is parsed as a TYPE_PARAM node carrying a TYPE field (the `&T`/`&mut T`/assoc-type node) instead of a NAME field, distinguishing it from the plain `T: Bound` form which falls through to the ordinary type_param rule. Sema's where-clause walker resolves the TYPE node; for a `&T`/`&mut T` subject over a type-param T, it records the listed bound(s) on the underlying type-param T, flagging each as `on_ref_subject` (with `is_ref_mut` set for `&mut`), so the method resolver applies these bounds only to a receiver of the matching (shared vs. mutable) reference kind. An associated-type projection subject is accepted and parsed but only parse-and-skipped semantically (not yet enforced); full projection-bound checking is a separate, unimplemented feature.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L1321-L1330`, `src/compiler/sema.cpp#L4210-L4259`
...
*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L2737-L2738`, `tools/peg_gen_cpp/grammars/logos.peg#L2774`
...
### `generic.pack.sizeof-and-expand` — Variadic pack size and expansion in expression position

`P...(N)` (`SIZEOF_PACK`) yields the length of variadic pack `P` bound to name `N`; `P...` in expression position (`PACK_EXPAND`) expands the pack `P`.

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L2605-L2606`, `tools/peg_gen_cpp/grammars/logos.peg#L2632`, `tools/peg_gen_cpp/grammars/logos.peg#L2834`
...
**Related:** `generic.spec.struct-pattern-classification`
...
**Related:** `item.name.forward-reference`
...
### `generic.lifetime.outlives` — Lifetime parameter with outlives bound

A lifetime_param is `'a : 'b (+ 'c)*` (with an outlives bound list of one or more lifetimes) or bare `'a`; each outlives lifetime is wrapped in a LIFETIME_PARAM sub-node so it is collected by the parent's `$...` array (bare LIFETIME tokens are not).

*Source:* `tools/peg_gen_cpp/grammars/logos.peg#L3195-L3205`
