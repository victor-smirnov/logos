...
*Evidence:* `stdlib/mem/deem/deem.logos` (`rt_i` / `rt_f` `B` arms — `rt_i`'s dates to c00b2888 2026-07-02, `rt_f`'s to ce973c17); the shared rule table `agg_result_ty` admitting BOOL; ⚠ its end-to-end pin `query_dyn_bool_arith_pinned` died at P5 (census row 14); `rt_i`/`rt_f` survive in `stdlib/mem/deem/deem.logos` and are reachable through `Tpl::render`, but no fixture drives their `B` arms
