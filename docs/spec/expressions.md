...
The receiver and argument verdicts are emitted by `SemaChecker::expect_type`, which owns the type-mismatch diagnostic (`scripts/lint-mismatch-monopoly.sh` makes a second emitter a build failure); only the arity verdict, which is not a type mismatch, is emitted directly. The dispatch selector and `expect_type` are ONE judgment on an integer literal: a SUFFIXED literal (`9u64`) has its suffix's type and adopts no other width (Rust, E0308), and an UNSUFFIXED literal adopts a narrower integer only when its value fits — the selector decides it by value before type compatibility, so `put(u8)`/`put(i64)` called with `300` reaches `put(i64)`. An overload set rejected at an argument therefore reports the first candidate's slot (`method 'C__put' arg 1: expected u8, got u64`); an unsuffixed literal no candidate can hold reports its range (`value N does not fit in T`). (Closed soundness-queue row `overload_set_arg_mismatch_says_no_method`, 2026-09-25; tests/logos/fail/overload_set_arg_mismatch_says_no_method.logos, tests/logos/pass/intlit_unsuffixed_adopts_overload.logos.)
...
The trait NAME is RESOLVED at sema (`SemaChecker::resolve_trait_query_name`) to the trait's registry IDENTITY — the bare name for a trait that owns the bare slot, `pkg::Name` for a same-name trait a B-mv-02 collision pushed under its package-qualified key — and that identity, not the spelling, is what travels to mono. Mono's fact table is keyed by the same identity (`impl_keys::CANONICAL_TRAIT` → `Mono::concrete_impls_`, a `(trait, type)` PAIR), so a package-local `trait Hash` and `logos.lang.hash::Hash` are two traits with two fact sets and a query naming one cannot read the other's impls. Two spellings remain compile errors rather than answers: a name that denotes no trait at all (in the current package, any imported or re-exported package, or the bare registry slot), and a name that denotes MORE THAN ONE trait and resolves to none of them in this scope. Resolution deliberately falls back to the bare slot, so a module that does not import the trait's package still answers: capability is a property of the type, not of the asker's import view.

RESIDUAL: mono also files each fact under the impl's bare spelling as an alias, because bound checks still reach mono as bare text, so a query naming the trait that owns the BARE slot can still see a homonym's impls through that alias. Closing that direction requires canonicalising bound trait names at emit.
