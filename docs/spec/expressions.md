...
Scope: every expression-form and compiler-intrinsic semantic rule of Logos (domains `expr`, `intrinsic`). Rules are extracted from the compiler source layers - grammar, sema, mono/CTFE, borrow-check, and MLIR codegen - and grouped by their id middle-segment. Each rule's `id` is its permanent linkable address.
...
## Names, paths, and variable references
...
## Operators - binary, comparison, cast
...
## Reference, dereference, address-of
...
`let s = *b` where `b: Box<T>` and `T` is a move type moves the boxed value out (consuming `b`) and frees the block without dropping the content (Rust's built-in `*b` move). Copy-T Box and non-Box derefs use the normal copy/deref path.

Source: `src/compiler/sema_stmt.cpp#L1915-L1922`

> **Note - duplicate id.** This id was extracted from 2 source sites describing complementary facets. The additional facet(s) are recorded below (no semantic conflict detected, but the id is non-unique across artifacts):

> - `*b` where `b` is a bare variable of type `Box<T>` and T is a move (non-Copy) type consumes `b`, moves the value out, and frees the box (via `box_take::<T>` with T inferred). For a Copy element, `*b` copies and leaves `b` live.
...
After coercion, each argument's type must be compatible with the corresponding parameter type (or satisfy a `&T`→`dyn` reference match); incompatibility is an error 'expected E, got G'. Error-typed args/params are exempt.

Source: `src/compiler/sema_expr.cpp#L3247-L3256`, `src/compiler/sema_expr.cpp#L3503-L3513`

> **Note - duplicate id.** This id was extracted from 2 source sites describing complementary facets. The additional facet(s) are recorded below (no semantic conflict detected, but the id is non-unique across artifacts):

> - After coercions, each argument's type must be compatible with the (substituted) parameter type; an incompatible non-error, non-TypeVar, non-AssocType parameter yields an "expected X, got Y" error. Variance is additionally checked for non-TypeVar/non-AssocType parameters.
...
A method call must supply exactly `param_count - 1` explicit arguments (excluding the `self` receiver); a mismatch is an error.

Source: `src/compiler/sema_expr.cpp#L8867-L8871`

> **Note - duplicate id.** This id was extracted from 2 source sites describing complementary facets. The additional facet(s) are recorded below (no semantic conflict detected, but the id is non-unique across artifacts):

> - A method call must supply exactly (param count − 1) explicit arguments (excluding the `self` receiver); a mismatch is an error.
...
A struct literal may end with a functional-update base `..expr` that supplies remaining fields.

Source: `src/compiler/sema_render.cpp#L386-L391`

> **Note - duplicate id.** This id was extracted from 2 source sites describing complementary facets. The additional facet(s) are recorded below (no semantic conflict detected, but the id is non-unique across artifacts):

> - With `S { ..., ..base }`, the `base` expression must have struct type S (same struct name); every field not explicitly initialized is read from base via field-read, with the struct's type-params substituted into the carried field's declared type (generic path). A base of differing struct type is an error.
...
## Drop semantics (expression-level)
...
## String and raw-pointer expressions
...
## Writ (SDN) literals and captures
...
## Intrinsics - layout and pointer arithmetic
...
## Intrinsics - integer and bit operations
...
## Intrinsics - reflection and type queries
...
## Intrinsics - dyn, vtables, slices, str, zone
...
## Intrinsics - drop and closure glue
...
## Intrinsics - annotations and metaprogramming
