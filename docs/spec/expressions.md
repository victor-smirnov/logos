...
Scope: expression forms and intrinsic/built-in operations of Logos (rule domains `expr`, `intrinsic`). Sourced from extracted rule artifacts under `tools/spec-extract/rules/` (sema, codegen, mono, runtime/stdlib layers); each rule id is a permanent linkable address — never rename. Source evidence cites `file#line` in the compiler/stdlib.
...
**Divergence from Rust:** A: i24/u24/i56/u56 are Logos-only integer widths (no Rust equivalent).
...
**Divergence from Rust:** Logos models &str as Slice<u8> (writ/string-repr); identity conversion.
...
**Divergence from Rust:** Logos variadic-pack feature.
...
**Divergence from Rust:** Logos explicit-metacall model replaces Rust const-expression array lengths.
...
Source: `src/compiler/mlir_gen.cpp#L937-L972`, `src/compiler/mlir_gen.cpp#L1126-L1160`
...
**Divergence from Rust:** Logos built-in; struct-literal syntax over a scalar type
...
### `expr.struct-lit.functional-update` — Struct functional-update base

> Note: multiple extracted statements share this id (different source sites). All are preserved below; they are complementary refinements, not conflicting — reconcile if they diverge.

- (src/compiler/sema_render.cpp#L386-L391) A struct literal may end with a functional-update base `..expr` that supplies remaining fields.
- (src/compiler/sema_expr.cpp#L9970-L10013, src/compiler/sema_expr.cpp#L10171-L10213) With `S { ..., ..base }`, the `base` expression must have struct type S (same struct name); every field not explicitly initialized is read from base via field-read, with the struct's type-params substituted into the carried field's declared type (generic path). A base of differing struct type is an error.
...
Source: `src/compiler/sema_impl.hpp#L2358-L2379`
...
Source: `src/compiler/sema_impl.hpp#L3938-L3943`
...
Source: `src/compiler/mlir_gen.cpp#L716-L772`, `src/compiler/mlir_gen.cpp#L844-L859`, `src/compiler/mlir_gen.cpp#L869-L879`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1166-L1172`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1173-L1189`
...
**Divergence from Rust:** Rust also rejects unary `-` on unsigned types (no `Neg` impl); Logos diagnostic is bespoke (B-ex-04). Extra widths u24/u56 are Logos-only (A11).
...
**Divergence from Rust:** Always traps on overflow (B-ex-01) regardless of build profile, unlike Rust where overflow wraps in release and panics in debug.
...
Source: `src/compiler/mlir_gen_expr.cpp#L923-L1004`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1006-L1078`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1080-L1121`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1080-L1133`
...
**Divergence from Rust:** bool ordering forced unsigned to preserve Rust's `false < true` despite i1 signed representation; documented inline as Rust-conformant intent.
...
**Divergence from Rust:** Mirrors Rust `impl PartialEq<str> for String`.
...
**Divergence from Rust:** Rust-conformant outcome (chained comparison is an error); Logos detects it grammatically for a better diagnostic.
...
**Divergence from Rust:** Ranges are nominal stdlib structs (RangeI32/RangeI64/RangeOfIncl), not language built-ins
...
Source: `src/compiler/mlir_gen.cpp#L789-L805`, `src/compiler/mlir_gen.cpp#L813-L829`
...
**Divergence from Rust:** Custom-DST / self-describing model — see DIVERGENCES B2.
...
**Divergence from Rust:** Custom-DST raw-pointer-shaped field access — see DIVERGENCES B2.
...
**Divergence from Rust:** Custom-DST model — see DIVERGENCES B2.
...
**Divergence from Rust:** Custom-DST dyn-tail model — see DIVERGENCES B2/B3.
...
Source: `src/compiler/mlir_gen_expr.cpp#L1288-L1392`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1305-L1315`, `src/compiler/mlir_gen_expr.cpp#L1383-L1389`
...
**Divergence from Rust:** Range-slicing relies on stdlib `slice_get_range`; open/inclusive ends are clamped to length rather than panicking on out-of-range as Rust does.
...
Source: `src/compiler/mlir_gen_expr.cpp#L5175-L5238`
...
Source: `src/compiler/mlir_gen_expr.cpp#L5263-L5284`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1762-L1802`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1811-L1819`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1811-L1816`
...
Source: `src/compiler/sema_impl.hpp#L4242-L4251`

### `expr.deref.box-move-out` — let-binding a deref of a Box moves the content out

> Note: multiple extracted statements share this id (different source sites). All are preserved below; they are complementary refinements, not conflicting — reconcile if they diverge.

- (src/compiler/sema_stmt.cpp#L1915-L1922) `let s = *b` where `b: Box<T>` and `T` is a move type moves the boxed value out (consuming `b`) and frees the block without dropping the content (Rust's built-in `*b` move). Copy-T Box and non-Box derefs use the normal copy/deref path.
- (src/compiler/sema_expr.cpp#L2688-L2725) `*b` where `b` is a bare variable of type `Box<T>` and T is a move (non-Copy) type consumes `b`, moves the value out, and frees the box (via `box_take::<T>` with T inferred). For a Copy element, `*b` copies and leaves `b` live.
...
**Divergence from Rust:** Rust rejects `*x` on a non-pointer; Logos relaxes it to identity to accept faithful Rust loop imports (B3-bg-07), since `for i in &v` already yields T not &T.
...
Source: `src/compiler/mlir_gen_expr.cpp#L1398-L1447`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1428-L1441`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1403-L1415`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1456-L1477`, `src/compiler/mlir_gen_expr.cpp#L1512-L1559`, `src/compiler/mlir_gen_expr.cpp#L1560-L1653`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1479-L1510`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1704-L1746`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1442-L1447`, `src/compiler/mlir_gen_expr.cpp#L1724-L1741`
...
**Divergence from Rust:** Logos lets an inherent fn on the pointee shadow the built-in pointer is_null.
...
**Divergence from Rust:** Logos addition: distinct DEREF_WRITE/DEREF_COMPOUND statement forms; semantics match Rust place-expression assignment.
...
**Divergence from Rust:** Rust-conformant (expr.assign.drop-target / B8)
...
**Divergence from Rust:** Rust-conformant (items.union.fields.write-safety)
...
**Divergence from Rust:** Rust-conformant operator-overload semantics; Logos struct-name-keyed impl lookup.
...
Source: `src/compiler/sema_impl.hpp#L3791-L3796`
...
**Divergence from Rust:** Rust places the turbofish after the method for trait/inherent fns (e.g. T::method::<U>); Logos surface form puts it before the method name on the type path.
...
- (src/compiler/sema_expr.cpp#L3247-L3256, src/compiler/sema_expr.cpp#L3503-L3513) After coercion, each argument's type must be compatible with the corresponding parameter type (or satisfy a `&T`→`dyn` reference match); incompatibility is an error 'expected E, got G'. Error-typed args/params are exempt.
- (src/compiler/sema_expr.cpp#L4276-L4287) After coercions, each argument's type must be compatible with the (substituted) parameter type; an incompatible non-error, non-TypeVar, non-AssocType parameter yields an "expected X, got Y" error. Variance is additionally checked for non-TypeVar/non-AssocType parameters.
...
**Divergence from Rust:** Rust requires explicit `(s.m)(args)` to call a callable field; bare `s.m(args)` is method-only
...
**Divergence from Rust:** A10: dyn Fn* collapses to the bare Closure type.
...
**Divergence from Rust:** Rust-conformant (DIVERGENCES.md: trait-qualified UFCS supported)
...
### `expr.method.arity-check` — Method argument arity

> Note: multiple extracted statements share this id (different source sites). All are preserved below; they are complementary refinements, not conflicting — reconcile if they diverge.

- (src/compiler/sema_expr.cpp#L8867-L8871) A method call must supply exactly `param_count - 1` explicit arguments (excluding the `self` receiver); a mismatch is an error.
- (src/compiler/sema_expr.cpp#L7492-L7497) A method call must supply exactly (param count − 1) explicit arguments (excluding the `self` receiver); a mismatch is an error.
...
**Divergence from Rust:** Mirrors Rust autoderef order: try T/&T/&mut T at a deref level before stepping deeper.
...
**Divergence from Rust:** Mirrors Rust's E0507 'cannot move out of index'; scoped here to Vec::get.
...
**Divergence from Rust:** Result type is i64 (Logos stdlib uses i64 for lengths), not usize as in Rust.
...
**Divergence from Rust:** Logos `use Type.{V}` variant-import surface (pkg `.` / item `::` path model)
...
Source: `src/compiler/sema_impl.hpp#L3944-L3959`
...
**Divergence from Rust:** Logos closure ref-binding param syntax; no direct Rust equivalent.
...
**Divergence from Rust:** Capture mode is inferred per-variable from usage (read-only vs mutated), conceptually aligned with Rust closure capture-mode inference.
...
**Divergence from Rust:** No real `!`/never subtyping for tail-return; the return-value's type is adopted as a block-type proxy instead of `!`.
...
Source: `src/compiler/mlir_gen_expr.cpp#L3710-L3780`
...
Source: `src/compiler/mlir_gen_expr.cpp#L3742-L3759`, `src/compiler/mlir_gen_expr.cpp#L3763-L3773`
...
Source: `src/compiler/mlir_gen_expr.cpp#L3715-L3724`, `src/compiler/mlir_gen_expr.cpp#L3775-L3779`
...
**Divergence from Rust:** Rust-conformant: matches Rust LUB for fn-item match arms.
...
**Divergence from Rust:** Logos extension: Writ structured-data pattern matching (not in Rust).
...
Source: `src/compiler/sema_impl.hpp#L3655-L3673`
...
Source: `src/compiler/sema_impl.hpp#L3634-L3638`, `src/compiler/sema_impl.hpp#L3655-L3666`
...
Source: `src/compiler/sema_impl.hpp#L3675-L3678`
...
Source: `src/compiler/sema_impl.hpp#L3731-L3736`
...
**Divergence from Rust:** Receiver for from_residual is explicit from fn ret type (no contextual Self inference)
...
**Divergence from Rust:** Logos addition: Python-style comprehensions; not present in Rust.
...
**Divergence from Rust:** Logos-specific surface syntax (Python-style comprehension); not present in Rust.
...
**Divergence from Rust:** Narrower than Rust: only concrete array/slice, no IntoIterator/Iterator protocol.
...
**Divergence from Rust:** Logos-specific: surface sugar depends on a stdlib import being present.
...
**Divergence from Rust:** Logos-specific surface syntax; not present in Rust.
...
**Divergence from Rust:** Logos-specific.
...
**Divergence from Rust:** Logos addition: Writ self-describing data-notation literals.
...
**Divergence from Rust:** Logos-specific Writ data-literal syntax; no Rust equivalent.
...
**Divergence from Rust:** Logos-specific Writ syntax.
...
**Divergence from Rust:** Logos-specific const-generic/Writ syntax.
...
**Divergence from Rust:** Logos addition: Writ first-class type values have no Rust equivalent.
...
**Divergence from Rust:** Logos addition (Writ type literals).
...
**Divergence from Rust:** Logos addition (Writ/CFG type literals).
...
**Divergence from Rust:** Logos addition (Writ literals).
...
**Divergence from Rust:** Logos addition (Writ literals); note i24/i56/u24/u56 width suffixes.
...
**Divergence from Rust:** Logos addition (Writ literals); escape set is a fixed subset.
...
**Divergence from Rust:** Logos addition (Writ literals/captures).
...
**Divergence from Rust:** Logos addition (Writ captures).
...
**Divergence from Rust:** Logos addition (Writ SDN literals); no Rust equivalent.
...
**Divergence from Rust:** Logos addition: @-literal (Writ) capture has no Rust analogue.
...
**Divergence from Rust:** Logos-specific Writ data-substrate sugar; no Rust equivalent.
...
**Divergence from Rust:** Logos-specific Writ sugar; no Rust equivalent.
...
**Divergence from Rust:** Logos-specific (v1 limitation: string keys only).
...
**Divergence from Rust:** Rust additionally permits `.*` and `.N$` precision forms; Logos here requires a literal number after `.`.
...
Source: `src/compiler/mlir_gen_expr.cpp#L5290-L5373`
...
Source: `src/compiler/mlir_gen_expr.cpp#L5375-L5386`
...
Source: `src/compiler/mlir_gen_impl.hpp#L314-L333`
...
**Divergence from Rust:** Rust drops array elements in forward (index-ascending) order; tuple reverse-order is conformant. Array order here is N forward but element-by-element; flagged as possibly observable only via Drop side effects.
...
**Divergence from Rust:** Logos drop flags / static drop tracking (B8). Models Rust's conditional drop flags.
...
**Divergence from Rust:** Logos spelling of size_of; result is i64 (Rust mem::size_of -> usize).
...
**Divergence from Rust:** Result is i64 (Rust mem::align_of -> usize).
...
**Divergence from Rust:** Rust's offset_of! yields usize; Logos yields i64.
...
**Divergence from Rust:** Logos addition (unsafe raw-parts constructor).
...
**Divergence from Rust:** Logos addition (custom-DST construction intrinsic).
...
Source: `src/compiler/mlir_gen_expr.cpp#L1839-L1881`
...
**Divergence from Rust:** A6 (Logos-only metaprog/reflection intrinsic; no Rust equivalent)
...
**Divergence from Rust:** A6 (Logos-only type-level pack intrinsic)
...
**Divergence from Rust:** A6 (Logos-only reflection value)
...
**Divergence from Rust:** A6 (Logos-only reflection intrinsic)
...
**Divergence from Rust:** A6 (Logos-only type-level composition intrinsic)
...
**Divergence from Rust:** A6 (Logos-only variadic type-pack splice)
...
**Divergence from Rust:** Logos addition: compile-time type reflection intrinsics (no Rust equivalent).
...
**Divergence from Rust:** Logos addition (Writ reflection intrinsic).
...
**Divergence from Rust:** Logos addition (type reflection).
...
**Divergence from Rust:** Logos addition (Writ substrate).
...
**Divergence from Rust:** Logos addition (Writ/zoned reflection intrinsic).
...
**Divergence from Rust:** Logos addition (compile-time reflection intrinsic).
...
**Divergence from Rust:** Logos addition (metaprogramming intrinsic).
...
**Divergence from Rust:** Logos addition (variadic-tuple support).
...
**Divergence from Rust:** Logos addition (variadic-tuple support intrinsic).
...
**Divergence from Rust:** Logos addition (reflection intrinsic); no Rust equivalent.
...
Source: `src/compiler/mlir_gen_expr.cpp#L1906-L1921`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1922-L1941`
...
Source: `src/compiler/mlir_gen_impl.hpp#L966-L975`, `src/compiler/mlir_gen_impl.hpp#L1101-L1104`
...
**Divergence from Rust:** Logos addition (zoned/Writ datatypes).
...
**Divergence from Rust:** Logos addition (annotation metaprogramming).
...
**Divergence from Rust:** Logos addition (compile-time annotation reflection intrinsic).
...
Source: `src/compiler/mlir_gen_expr.cpp#L2118-L2189`, `src/compiler/mlir_gen_expr.cpp#L2001-L2117`
...
Source: `src/compiler/mlir_gen_expr.cpp#L2001-L2004`, `src/compiler/mlir_gen_expr.cpp#L2088-L2102`, `src/compiler/mlir_gen_expr.cpp#L2118-L2133`
...
Source: `src/compiler/mlir_gen_expr.cpp#L1983-L2000`, `src/compiler/mlir_gen_expr.cpp#L2135-L2154`, `src/compiler/mlir_gen_expr.cpp#L2149-L2154`
...
Source: `src/compiler/mlir_gen_expr.cpp#L2025-L2077`, `src/compiler/mlir_gen_expr.cpp#L1969-L1976`
...
Source: `src/compiler/mlir_gen_expr.cpp#L2235-L2265`
...
Source: `src/compiler/mlir_gen_expr.cpp#L2248-L2253`
...
**Divergence from Rust:** Logos addition: explicit free-function bit-op intrinsics.
...
**Divergence from Rust:** column!() always returns 0 rather than the true column.
...
**Divergence from Rust:** Rust supports item-position include!; Logos supports only expression position.
...
**Divergence from Rust:** Rust's include_bytes! has type &[u8;N] distinct from &str; in Logos both are Slice<u8>.
...
**Divergence from Rust:** option_env! returns an empty &str tombstone rather than Option<&str>.
...
**Divergence from Rust:** Floats and char literals are not supported (Rust supports them).
...
Source: `src/compiler/mlir_gen_expr.cpp#L2216-L2233`
...
**Divergence from Rust:** Logos addition: no Rust equivalent free function.
...
Source: `src/compiler/mlir_gen_impl.hpp#L1085-L1098`
...
Source: `src/compiler/mlir_gen_impl.hpp#L1089-L1096`
...
Source: `src/compiler/mlir_gen_impl.hpp#L1093-L1098`
...
Source: `src/compiler/mlir_gen_impl.hpp#L1099-L1104`
...
Source: `src/compiler/mlir_gen_impl.hpp#L1105-L1107`
...
Source: `src/compiler/mlir_gen_impl.hpp#L1108-L1111`
...
Source: `src/compiler/mlir_gen_impl.hpp#L1112-L1118`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L514-L545`, `src/compiler/mlir_gen_stmt.cpp#L591-L597`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L547-L605`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L606-L656`
...
**Divergence from Rust:** Logos addition: Writ/zone memory model intrinsic.
...
**Divergence from Rust:** Logos addition (zoned-reference construction intrinsic).
...
Source: `src/compiler/mlir_gen_impl.hpp#L438-L454`
...
**Divergence from Rust:** Logos addition: type-reflection metaprogramming intrinsic.
...
**Divergence from Rust:** Logos addition: type-level composition metaprogramming intrinsics.
