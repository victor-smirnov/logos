...
Scope: compile-time metaprogramming surface — function-like macros (`#[fn_macro]`, `#[token_macro]`), `quote!`/`quote_expr!`/`quote_item!`/`quote_ty!` AST quoting and antiquotation, explicit `comptime` metacalls, annotations (`#[...]`) and their `cfg!`/derive/trigger machinery, the `format!` family, and the `AnyVal`/`WAny` inline value-form + type/enum reflection intrinsics that back them. Sources: `tools/peg_gen_cpp/grammars/logos.peg` (grammar layer) and `src/compiler/mlir_gen_expr.cpp` / related codegen (lowering layer); extracted into `tools/spec-extract/rules/grammar/hrpc/*.json`.
