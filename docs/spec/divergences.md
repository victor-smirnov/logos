...
- **Divergence**: Logos addition.
- **Rule**: `has_trait::<T, Trait>()` requires two type arguments and yields `bool`: whether concrete T implements Trait, resolved at mono against the same impl tables (concrete + recursive blanket lookup) that drive method dispatch. The second argument is read by its identifier name only (passed as a string literal arg), not resolved as a type. Missing T or empty Trait name is a compile error. The NAME is resolved at sema by `SemaChecker::resolve_trait_query_name` (shared with `has_trait_of`) to the trait's registry IDENTITY, which is what reaches mono; `Mono::concrete_impls_` is keyed by the `(trait identity, target type)` PAIR (fed by `impl_keys::CANONICAL_TRAIT`), so a package-local homonym of a stdlib trait is a DISTINCT fact set and is answered rather than refused. A spelling that denotes no trait in scope, and one that denotes several and resolves to none of them here, remain compile errors. Lookup falls back to the bare registry slot, so the answer stays independent of the asker's imports. Residual: facts are also filed under the impl's bare spelling as an alias for bare-text bound lookups, so the bare-slot direction of the conflation is still open.
- **Source**: ``SemaChecker::lower_intrinsic_call` (the `has_trait` branch) and `SemaChecker::lower_intrinsic_has_trait_of`, both in src/compiler/sema_expr.cpp — cited by SYMBOL because the line range pointed at `lower_intrinsic_template_of` and had been stale since before P5`
