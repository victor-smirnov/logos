...
This is the **compiler-derived** divergence register: every rule whose extracted `divergence` field is non-empty, harvested from `tools/spec-extract/rules/**/*.json` across all domains (grammar, sema, mono, codegen, runtime, …). It is the mechanical cross-cutting companion to the hand-maintained policy document `docs/DIVERGENCES.md` (the single source of truth for *blessed* §A divergences and §B catch-up TODOs). Each group below cross-references its §A/§B/§G status there; a tag with no counterpart is flagged **unregistered — needs triage**.

Total rules carrying a divergence note: **485**, across **17** tag families.

## A1 — const-eval → `metacall`

_Cross-ref:_ Matches blessed **§A1** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
- **Statement:** `metacall` accepts exactly three operand shapes — a block (`metacall { … }`), a parenthesized expression (`metacall (e)`), or a call expression (`metacall f(…)`, including generic `f::<T>(…)` and static `Type::m(…)`) — and evaluates its argument at compile time.
- **Domain:** `metaprog`
...
_Cross-ref:_ Matches blessed **§A2** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
_Cross-ref:_ Matches blessed **§A3** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
_Cross-ref:_ Matches blessed **§A6** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
- **Statement:** The configured implicit-prelude package is implicitly wildcard-imported into every source-side module loaded for the current compilation, making prelude names resolvable unqualified, EXCEPT: (1) modules loaded from binary archives (their producer already applied its own prelude), (2) the prelude package itself, and (3) any file carrying the inner attribute `#![no_implicit_prelude]`. The injection is deduplicated against an explicit `use` of the same package, and an empty implicit-prelude setting injects nothing. The same injection is applied identically in the metaprog discovery pass and the final pass.
- **Domain:** `module`
...
## A7 — panic strategy: abort-only, no unwind

_Cross-ref:_ Matches blessed **§A7** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
## A8 — `#[pinned]` (type-level non-movability) + Rust `Pin<P>` coexist

_Cross-ref:_ Matches blessed **§A8** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
_Cross-ref:_ Matches blessed **§A9** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
## A10 — `dyn Fn*` collapses to the bare Closure type

_Cross-ref:_ Matches blessed **§A10** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
- **Statement:** A closure value is represented as a two-field aggregate (16 bytes on a 64-bit target): field 0 = function pointer, field 1 = environment pointer. Calling a closure invokes the function pointer with env_ptr prepended as the implicit first argument, followed by the user arguments.
- **Domain:** `layout`
...
## A11 — extra integer widths (I24/U24/I56/U56 for Writ)

_Cross-ref:_ Matches blessed **§A11** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
## A13 — integer overflow always traps (profile-independent)

_Cross-ref:_ Matches blessed **§A13** in `docs/DIVERGENCES.md` (intentional, will not converge to the Rust form).
...
## B1 — generic `T: Copy` auto-copy (resolved)

_Cross-ref:_ Tracked as catch-up TODO **§B1** in `docs/DIVERGENCES.md` (НЕ ОТКЛАДЫВАТЬ — must reach Rust parity unless marked resolved there).
...
## B2 — custom-DST tail-slice / owning `Box` (fat-pointer model)

_Cross-ref:_ Tracked as catch-up TODO **§B2** in `docs/DIVERGENCES.md` (НЕ ОТКЛАДЫВАТЬ — must reach Rust parity unless marked resolved there).
...
## B6 — slice mutability / NLL scope-lifetime (resolved)

_Cross-ref:_ Tracked as catch-up TODO **§B6** in `docs/DIVERGENCES.md` (НЕ ОТКЛАДЫВАТЬ — must reach Rust parity unless marked resolved there).
...
## B8 — assignment drop-before-replace (resolved)

_Cross-ref:_ Tracked as catch-up TODO **§B8** in `docs/DIVERGENCES.md` (НЕ ОТКЛАДЫВАТЬ — must reach Rust parity unless marked resolved there).
...
## B87 — dropck machinery

_Cross-ref:_ Not a top-level row in `docs/DIVERGENCES.md`; B87 is the dropck machinery referenced inside the §B6 (NLL) row. **unregistered as a standalone tag — needs triage.**
...
## G156-1 — two `Trait<T>` impls at distinct `T` (mangling/projection)

_Cross-ref:_ Tracked via the **§B-assoc** row in `docs/DIVERGENCES.md` (`G156-1` — narrow associated-type/method-mangling collision; catch-up, low priority).
...
## Logos-specific additions (no §A tag in the divergence text)

_Cross-ref:_ Maps to the **§A6 additions** umbrella in `docs/DIVERGENCES.md` (Logos-only features: variadics, Writ, metaprog/metacall, fibres — not parity items).
...
- **Statement:** The optional `from <module>` clause of a `use` names the providing module as its operand. The operand may be a bare identifier or a double-quoted string literal; surrounding quotes are stripped to yield the module name. Absence of the `from` clause records an empty module name, selecting default resolution. The clause is recorded as a (package dotted-path, from-module) pair.
- **Domain:** `module`
...
- **Statement:** An item's restricted-visibility marker `pub(W)` is accepted only when W is the contextual word `module` (module-linkage). Plain `pub` and no marker are non-module. Any other word (e.g. `pub(crate)`, `pub(super)`, `pub(in path)`) is rejected with the diagnostic "unsupported visibility `pub(W)` — only `pub(module)` is recognised".
- **Domain:** `module`
