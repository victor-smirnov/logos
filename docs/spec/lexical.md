...
Scope: lexical-domain (`lex.*`) rules — how raw source bytes are split into tokens for the Logos language and for the two embedded data/IDL grammars (Writ, HRPC). Source layers: the PEG token grammars (`tools/peg_gen/grammars/*.peg`) and the hand-rolled decoders/validators in the compiler (`src/compiler/*`). Each rule's id (the `### ...` heading) is its permanent linkable address, reproduced verbatim from the rule artifacts.
...
### `lex.comment.block-doc-strip` — Block doc-comment body stripping
A `/** ... */` doc comment yields its body with each line stripped of leading whitespace, one optional leading `*`, and one optional space after that `*`; trailing whitespace is removed; the envelope's own leading/trailing empty lines (from `/**\n ... \n*/`) are dropped.

Evidence: `src/compiler/sema_impl.hpp#L2791-L2827`

---

## Identifiers, Lifetimes, Keywords
...
> Note — magnitude-bound interaction: `lex.literal.int-overflow-i64` states a 64-bit raw bound, while `lex.literal.int128-magnitude` permits a 128-bit magnitude for `i128`/`u128` targets. These are not in conflict: the 64-bit bound is the default/unsuffixed raw path, the 128-bit path is selected when the target kind is `i128`/`u128`. Both retained verbatim.
...
> ⚠ Conflict — float grammar shape: `lex.literal.float` (PEG token grammar) requires a mandatory decimal point with a fractional part — it admits no exponent-only form. `lex.literal.float-format` (sema validator) additionally accepts a point-less form **(b)** where a mandatory exponent makes the token a float (e.g. `1e9`). The two layers disagree on whether an exponent-only token (no `.`) is a float literal. Both rules are surfaced verbatim; reconcile grammar vs. sema source before treating either as canonical.
...
> Note — two escape descriptions, different layers: `lex.litstr.escape-sequences` documents the **decode** path (`mlir_gen_expr.cpp`) and is the broader set (adds `\'` and `\u{...}`). `lex.str.escape-set` documents the **render** path (`sema_render.cpp`) and omits `\'` and `\u{...}`. Not a strict contradiction (decode vs. render are opposite directions), but the render escape set is narrower than the decode set; both retained for traceability.
...
## Writ Data-Language Lexer

The `lex.writ.*` rules govern the embedded Writ data-language grammar (`tools/peg_gen/grammars/writ.peg`), not Logos source.
...
The `lex.hrpc.*` rules govern the embedded HRPC interface-definition-language grammar (`tools/peg_gen/grammars/hrpc.peg`), not Logos source.
