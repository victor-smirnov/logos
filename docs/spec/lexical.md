...
Scope: the token grammar of the Logos source language plus its two embedded data-language surfaces (Writ data literals, HRPC IDL). Source layers: PEG token definitions (`tools/peg_gen/grammars/*.peg`), literal/escape decoding in semantic analysis and codegen (`src/compiler/*`), and UTF-8 / identifier handling in the runtime module loader. Each rule id (the `### ...` heading) is a permanent linkable address, reproduced verbatim from the rule artifacts.
...
Uncertainty: Reserved-without-use status is stated in the source comment; actual rejection behavior in surface grammar is defined in the %rules unit.
...
## Identifier and lifetime tokens
...
Divergence: width set includes Writ-fabric widths i24/u24/i56/u56 beyond Rust's {8,16,32,64,128}+size. Also: a leading `-` is part of the integer token itself (Rust treats `-` as a separate unary operator).
...
Uncertainty: The PEG token regex under-specifies raw strings vs the actual decoder; behavior corrected against the compiler (raw strings are Rust-like, hash-delimited forms supported), so this is NOT a divergence.

Related: `lex.litstr.raw-hash-count`

Source: `tools/peg_gen/grammars/logos.peg#L453-L455`, `src/compiler/mlir_gen_expr.cpp#L317-L336`
...
Source: `src/compiler/sema_impl.hpp#L4604-L4654`, `src/compiler/sema_impl.hpp#L4659-L4678`
...
Uncertainty: Rust permits leading/trailing and repeated underscores in some positions; exact conformance not confirmed from this unit.

Source: `src/compiler/sema_impl.hpp#L4705-L4720`
...
Source: `src/compiler/sema_impl.hpp#L4722-L4767`
...
Source: `src/compiler/sema_impl.hpp#L4769-L4808`
...
Source: `src/compiler/sema_impl.hpp#L4843-L4847`
...
Source: `src/compiler/mlir_gen_expr.cpp#L317-L336`
...
Source: `src/compiler/mlir_gen_expr.cpp#L338-L402`
...
Source: `src/compiler/sema_impl.hpp#L2791-L2827`
