...
Scope: the `stmt` domain — every statement form recognized at block-statement position, its parse-order/desugaring, and the sema/codegen rules governing bindings, assignment, control flow, and drop/divergence semantics around it. Source layers: grammar (`tools/peg_gen_cpp/grammars/logos.peg`), sema lowering (`src/compiler/sema_stmt.cpp`, `sema_impl.hpp`, `sema_collect.cpp`, `sema_render.cpp`), and codegen (`src/compiler/mlir_gen_stmt.cpp`, `mlir_gen_fn.cpp`). 181 rules across 33 groups.

> **Overlap flagged.** The grammar layer independently extracted near-duplicate rules for the same productions from different `.peg` slices: `stmt.assign.place-general` / `stmt.assign.place` both describe the general postfix-chain place-assignment production, and `stmt.assign.compound` / `stmt.assign.compound-place` both describe compound assignment over an arbitrary place. `stmt.assign.destructuring-into-places` (a broader, earlier-pass summary covering both destructuring assign and the general place-write fallback) also overlaps `stmt.assign.destructure` and `stmt.assign.place-general` in scope. None of these conflict on content — they are redundant descriptions of the same rule captured at different extraction granularities — but all ids are kept per extraction provenance; treat each group of overlapping ids as one production, not four.

## Statement Forms — Overview

Enumerates the statement forms a block may contain and their parse/dispatch order.

### `stmt.kinds.dispatch` — Statement forms

A statement is one of: repeat-group (`#(stmt)*`), nested-fn, labeled-loop, let-else, let, for, while, loop, return, break, continue, deref-write, if-expr, match, destructure-assign, assign, compound-assign, place-assign, unsafe-block, block, `expr ;` (expression statement), or a trailing `expr` (block-tail value, becomes the block's value with no `;`).

Source: `tools/peg_gen_cpp/grammars/logos.peg#L1839-L1866; tools/peg_gen_cpp/grammars/logos.peg#L1899-L1926`
...
`let` in its plain, destructuring, pattern-matching, and `let`/`let-else` fallible forms; binding-storage strategy per RHS shape; definite-assignment tracking for uninitialized bindings.

### `stmt.let.forms` — let bindings

`let` supports: tuple destructure `let (a,b) [: T] = e;`; `let ref x [: T] = e;` (sugar for `let x = &e;` — detected via IS_REF, sema wraps the lowered RHS in an addr-of); `let mut x [: T] [= e];` (mutable; `let mut x: T;` with no initializer declares an uninitialized typed mutable binding); `let x [: T] [= e];` (immutable; `let x: T;` likewise declares an uninitialized typed binding); and `let PAT = e;` (arbitrary irrefutable full pattern — struct/enum-variant/tuple-with-non-ident-subs — for forms not covered by the simpler IDENT-prefixed alternatives above; refutability is checked at sema time).

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2306-L2335`
...
Source: `src/compiler/sema_impl.hpp#L1897-L1906`
...
Source: `src/compiler/sema_impl.hpp#L1887-L1896`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1890-L1908`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1540-L1547`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1637-L1668`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1421-L1436`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L424-L446`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1597-L1635`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1549-L1561`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1437-L1439`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1672-L1701; src/compiler/mlir_gen_stmt.cpp#L1803-L1824`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1909-L1911`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1564-L1595`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1478-L1486`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1511-L1518`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1520-L1537`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L1440-L1454`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L310-L353`
...
Source: `src/compiler/sema_stmt.cpp#L1471-L1474; src/compiler/sema_stmt.cpp#L1567-L1572`
...
Source: `src/compiler/sema_stmt.cpp#L1499-L1512; src/compiler/sema_stmt.cpp#L1544-L1563`
...
Source: `src/compiler/sema_stmt.cpp#L1086-L1119; src/compiler/sema_stmt.cpp#L1137-L1237`
...
Source: `src/compiler/sema_stmt.cpp#L1407-L1418`
...
Source: `src/compiler/sema_stmt.cpp#L1322-L1406`
...
Source: `src/compiler/sema_stmt.cpp#L1419-L1466`
...
Source: `src/compiler/sema_stmt.cpp#L1513-L1517`

### `stmt.let-else.form` — let-else statement

`let PAT = expr else { block };` binds a refutable pattern; on match failure the else block runs (must diverge — the pattern's bindings are otherwise unavailable after the statement).

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2242-L2244; tools/peg_gen_cpp/grammars/logos.peg#L2303-L2304`
...
## Nested Function Items

A `fn` item written at statement position.

### `stmt.fn.nested-lifts-to-toplevel` — Nested fn lifts to a top-level free function

`fn name(params) [-> T] { body }` at statement position (NESTED_FN) is sema-lifted: the body becomes a top-level free function under a gensym'd name, and the local name is bound as a fn-pointer `let`.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L304`
...
Plain, compound, destructuring, and dereferencing assignment forms; place resolution, drop-before-replace ordering, and type/coercion rules.

### `stmt.assign.chain-field-write-path` — N-deep field-chain write path

An assignment through an N-deep chain of field accesses (e.g. `a.b.c = expr;`) is represented with a PATH sub-node carrying ITEMS=[str, ...] naming the successive field components of the chain.

**Uncertainty.** This slice shows only the field-slot comment for the chain-write PATH sub-node, not the statement's full grammar production.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L77`

### `stmt.assign.compound` — Compound assignment over any place

Compound-assignment operators: `+= -= *= /= %= &= |= ^= <<= >>=`. `place OP= expr ;` is valid for any postfix-chain place (`atom`); a bare variable takes a direct update path, any other place desugars to `place = place OP expr` (or an in-place-op call). A bare-deref place `*p OP= v` is handled separately (`stmt.assign.deref-write`), since `*p` alone is not an `atom`.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2384-L2387`
...
Source: `tools/peg_gen_cpp/grammars/logos.peg#L2317-L2323`

### `stmt.assign.deref-write` — Dereference write and compound-through-deref

`*p = v ;` writes value `v` through dereferenced place `p` (a `unary_expr`). `*p OP= v ;` is compound assignment through a bare dereference, defined to lower to `*p = *p OP v`.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2395-L2400`

### `stmt.assign.destructure` — Destructuring assignment into existing places

`(n1, n2, …) = expr;` and `[n1, n2, …] = expr;` destructure-assign a tuple/array RHS into existing bindings n1, n2, … (reusing `pat_binding_list`); `Name { f1, f2, … } = expr;` destructure-assigns a struct RHS by field (reusing `pat_field_list`). Per RFC 2909, this writes into EXISTING places — unlike `let (a,b) = e;`, it introduces no new bindings. Ordered before the general expression-statement rule so a parenthesised/bracketed LHS followed by `=` is recognized as destructuring assign; the parser backtracks to an ordinary expr-statement when no `=` follows.

**Related:** `stmt.let.forms`, `stmt.assign.simple`, `stmt.assign.place-general`, `stmt.assign.compound`

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2366-L2375`

### `stmt.assign.destructuring-into-places` — Destructuring assignment into existing places

`(a,b)=e`, `[a,b]=e`, and `S{a,b}=e` (DESTRUCTURE_ASSIGN) destructure into pre-existing places (not new bindings); sema desugars to a synthetic `let __da = rhs;` followed by one assignment per named place. A general postfix-chain lvalue not covered by the specialised write productions (`a[i][j]`, `(*p).0`, `m.f[i].g`, PLACE_ASSIGN) instead lowers directly to `deref_write(&mut <place>, rhs)`.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L312; tools/peg_gen_cpp/grammars/logos.peg#L314`
...
Source: `tools/peg_gen_cpp/grammars/logos.peg#L2295-L2304`

### `stmt.assign.place-general` — General place-assignment

`place = expr ;` where `place` is an arbitrary postfix-chain read-expression (`atom`: chained index/field/tuple-index/deref — e.g. `a[i][j] = v`, `(*p).0 = v`) is a valid assignment; the place's address is computed from the read-expr shape and a deref-write emitted. Tried after the specialized single/double-level write productions (which win on the shapes they cover) and after the bare-`IDENT` `stmt.assign.simple` rule.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2355-L2364`

### `stmt.assign.simple` — Simple variable assignment

`IDENT = expr ;` assigns to a plain (unqualified) variable place.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2352-L2353`
...
Source: `src/compiler/sema_stmt.cpp#L2567-L2575; src/compiler/sema_stmt.cpp#L2756-L2762`
...
Source: `src/compiler/sema_stmt.cpp#L2723-L2754`
...
Source: `src/compiler/sema_stmt.cpp#L2585-L2638`
...
Source: `src/compiler/sema_stmt.cpp#L2567-L2575`
...
Source: `src/compiler/sema_stmt.cpp#L2654-L2722`
...
Source: `src/compiler/sema_stmt.cpp#L2755-L2765`
...
Source: `src/compiler/sema_stmt.cpp#L2547-L2566`
...
Source: `src/compiler/sema_stmt.cpp#L2766-L2775`
...
Source: `src/compiler/sema_stmt.cpp#L2639-L2646`
...
Source: `src/compiler/sema_stmt.cpp#L2538-L2546`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L2015-L2039`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L2000-L2003; src/compiler/mlir_gen_stmt.cpp#L2049-L2051; src/compiler/mlir_gen_stmt.cpp#L2153`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L2040-L2048`
...
Source: `src/compiler/sema_stmt.cpp#L905-L920`
...
Source: `src/compiler/sema_stmt.cpp#L879-L897; src/compiler/sema_stmt.cpp#L924-L929; src/compiler/sema_stmt.cpp#L1038-L1045`
...
## Conditionals: `if` / `if let`

`if`, `if let`, and let-chain desugaring to `match`; branch divergence and definite-assignment merge.
...
The four loop forms (`while`, `while let`, `for`, `loop`), labeling, and loop-value (`break`-supplied) semantics.

### `stmt.while.forms` — while-loop statement forms

`while` has four forms, tried in this order: (1) `while <if_let_chain> { body }` — a multi-segment `&&`-joined let-chain (≥2 segments, first must be `let`), ordered first so it is not shadowed by the single-let form; (2) `while let P = e && guard { body }` — single let-binding plus a trailing guard (guard parsed as `expr_ns`, scrutinee `e` at `cmp_expr_ns`); (3) `while let P = e { body }`; (4) `while cond { body }` with `cond` a bare `expr_ns` (no-struct-lit). The let/let-chain forms loop while the pattern(s) match / all chain segments hold; form (4) is a plain conditional loop. The let-chain and while-let productions reuse the same `if_let_chain` parser as the `if`-expression let-chain form.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2340-L2347; tools/peg_gen_cpp/grammars/logos.peg#L2277-L2287`
...
`for x in iter { }` iterates over an iterable, binding the loop variable per element. If the loop variable is a bare identifier, the fast-path alternative binds it directly (tried first). If it is not a bare identifier (a tuple/struct/etc. pattern, e.g. `for (a, b) in v { }`), the parser falls through to the pattern alternative, and sema binds the full destructuring pattern against each element.
...
Source: `tools/peg_gen_cpp/grammars/logos.peg#L1938-L1946`

### `stmt.for.range` — For-range loop

`for i in lo..hi { }` iterates the exclusive integer range `[lo, hi)`; `for i in lo..=hi { }` iterates the inclusive range `[lo, hi]`.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L1868-L1877; tools/peg_gen_cpp/grammars/logos.peg#L1934-L1937`
...
Source: `tools/peg_gen_cpp/grammars/logos.peg#L1888-L1894; tools/peg_gen_cpp/grammars/logos.peg#L1948-L1954`
...
Source: `src/compiler/sema_stmt.cpp#L6887-L6889; src/compiler/sema_stmt.cpp#L6901-L6904`
...
Source: `src/compiler/sema_stmt.cpp#L6877-L6894`
...
Source: `src/compiler/sema_stmt.cpp#L6887-L6889; src/compiler/sema_stmt.cpp#L6895-L6897`
...
Source: `src/compiler/sema_stmt.cpp#L6871-L6872; src/compiler/sema_stmt.cpp#L6883-L6885; src/compiler/sema_stmt.cpp#L6891`
...
Label resolution, value consistency across `break` sites, and drop ordering on early loop exit.

### `stmt.break.forms` — break

`break;`, `break expr;`, `break 'label;`, and `break 'label expr;` are all valid; a value and/or a target label are each optional. A bare `break` (no label, no value) may be terminated by either `;` or `,`.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L1964-L1971`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L2549-L2567`
...
`continue;` and `continue 'label;` are valid; the target label is optional. A bare (unlabeled) `continue` may be terminated by either `;` or `,`.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L1973-L1976`
...
Source: `src/compiler/sema_stmt.cpp#L667-L670; src/compiler/sema_stmt.cpp#L726-L734`

## `match` Statement

Match-statement scrutinee parsing and arm grammar (full pattern-matching semantics are covered in the `pat` domain).

### `stmt.match.arm` — Match arm with optional guard

A match arm is `PAT [if GUARD] => BODY [,]`, where BODY is a block, an expression, or a statement, with an optional trailing comma. The optional `if GUARD` is a boolean guard expression gating the arm.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L1930-L1941; tools/peg_gen_cpp/grammars/logos.peg#L1990-L2001`
...
Source: `tools/peg_gen_cpp/grammars/logos.peg#L1978-L1988`

## `return`

Explicit and implicit (tail-position) `return`; operand rules for `!`/void/`impl Trait` returns; type-hint propagation; the `!`-fallback rule for divergent generic bodies.

### `stmt.return.form` — return statement

`return [expr] (;|,)` exits the enclosing function, optionally yielding a value; terminated by `;` or `,`.

**Uncertainty.** COMMA as an alternate terminator (besides SEMI) is unusual; likely to admit `return e` inside a macro/list-expansion context — not confirmed from this slice alone.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L2349-L2350`
...
Source: `src/compiler/sema_collect.cpp#L4736-L4741`
...
Source: `src/compiler/sema_stmt.cpp#L2784-L2817`
...
Source: `src/compiler/sema_stmt.cpp#L2854-L2858`
...
Source: `src/compiler/sema_stmt.cpp#L3002-L3015`
...
Source: `src/compiler/sema_stmt.cpp#L2859-L2868`
...
Source: `src/compiler/sema_stmt.cpp#L3019-L3024`
...
Source: `src/compiler/mlir_gen_fn.cpp#L523-L536`
...
Source: `src/compiler/mlir_gen_fn.cpp#L509-L525`
...
## Blocks, Scopes, and `unsafe`

Bare `{ }` scoping blocks, shadow-restore on block exit, statement-temporary lifetime, and the `unsafe` block context.

### `stmt.block.bare-scoping` — Bare { } block is a scoping statement

A bare `{ stmts... }` block appearing at statement position (BLOCK_STMT, BODY=block) introduces a nested scope without being an `if`/`loop`/`fn` body or any other control-flow construct.

Source: `tools/peg_gen_cpp/grammars/logos.peg#L266`
...
Source: `src/compiler/sema_stmt.cpp#L671; src/compiler/sema_stmt.cpp#L679-L696`
...
Source: `src/compiler/sema_stmt.cpp#L664; src/compiler/sema_stmt.cpp#L739-L754`
...
Source: `src/compiler/mlir_gen_stmt.cpp#L447-L470`
...
## Expression Statements and Tail Position

`expr;` vs. a block's unterminated tail expression, and when a tail expression counts as an implicit `return`.
...
The always-returns/never-falls-through predicate used for reachability, dead-code, and definite-assignment analysis.
