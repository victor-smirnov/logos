...
Scope: statement-level constructs of Logos - `let` bindings and patterns, assignment forms, control flow (`if`, `loop`, `while`, `for`, `break`, `continue`, `return`), blocks/scoping, divergence (`!`) analysis, and tail expressions. Source layers: grammar (`tools/peg_gen`), semantic lowering/analysis (`src/compiler/sema_stmt.cpp`, `sema_render`, `sema_impl.hpp`), and codegen (`src/compiler/mlir_gen_stmt.cpp`, `mlir_gen_fn`). Each `### ` heading is the rule's permanent, linkable id; never rename or merge ids.
...
**Divergence from Rust:** Rust-conformant (unreachable_code lint, here always a warning not deny-by-default)
...
**Divergence from Rust:** Implementation strategy of Rust drop-flag elaboration; observable equivalence to Rust drop semantics.
...
**Divergence from Rust:** Rust-conformant (items.union.pattern.safety / one-field)
...
**Divergence from Rust:** Accepted limitation: ELSE side effects duplicated per fall-through site.
...
**Divergence from Rust:** G154-5: comment notes non-matching literal `let 4 = x else {…}` was previously silently accepted; now correctly refutable.
...
**Divergence from Rust:** Accepting Never as an `if` condition (e.g. `if (return x){}`) is a Logos accommodation, tag G160-10.
