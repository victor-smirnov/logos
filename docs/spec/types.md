...
> Scope: the `type`, `layout`, and `coerce` rule domains of the Logos language specification — type formation and identity, physical value representation, and the coercion/cast/inference rules. Source layers: `grammar`, `sema`, `mono`, and `codegen`, extracted as machine-checkable rule artifacts under `tools/spec-extract/rules/` and assembled here. Each `### \`id\`` heading is the permanent, linkable address of a rule; a rule's domain is taken from its `domain` field and its group from the middle segment of its id.

## Contents

- Type System (`type`)
- Memory Layout & Representation (`layout`)
- Coercions, Casts & Inference (`coerce`)

## Type System (`type`)

Rules governing type formation, type identity, generic instantiation, traits-as-types, and well-formedness.

### Group: Alias (`type.alias`)
...
### Group: Assoc Ref (`type.assoc-ref`)
...
### Group: Canonicalize (`type.canonicalize`)
...
### Group: Closure Arg (`type.closure-arg`)
...
### Group: Closure Type (`type.closure-type`)
...
### Group: Generic Inst (`type.generic-inst`)
...
### Group: Impl Method (`type.impl-method`)
...
### Group: Impl Trait (`type.impl-trait`)
...
*Divergence from Rust:* Rust E0121 analog.
...
### Group: Inhabited (`type.inhabited`)
...
### Group: Method Recv (`type.method-recv`)
...
### Group: Pack Expand (`type.pack-expand`)
...
### Group: Primitive (`type.primitive`)
...
### Group: Primitive Method (`type.primitive-method`)
...
### Group: Recursion (`type.recursion`)
...
### Group: Recursive (`type.recursive`)
...
### Group: Self Describing (`type.self-describing`)
...
*Divergence from Rust:* B84 (compiler tag); Rust-conformant for *const/*mut variance
...
### Group: Traitobject (`type.traitobject`)
...
### Group: Uninhabited (`type.uninhabited`)
...
Rules governing the physical representation of values in memory: sizes, alignment, niches, fat pointers, and self-relative references.

### Group: Abi (`layout.abi`)
...
### Group: Aggregate (`layout.aggregate`)
...
### Group: Aggregate Member (`layout.aggregate-member`)
...
### Group: Closure (`layout.closure`)

### `layout.closure.fn-env-pair` — Closure value is a {fn_ptr, env_ptr} pair

> NOTE: 2 extractions in the corpus carry this id; all are preserved verbatim below. They are restatements of the same rule from different source files — reconcile into a single normative statement before publication.

**Extraction 1 — `codegen/mlir_gen_expr/gen_expr_kind-4.json`:**
...
**Extraction 2 — `codegen/mlir_gen_types/repr_storage_type.json`:**

A closure value is the pair {fn_ptr, env_ptr}, 16 bytes.

Source: `src/compiler/mlir_gen_types.cpp#L957-L960`
...
### Group: Customdst (`layout.customdst`)
...
**Extraction 1 — `codegen/mlir_gen/logos.json`:**
...
Source: `src/compiler/mlir_gen.cpp#L157-L162`

**Extraction 2 — `codegen/mlir_gen_types/logos.json`:**
...
### Group: Field Align (`layout.field-align`)
...
### Group: Field Index (`layout.field-index`)
...
### Group: Non Null (`layout.non-null`)
...
### Group: Repr Transparent (`layout.repr-transparent`)
...
### Group: Type Code (`layout.type-code`)
...
### Group: Zero Size (`layout.zero-size`)
...
### Group: Zone Mut (`layout.zone-mut`)
...
### Group: Zone Mut Ref (`layout.zone-mut-ref`)
...
Rules governing implicit coercions, explicit `as` casts, auto-ref/auto-deref, unsizing, and literal/inference defaulting.

### Group: Anyval (`coerce.anyval`)
...
*Divergence from Rust:* RFC 2005 match-ergonomics interaction; behaviorally Rust-conformant (note T2-26)
...
### Group: Compatible (`coerce.compatible`)
...
### Group: Method Arg (`coerce.method-arg`)
...
### Group: Method Recv (`coerce.method-recv`)

### `coerce.method-recv.auto-ref` — Auto-ref of receiver to match &self / &mut self

> NOTE: 2 extractions in the corpus carry this id; all are preserved verbatim below. They are restatements of the same rule from different source files — reconcile into a single normative statement before publication.

**Extraction 1 — `sema/sema_expr/lower_method_call.part2.json`:**
...
**Extraction 2 — `sema/sema_expr/lower_method_call.part4.json`:**
...
### Group: Numericlit (`coerce.numericlit`)
...
### Group: Reborrow (`coerce.reborrow`)
...
*Divergence from Rust:* Logos-specific miscompile fix (G149-2); Rust never has this hazard since &literal infers the pointee type.

Source: `src/compiler/sema_impl.hpp#L4365-L4400`

### Group: Rel Ptr (`coerce.rel-ptr`)
...
### Group: Struct Lit (`coerce.struct-lit`)
...
### Group: Taggedptr (`coerce.taggedptr`)
...
### Group: Variance (`coerce.variance`)
...
### Group: Writ Anyval (`coerce.writ-anyval`)
