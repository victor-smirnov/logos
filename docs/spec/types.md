...
⚠ **SCOPE — this is the GLUE MATCHER, not a conformance rule.** It says which method the destructor lookup will FIND, not which receiver an `impl` may write. For the stdlib lang-item `Drop`, whose declaration is `fn drop(self: &mut Self)` (`stdlib/lang/drop/drop.logos`), the by-value and `&T` shapes are refused from 2026-09-10 by `trait.impl.method-receiver-conforms`, so only `&mut T` reaches this matcher. The by-value shape stays reachable through a package that DECLARES its own `trait Drop { fn drop(self: Self); }`: such a declaration is merged into the lang item for glue purposes (`src/compiler/sema.cpp`, `is_drop_impl_`, keyed on the bare trait name `Drop`) while its own signature governs conformance. Measured 2026-09-10 on build `911811129fca26d5 43`, three programs differing in one line: a local `trait Drop` + by-value impl runs the destructor (rc 1), the same program with no local declaration (stdlib `Drop`) runs it (rc 1), and the same program with the trait renamed `Kill` does NOT (rc 0). ⚠ **RE-MEASURED 2026-09-10 AFTER THE CORPUS CONVERSION: THREE files rely on this, not 90.** 123 of the 126 that declared their own `trait Drop` were converted to the lang item in one commit with every assertion byte-identical, so the by-value shape is now reached by exactly `tests/logos/pass/bc_dropcall_user_trait_method.logos`, `tests/logos/pass/bc_dropcall_user_trait_path.logos` (which assert the RUST-CORRECT thing: a module-local `trait Drop` shadows the prelude item, so `s.drop()` calls it and is not E0040) and `tests/imported/pass/drop/drop-uninhabited-enum-b154.logos` (blocked by soundness-queue row `stdlib_drop_uninhabited_enum_byvalue_consumer`). The open corpus decision it named is no longer open and no longer a corpus decision: `Drop` is a LANG ITEM in Rust, selected by `#[lang = "drop"]` and never by spelling, no blessed divergence covers the trait's identity (both registries re-checked by CONSTRUCT), and the defect is now soundness-queue row `user_trait_named_drop_drives_glue` (tier 1, `run 1`) with a one-variable rename control.

**Source:** `src/compiler/sema.cpp`, `SemaChecker::drop_fn_for` and its `is_drop_impl_` lambda (cited by SYMBOL: the previous `#L2742-L2780` had drifted onto the layout-recording sweep, which is not this rule).
