# Logos Language Specification — Types, Layout, and Coercions

Scope: structural type identity, layout/representation (ABI, enums, DSTs, refs, vtables), and coercion/cast rules. Source layers: grammar, sema (resolve/subtype/variance/trait-engine), mono (clone/subst/CTFE), and codegen (MLIR type & expr lowering). Rules are organized by `domain`, then by the middle segment of each rule id; every id is reproduced verbatim and is a permanent linkable address.

---

# Type System (`domain: type`)
