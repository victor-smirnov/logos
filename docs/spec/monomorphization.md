...
Scope: the monomorphization (`mono`) and compile-time constant-evaluation (`const`) layers — how generic items are instantiated, specialized, mangled, and laid out, and which expression forms are evaluable at compile time. Rules are extracted from the compiler source (`src/compiler/`); each `###` id is the permanent linkable address.

## CTFE — Accepted Expression Forms (`const.ctfe`)
...
## Literal Constants (annotations) (`const.literal`)
...
**Divergence from Rust:** Logos has additional integer widths I24/U24/I56/U56 beyond Rust's fixed set.
...
## Integer-Literal Constants (`const.intlit`)
...
## Constant Binary Operators (`const.binop`)
...
**Divergence from Rust:** Rust folds in the inferred type; Logos folds in i64 and errors on i64 overflow, deferring per-type fit to the coercion site.
...
**Divergence from Rust:** usize/isize fixed at 64-bit (target-specific).
...
## Constant Unary Operators (`const.unary`)
...
**Divergence from Rust:** A2 — const-evaluable bare fn calls are not supported; the escape hatch is explicit `metacall fn(...)` (Rust would allow `const fn`).
...
**Divergence from Rust:** B-ca-02: diagnostic surfaced at sema rather than at MLIR-verifier time.
...
**Divergence from Rust:** A: Writ-static literal coercion is a Logos addition.

Source: `src/compiler/sema_decl.cpp#L1549-L1554`

## Const in Assignment (`const.assign`)
...
## Const Enum Discriminants (`const.enum`)
...
**Divergence from Rust:** Cross-enum discriminant reference `OtherEnum::Variant` as a discriminant value has no Rust analog.
...
**Divergence from Rust:** A6 — Writ is a Logos-only feature; no Rust analogue.
...
**Divergence from Rust:** A6 — Writ-specific.
...
## Instantiation Gating (`mono.gate`)
...
## Reachability (`mono.reachability`)
...
## Incremental Mono (`mono.incremental`)
...
## `instantiate` Declarations (`mono.instantiate`)
...
**Divergence from Rust:** No Rust equivalent; analog of C++ `template class Foo<int>;`.
...
## Impl-Method Instantiation (`mono.impl-method`)
...
## Dispatch Selection (`mono.dispatch`)
...
**Divergence from Rust:** Logos-specific monomorphization fixup
...
**Divergence from Rust:** Logos-specific name-mangling scheme; see G156-1 baghunt for two-impl collision
...
**Divergence from Rust:** B8 (resolved — Rust-conformant)
...
**Divergence from Rust:** Logos-specific compile-time Writ-config-driven type projection; no Rust analogue.
...
**Divergence from Rust:** RFC-2229 disjoint-closure-capture metadata preserved through mono
...
**Divergence from Rust:** A1/A2 related: const-generics are real, distinct from const-eval
...
**Divergence from Rust:** Logos-specific (T9-tr-02) plus variadic-pack handling
...
**Divergence from Rust:** Logos-specific blanket-impl monomorphization (G159-1)
...
**Divergence from Rust:** Logos-specific Writ/RefRepr self-describing-DST contract; no Rust analogue.
...
**Divergence from Rust:** Logos reflection/metaprogramming extension; no Rust equivalent.
...
**Divergence from Rust:** Logos-specific (SL-sl-08 / CP-cm-08b)
...
**Divergence from Rust:** A6 (Logos addition — variadics)
...
**Divergence from Rust:** Variadic tuples are a Logos addition not present in Rust.

Source: `src/compiler/sema.cpp#L4639-L4659`

## Const-Argument Specialization (`mono.const-arg`)
...
**Divergence from Rust:** Logos const-generic-like specialization driven by const-eval reachability; see explicit-metacall comptime model.

Source: `src/compiler/mono_impl.hpp#L368-L401`

## Array-Literal Lengths (`mono.arr-lit`)
...
## Struct Monomorphization (`mono.struct`)
...
## Struct Specializations (`mono.struct-spec`)
...
## Const Enum Discriminants (`mono.enum`)
...
## Operator Methods (`mono.operator`)
...
**Divergence from Rust:** T2-15: the logical-not overload method is named `not` (not Rust `Not::not` symbol but conformant in spirit); note earlier name was `not_`.

Source: `src/compiler/mono_clone.cpp#L1002-L1024`

## Intrinsics (`mono.intrinsic`)
...
**Divergence from Rust:** Logos reflection extension.
...
**Divergence from Rust:** Logos reflection extension (compile-time trait-satisfaction predicate).
...
**Divergence from Rust:** Logos reflection extension (Type-method form).
...
**Divergence from Rust:** Logos compile-time-static (Writ) extension.
...
**Divergence from Rust:** A6 (mechanism specific to Logos compile-time reflection)
...
## Reflection (reflect_of) (`mono.reflect-of`)
...
## Cross-Module Coexistence (`mono.coexist`)
...
A trait object that is OWNING (Box<dyn T>) mangles as 'owndyn_<trait-name>' plus '__<arg>' per type-arg, while a borrowed &dyn T keeps the plain type-string mangling. This keeps Vec<Box<dyn T>> and Vec<&dyn T> as DISTINCT specs so the owning bit is not dropped (which would skip element drop and leak).

> Note: a second source layer states the same rule with complementary detail: An owning TraitObject mangles to owndyn_<trait>$<args...> while a borrowed &dyn T keeps the type_str form, so e.g. Vec<Box<dyn T>> and Vec<&dyn T> get distinct generic spec names (preserving owning-ness / drop). FnItem and FnPtr mangle identically (FnItem ZST auto-coerces to FnPtr).

**Divergence from Rust:** Internal mangling distinction with no Rust analog; reflects owning-dyn vs borrowed-dyn repr split.
...
**Divergence from Rust:** A9 — Logos coexistence of same-named types across modules; no Rust crate-disjointness analog.

Source: `src/compiler/mono_impl.hpp#L772-L808`

## Associated Consts (`mono.assoc`)
...
**Divergence from Rust:** G156-1: addresses two same-trait impls at distinct type-args; tracked as a known narrow area.
