# Monomorphization and Const Evaluation

Scope: monomorphization (generic instantiation, specialization, name mangling, dispatch lowering, drop glue, reflection intrinsics) and compile-time const evaluation (CTFE, const/static definitions, literal folding). Rules are extracted from the compiler source layers `src/compiler/` (mono, subst, clone, ctfe), sema, and MLIR codegen; each rule's `id` is its permanent linkable address.
...
## Reachability (entry-point pruning)
