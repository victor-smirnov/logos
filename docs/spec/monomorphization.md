...
Scope: generic instantiation (mono), const-argument specialization, and compile-time constant evaluation (const). Rules extracted from the compiler source layers (`src/compiler/`) and the grammar layer (`tools/peg_gen/grammars/`); each rule's `id` is its permanent linkable address.
...
## Integer-literal assignment fit
...
## Integer-literal let-binding fit
...
### `mono.subst.array-const-size` — Array size const-generic resolution

For [T; N] where N is a symbolic const param: if the substitution binds N to a const value, the array size becomes that literal and the symbolic size is dropped; if N binds to another ConstVar, the size stays symbolic under the new name; the element type is substituted recursively.

**Source:** `src/compiler/mono_subst.cpp#L33-L67`

### `mono.subst.array-sizeof-pack` — [T; sizeof...(P)] resolves to pack length

An array size written as sizeof...(P) (lowered to the symbolic form '__sizeof_pack:P') is resolved at monomorphization to the literal length of the concrete expansion of pack P.

```logos
fn f<const ...P: T>() -> [u8; sizeof...(P)] { ... }
```

**Source:** `src/compiler/mono_subst.cpp#L38-L48`
...
### `mono.subst.assoc-type-blanket-fallback` — Associated type resolution via satisfied blanket impl

If no direct impl supplies the associated type, a blanket impl `impl<T: Bound> Trait for T` is used when the concrete base satisfies Bound and all extra bounds (each checked recursively for an impl); the blanket's associated-type binding is substituted with the blanket type-var bound to the concrete base.
...
An associated type Trait::Base::Assoc is resolved by first substituting Base; if Base resolves to a struct, enum, or scalar primitive, the concrete impl's associated type binding is looked up by key 'Trait::ConcreteBase::Assoc' and the result is recursively collapsed. Scalar primitives use their canonical type name as the base, enabling resolution through the Primitive→Container blanket chain.

**Source:** `src/compiler/mono_subst.cpp#L366-L402`
...
A CfgSlotType <type:CFG.path> resolves by substituting CFG to a WStaticLit, looking up the bound Writ literal value, and walking the encoded path (F=string-keyed map field, I=integer-keyed map field, A=array index). If the reached Writ value is a Type, it yields the corresponding scalar primitive (u8..f64,bool) or the named struct/enum from the program; otherwise the projection stays deferred (unchanged). The path is a 0x1F-joined sequence of (kind-byte + payload) steps.
...
### `mono.subst.dst-pointee-to-dstref` — Pointer to a custom-DST struct becomes a fat DstRef

A &/&mut/*const/*mut to a struct that is a custom-DST canonicalizes to a fat reference (Kind::DstRef) preserving struct name, package, mutability and type args. A struct is a custom-DST if its template is flagged is_dst, or if a generic instantiation binds the struct's trailing type-param (whose template field is exactly that bare param) to an unsized type-arg: UnsizedDyn, UnsizedSlice, or a bare `dyn` TraitObject (owning kind Borrow). A sized fat-handle arg (Box/Rc/Arc<dyn>, &[T]) does NOT make the instance a DST.
...
### `mono.subst.fn-closure-signature` — Substitution descends into fn/closure signatures

For FnItem, FnPtr and Closure types, each parameter type and the return type are substituted recursively; e.g. fn(T, U) -> V monomorphizes its argument and result types.
...
### `mono.subst.generic-struct-instantiation` — Generic struct/enum instantiation recorded for monomorphization

Substituting a struct/zoned-struct or enum recurses into each type-arg; each fully-substituted concrete struct/enum instantiation is recorded as a needed monomorphization. A non-generic-args enum still records its need; enum substitution preserves the enum's package name.
...
### `mono.subst.identity-preservation` — Substitution is identity when nothing changes

For every aggregate type kind, substitution returns the original type unchanged when no contained type-arg/element/pointee/size changed, allocating a new type only when a sub-component was rewritten.

**Source:** `src/compiler/mono_subst.cpp#L62-L62`, `src/compiler/mono_subst.cpp#L185-L205`, `src/compiler/mono_subst.cpp#L230-L297`, `src/compiler/mono_subst.cpp#L536-L538`
...
### `mono.subst.self-describing-dst-thin-ptr` — Raw pointer to #[self_describing] DST stays thin

A raw *const/*mut to a #[self_describing] DST struct (whose tail length is recoverable from an in-band prefix field) stays a thin 8-byte pointer and is NOT canonicalized to a fat DstRef; &/&mut to such a struct keep the fat representation.

**Source:** `src/compiler/mono_subst.cpp#L163-L174`
...
### `mono.subst.tuple-pack-expansion` — Variadic tuple pack splicing

A single-element tuple Tuple<[TypeVar(A)]> whose variable A maps (via the substitution map or the active pack set) to a concrete tuple is replaced in-place by the spliced, recursively-substituted elements of that tuple; e.g. &(A...) becomes &(T1, T2, ...).
...
During monomorphization, a type whose kind is TypeVar or ConstVar is replaced by the binding for its name in the active substitution map; a name with no binding is left unchanged.
...
### `mono.subst.unsized-dyn-to-traitobject` — &/* over ?Sized binding to dyn canonicalizes to TraitObject

When a ?Sized type parameter behind a pointer/reference is substituted with an unsized dyn type, the pointer type canonicalizes to a fat trait-object pointer (Kind::TraitObject) carrying the trait name and its type args.
...
When a ?Sized type parameter behind &T, &mut T, *const T or *mut T is substituted with an unsized slice type, the pointer type canonicalizes to the fat-pointer slice type &[U] (Kind::Slice).
...
## Struct specialization selection
...
## Demand-driven method instantiation
...
### `mono.mangle.concrete-struct-name` — Concrete struct symbol name = base + module suffix + generic args

A Struct/ZonedStruct type's symbol name is its struct_name, plus a module-qualification suffix `$M<module_id>` for non-stdlib packages with a known owning module (stdlib `logos.*` and empty-map compiles get no suffix), plus, when type_args are present, `$G<n>` followed by `$`-joined per-arg mangled type names.

**Source:** `src/compiler/sema.cpp#L1404-L1444`

### `mono.mangle.fn-signature-key` — Function signature key = base + per-param mangled types

A function's signature key is `<base>__<mangled param type>...`, with `__void` appended when there are no params and `__vararg` when variadic. For a fn-ptr-impl method (base begins `$fnptr$`), fn-pointer parameters are erased to their arity-only `$fnptr$<n>` form so the symbol is stable across the impl's type-vars.

**Source:** `src/compiler/sema.cpp#L1522-L1541`
...
### `mono.mangle.owning-vs-borrowed-dyn` — Owning Box<dyn T> and borrowed &dyn T mangle distinctly

An owning trait object mangles to `owndyn_<trait>` plus its type-args, distinct from a borrowed &dyn T (which keeps the type_str mangling), so e.g. Vec<Box<dyn T>> and Vec<&dyn T> are distinct generic struct specs (preserving owning-ness for drop).

**Source:** `src/compiler/sema.cpp#L1500-L1512`

### `mono.mangle.type-args-recursive` — Per-type mangling for generic-symbol identity

Each type mangles to an identity-stable string: Ptr→pmut_/pcst_+pointee; Ref→ref_; MutRef→refmut_; Array→arr<N>_elem; Tuple→tup$<n>$..; Slice→slice_/UnsizedSlice→uslice_; UnsizedDyn→udyn_<trait>; DstRef→dst[mut]ref_<name>; AssocType→base::name; WStaticLit→hs_<hex64 of const value> (identical bytes ⇒ same suffix); owning TraitObject→owndyn_<trait>+args distinct from borrowed &dyn; struct/enum module-qualified. Two structurally distinct generic instantiations get distinct symbols.
