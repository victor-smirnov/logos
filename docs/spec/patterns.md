...
Scope: pattern syntax, binding modes, refutability, exhaustiveness, match/let/for destructuring, and Writ-substrate patterns. Extracted from the grammar (`logos.peg`), sema lowering, borrow-check, and MLIR codegen layers.

This section is generated from 260 extracted `pat`-domain rules. Each `###` heading is the rule's permanent linkable id.
...
### `pat.bytes.reserved-lexer-pending` — Byte-string pattern reserved but not lexer-wired

A byte-string literal in match-arm pattern position (PAT_BYTES) is a reserved node code; per the source comment, lexer support for producing it is still pending.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L285*
...
**Divergence from Rust:** Rust permits byte-string patterns against `&[u8]`/`&[u8; N]`; Logos requires fixed `[u8; N]` and rejects dynamic slices.
...
### `pat.char.literal-and-range` — Char-literal and char-range patterns decode to u32 scalar

A char literal in match-arm position, `'A'` (PAT_CHAR, VALUE=original token text), and a char range, `'a' ..= 'z'` (PAT_CHAR_RANGE, LHS/RHS=original endpoint token text), are both sema-decoded to `u32` Unicode scalar values for matching.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L297-L298*
...
**Divergence from Rust:** Rust deprecated-but-still-accepts float patterns; Logos hard-rejects them.

*Source: src/compiler/sema_stmt.cpp#L4286-L4294*

### `pat.float.rejected-at-sema` — Float-literal pattern parses but is rejected at sema

A float literal in match-arm pattern position (PAT_FLOAT) is grammatically accepted but rejected by sema (float equality patterns are not permitted).

*Source: tools/peg_gen_cpp/grammars/logos.peg#L284*
...
`true` and `false` match the corresponding boolean values.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2208-L2211; tools/peg_gen_cpp/grammars/logos.peg#L2268-L2271*

### `pat.lit.bytes-rejected` — Byte-string pattern rejected

A byte-string-literal pattern parses (PAT_BYTES) but sema currently rejects it, pending &[u8]-equality-matching codegen.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2200-L2203; tools/peg_gen_cpp/grammars/logos.peg#L2260-L2263*
...
**Divergence from Rust:** Rust historically deprecated float patterns; Logos rejects them outright rather than accept-with-lint.
...
*Source: tools/peg_gen_cpp/grammars/logos.peg#L2186-L2189; tools/peg_gen_cpp/grammars/logos.peg#L2246-L2249*
...
`()` is the unit pattern, matching the unit value (PAT_UNIT).

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2087-L2091; tools/peg_gen_cpp/grammars/logos.peg#L2150-L2151*
...
A char literal pattern `'a'` matches a char value; `'a'..='z'` matches an inclusive char range.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2190-L2194; tools/peg_gen_cpp/grammars/logos.peg#L2250-L2254*
...
*Source: src/compiler/mlir_gen_expr.cpp#L4394-L4404; src/compiler/mlir_gen_expr.cpp#L4458-L4482; src/compiler/mlir_gen_expr.cpp#L4679-L4699; src/compiler/mlir_gen_stmt.cpp#L4382-L4402*
...
*Source: tools/peg_gen_cpp/grammars/logos.peg#L2148-L2185; tools/peg_gen_cpp/grammars/logos.peg#L2208-L2245*
...
**Divergence from Rust:** Logos char ranges are handled separately (PAT_CHAR_RANGE); PAT_RANGE is integer-only.
...
## Identifier, wildcard, and binding patterns
...
`name @ subpat` binds `name` to the value matched by `subpat`. `ref name @ subpat` binds by reference; ordered before the bare `ref name` alternative so the trailing `@ subpat` is not left unparsed.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2043-L2052; tools/peg_gen_cpp/grammars/logos.peg#L2106-L2112*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4445-L4534*
...
*Source: src/compiler/mlir_gen_expr.cpp#L4191-L4210; src/compiler/mlir_gen_expr.cpp#L4673-L4720*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4204-L4217*
...
**Divergence from Rust:** RFC 2005 default binding modes (Rust-conformant intent).
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3541-L3561*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3520-L3570*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4252-L4260*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3720-L3747*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4219-L4245*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4246-L4251*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4080-L4120*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4121-L4199; src/compiler/mlir_gen_stmt.cpp#L4164-L4174; src/compiler/mlir_gen_stmt.cpp#L4180-L4198*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4064-L4075*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4033-L4063*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3993-L4032; src/compiler/mlir_gen_stmt.cpp#L3998-L4024*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3670-L3719*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3572-L3597*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3857-L3862; src/compiler/mlir_gen_stmt.cpp#L3944-L3957*
...
*Source: src/compiler/borrow_check.cpp#L1487-L1512*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3598-L3633*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4261-L4292; src/compiler/mlir_gen_stmt.cpp#L4266-L4282*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3235-L3238; src/compiler/mlir_gen_stmt.cpp#L3257-L3259*
...
The mode is recorded AS A PATTERN NODE, not only as a type: at the tuple, struct and slice doors a plain named element/field is emitted as the same `PatRefBind` a written `ref`/`ref mut` emits, carrying binding type `&T`/`&mut T`. A binding type alone is invisible to codegen at every door but the variant one, so the node is what makes the write land (measured 2026-09-06j).

**Divergence from Rust:** ARRAY- and SLICE-typed elements are still exempt — they bind by value and a write through them is refused. The codegen ref-bind path binds an aggregate place by address and carries its shape (`var_struct_` for a struct, `var_tuple_` for a tuple) and has no array shape to carry, so minting there would replace the refusal with a silent wrong answer. Soundness queue row `arrayelem_default_ref_mode_not_minted`.
...
The rule holds for ALL THREE modifiers at EVERY door. `SemaChecker::modifier_under_ref_scrutinee` mints the sentence once and it is asked at four sites, because the three container doors funnel every sub-pattern through one lambda: the VARIANT-PAYLOAD loop in `build_pattern_variant_data`; the two SHORTHAND spellings that are consumed at their own door and never reach the lambda (`S { ref x }` in the PAT_STRUCT arm, `(ref a, b)` in `push_ref_elem`); `dbm_sub_ty`'s LEAF arm, which answers the struct-field sub-pattern `S { x: ref v }`, the struct-pattern tuple index `TS { 0: ref a }`, the tuple-element sub and the slice/array element `[ref a]` — for a written `mut` as well as a written `ref`, since a `mut` leaf reaches no other site (`dbm_named_bind` rejects `IS_MUT`); and the nested `@`-BINDING arm of the payload loop, where a written `ref` is DISCARDED (`binding_is_ref` is pushed false for `ref n @ sub`) and so is invisible everywhere downstream, while the `mut` spelling of the same shape is carried and already refused.

Two doors do NOT ask it, and the reason is that the mode never shifts there in the first place — a modifier cannot violate a mode the door does not have (`pat.binding.default-mode-carried-into-subpatterns`' recorded divergences): a nested STRUCT/TUPLE sub of a variant payload (`match &o { Some(P { ref x, y }) }`), and the TUPLE-STRUCT call spelling (`match &t { TS(ref a, b) }`, soundness-queue row `tuplestruct_door_default_ref_mode_not_carried`). At both, `match &… { … mut a … }` is admitted too, so this is one missing carry and not a `ref`-specific gap.

⚠ THE PREDICATE IS "THE PROGRAMMER WROTE A MODIFIER", NOT "the binding is by-reference". At the variant-payload door `binding_is_ref` has two producers — a written `ref` (`la::IS_REF`) and the compiler's own refutable-sub synthesis (`synth_wants_ref`) — and asking the coarse one refuses `match &e { Outer::W(Option::Some(a)) }`, which is legal and writes no modifier anywhere, blaming a synthesized name (`__refut_W_0_0`). `binding_from_wild` is the separating fact.

*Source: decision 2026-09-05; `SemaChecker::modifier_under_ref_scrutinee` (src/compiler/sema_stmt.cpp), asked in `build_pattern_variant_data`'s `bind_ref_modes` loop and its nested `@`-binding arm, in `build_pattern_impl`'s `push_ref_elem` and struct-field-shorthand arm, and in `dbm_sub_ty`'s leaf arm — and so, via `pat.binding.default-mode-carried-into-subpatterns`, at every nested position those doors reach.*
...
A LEAF BINDER is the one sub-pattern kind that is NOT given the reference: `S { x: ref mut rx }`, `[ref a]` and a plain named element already consume the mode where they are written, and handing them a reference too binds `&mut &mut T`. The carry applies to a sub-pattern that RE-DERIVES the mode from the type it is handed, never to one that answers it directly. ⚠ The three fixtures that measurement named — `tests/spec/pass/pat_4.logos`, `tests/imported/pass/binding/match-ref-binding-mut.logos`, `tests/logos/pass/match_struct_move_field_drop.logos` — were all writing a modifier under a by-reference mode, which `pat.binding.modifier-requires-move-mode` makes an ERROR; they are `match *p` / `match *x` since 2026-09-09f, and a leaf still reaching this arm is either such a modifier binder (refused there) or a plain binder `mint_dbm_ref` declined.
...
An explicit `ref v` (resp. `ref mut v`) sub-pattern in a variant payload binds by reference: the binding type is wrapped in `&` (resp. `&mut`), binding the payload slot's address rather than a load. Explicit ref overrides default-binding-mode wrapping — but only where writing it is legal at all: under a by-reference default binding mode `pat.binding.modifier-requires-move-mode` refuses it, so this rule describes the `move`-mode case (a by-value scrutinee, a `*`-dereferenced one, or one below an explicit `&`-pattern).
...
*Source: src/compiler/mlir_gen_expr.cpp#L4345; src/compiler/mlir_gen_expr.cpp#L4359*
...
*Source: src/compiler/sema_impl.hpp#L2899-L2903*
...
*Source: tools/peg_gen_cpp/grammars/logos.peg#L2237-L2238; tools/peg_gen_cpp/grammars/logos.peg#L2297-L2298*
...
## Reference and binding-mode patterns
...
*Source: src/compiler/mlir_gen_stmt.cpp#L19-L49*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L144-L161*
...
`ref x` binds the matched place by shared reference; `ref mut x` by mutable reference; `mut x` introduces a fresh re-bindable mutable local from the scrutinee (by value).

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2053-L2063; tools/peg_gen_cpp/grammars/logos.peg#L2113-L2123*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4323-L4338*

### `pat.ref.reference-pattern` — Reference patterns

`&pat` / `&mut pat` match through one reference layer (PAT_REF). `&&pat` / `&&mut pat` (lexed as a single fused `&&`/AND token) match through two reference layers, built as PAT_REF(PAT_REF(pat)).

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2064-L2085; tools/peg_gen_cpp/grammars/logos.peg#L2124-L2145*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4671-L4703*
...
*Source: src/compiler/mlir_gen_expr.cpp#L4187-L4190; src/compiler/mlir_gen_expr.cpp#L4306-L4312; src/compiler/mlir_gen_expr.cpp#L4646-L4672*

## Tuple and tuple-struct patterns

### `pat.tuple-bind.let` — Let-binding tuple pattern (binder list)
...
*Source: tools/peg_gen_cpp/grammars/logos.peg#L1943-L1960; tools/peg_gen_cpp/grammars/logos.peg#L2003-L2020*
...
### `pat.tuple.default-ref-move-only` — Default-ref binding at a tuple element under a reference scrutinee

When the tuple scrutinee is reached through a `&`/`&mut`, EVERY plain named element binds by reference (`&et`/`&mut et`), Copy elements included; the element is emitted as a `PatRefBind`. Array/slice-typed elements are the one exemption (`pat.binding.default-by-ref-mode`).

*Source: src/compiler/sema_stmt.cpp `SemaChecker::build_pattern_impl`, `mint_dbm_ref` at the PAT_TUPLE element loop*

### `pat.struct.default-ref-move-only` — Default-ref binding at a struct field under a reference scrutinee

When the struct scrutinee is reached through a `&`/`&mut`, EVERY plain field binding — shorthand `{ x }` and rename `{ x: nx }` alike — binds by reference, Copy fields included; the field's sub-pattern is emitted as a `PatRefBind`. `match &mut s { S { x, .. } => *x = … }` over a Copy `x` compiles and the write lands in `s`. Array/slice-typed fields are the one exemption.

*Source: src/compiler/sema_stmt.cpp `SemaChecker::build_pattern_impl`, `mint_dbm_ref` at the PAT_STRUCT field loop*

### `pat.slice.default-ref-move-only` — Default-ref binding at a slice element under a reference scrutinee

When the array/slice scrutinee is reached through a `&`/`&mut`, every prefix and suffix element binding binds by reference, Copy elements included. A named rest (`xs @ ..`) keeps binding the sub-slice `&[T]`.

*Source: src/compiler/sema_stmt.cpp `SemaChecker::build_pattern_impl`, `mint_dbm_ref` at the PAT_SLICE element loop*

### `pat.tuple.elem-rest` — Tuple-pattern rest element

A tuple-pattern element is `..` (rest — later lowered to `_` wildcard skips, preserving fixed tuple arity) or an or-pattern of sub-patterns.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2015-L2026; tools/peg_gen_cpp/grammars/logos.peg#L2075-L2086*
...
*Source: src/compiler/mlir_gen_expr.cpp#L3904-L3926; src/compiler/mlir_gen_expr.cpp#L4407-L4433*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3240-L3248*
...
`(e1, e2, ...)` is a tuple pattern (>=2 elements), admitting a `..` rest at any single element position; `(x,)` (trailing comma) is a 1-element tuple pattern, distinguished from a parenthesised pattern `(x)` by the trailing comma.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2228-L2236; tools/peg_gen_cpp/grammars/logos.peg#L2288-L2296*
...
**Divergence from Rust:** Logos addition: tuple-arm codegen lacks a native str_eq dispatch, so string elements are desugared to guards.
...
*Source: src/compiler/mlir_gen_stmt.cpp#L252-L284*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L212-L221; src/compiler/mlir_gen_stmt.cpp#L262-L272*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L188-L211; src/compiler/mlir_gen_stmt.cpp#L222-L238*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L117-L142*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L270-L276; src/compiler/mlir_gen_stmt.cpp#L286-L294*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L163-L184*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L95-L96*
...
A struct-pattern field is `..` (rest), `N: subpat` (tuple-struct field by positional index N, resolved positionally), `name: subpat`, `ref name` / `ref mut name` (bind the field by reference), or a bare `name` shorthand.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L1980-L1999; tools/peg_gen_cpp/grammars/logos.peg#L2040-L2062*
...
*Source: src/compiler/mlir_gen_expr.cpp#L4071-L4160; src/compiler/mlir_gen_expr.cpp#L4378-L4381; src/compiler/mlir_gen_expr.cpp#L4434-L4457*
...
*Source: tools/peg_gen_cpp/grammars/logos.peg#L2138-L2142; tools/peg_gen_cpp/grammars/logos.peg#L2198-L2202*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4704-L4726*
...
*Source: tools/peg_gen_cpp/grammars/logos.peg#L2130-L2131; tools/peg_gen_cpp/grammars/logos.peg#L2190-L2191*
...
`E::V { x, y: pat, .. }` / `E::V {}` match a struct-shaped enum variant; field names resolve to variant payload indices via payload_field_names.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2123-L2129; tools/peg_gen_cpp/grammars/logos.peg#L2183-L2189*
...
`E::V(args)` matches an enum tuple-variant payload; payload args are full nested patterns (rest `..` and or-patterns admitted). `Foo(a, b)` (no `::`) matches a tuple-struct pattern; sema dispatches to tuple-struct destructure vs. enum-variant destructure based on how the name resolves.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2121-L2122; tools/peg_gen_cpp/grammars/logos.peg#L2132-L2137; tools/peg_gen_cpp/grammars/logos.peg#L2181-L2182; tools/peg_gen_cpp/grammars/logos.peg#L2192-L2197*
...
**Divergence from Rust:** Rust-conformant (rustc issue-41888 `Err(err)` over `Result<(),()>`)
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4612-L4670*
...
*Source: src/compiler/mlir_gen_expr.cpp#L3979-L4068; src/compiler/mlir_gen_expr.cpp#L4566-L4645*
...
*Source: tools/peg_gen_cpp/grammars/logos.peg#L2004-L2013; tools/peg_gen_cpp/grammars/logos.peg#L2064-L2073*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4551-L4611*
...
*Source: src/compiler/mlir_gen_expr.cpp#L3927-L3978; src/compiler/mlir_gen_expr.cpp#L4513-L4565*
...
`[elems]` / `[]` match a slice/array by element sub-patterns.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2143-L2147; tools/peg_gen_cpp/grammars/logos.peg#L2203-L2207*

## Or-patterns and grouping

### `pat.group.paren` — Parenthesised / grouped pattern

`(P)` is exactly P. `(P | Q | ...)` is a grouped or-pattern, inlined into the same PAT_OR shape as a bare or-pattern at that position (composes with `@`-bindings, e.g. `n @ (1 | 2 | 3)`). `(..)` matches any tuple irrefutably, binding nothing.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L2212-L2227; tools/peg_gen_cpp/grammars/logos.peg#L2272-L2287*
...
*Source: src/compiler/mlir_gen_expr.cpp#L4211-L4224; src/compiler/mlir_gen_expr.cpp#L4483-L4512*

### `pat.or.alternatives` — Or-pattern

A top-level pattern is one or more `pat_single` alternatives separated by `|` (optional leading `|`), combined into a single or-pattern. A variant-payload argument may itself be an or-pattern `Some(A | B)`; a single alternative unwraps transparently.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L1962-L1978; tools/peg_gen_cpp/grammars/logos.peg#L2022-L2038*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4405-L4444; src/compiler/mlir_gen_stmt.cpp#L4420-L4428*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3263-L3267*
...
*Source: src/compiler/sema_impl.hpp#L4211-L4216*
...
*Source: src/compiler/sema_stmt.cpp#L8765-L8809; src/compiler/sema_stmt.cpp#L8810-L8842*
...
*Source: src/compiler/sema_impl.hpp#L2317-L2331*
...
**Related:** `pat.or.alt-binding-consistency`

*Source: src/compiler/sema_stmt.cpp#L8272-L8276; src/compiler/sema_stmt.cpp#L8304-L8307; src/compiler/sema_stmt.cpp#L8384-L8388; src/compiler/sema_stmt.cpp#L8788-L8791; src/compiler/sema_stmt.cpp#L8837-L8841; src/compiler/sema_stmt.cpp#L8904-L8912*
...
*Source: src/compiler/sema_stmt.cpp#L9064-L9103*
...
*Source: src/compiler/sema_impl.hpp#L4241-L4246*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3783-L3823; src/compiler/mlir_gen_stmt.cpp#L3824-L3836; src/compiler/mlir_gen_stmt.cpp#L3882-L3890*
...
*Source: src/compiler/sema_impl.hpp#L1816-L1833*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3767-L3777*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3796-L3811; src/compiler/mlir_gen_stmt.cpp#L3812-L3821*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3850-L3927; src/compiler/mlir_gen_stmt.cpp#L3928-L3937*
...
*Source: src/compiler/sema_stmt.cpp#L9136-L9148*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L4319-L4320; src/compiler/mlir_gen_stmt.cpp#L4379-L4381; src/compiler/mlir_gen_stmt.cpp#L4754*
...
*Source: src/compiler/sema_stmt.cpp#L8777-L8784; src/compiler/sema_stmt.cpp#L8823-L8827*

### `pat.match.guard-bool` — match guard must be bool

A match-arm guard expression `pat if <e> =>` must have type `bool`.

*Source: src/compiler/sema_stmt.cpp#L8956-L8963*
...
*Source: src/compiler/sema_stmt.cpp#L8964-L8998*
...
*Source: src/compiler/sema_impl.hpp#L4247-L4251*
...
*Source: src/compiler/sema_stmt.cpp#L8939-L8952; src/compiler/sema_stmt.cpp#L8999-L9019; src/compiler/sema_stmt.cpp#L9041-L9049*
...
*Source: src/compiler/sema_impl.hpp#L4217-L4231*
...
*Source: src/compiler/sema_stmt.cpp#L8892-L8899; src/compiler/sema_stmt.cpp#L8986-L8998*
...
*Source: src/compiler/sema_impl.hpp#L4252-L4258*
...
*Source: src/compiler/sema_stmt.cpp#L8622-L8683; src/compiler/sema_stmt.cpp#L8705-L8719; src/compiler/sema_stmt.cpp#L8514-L8545*
...
*Source: src/compiler/sema_stmt.cpp#L8685-L8719; src/compiler/sema_stmt.cpp#L8900-L8920*
...
*Source: src/compiler/sema_stmt.cpp#L9029-L9040*
...
*Source: src/compiler/sema_stmt.cpp#L8514-L8545; src/compiler/sema_stmt.cpp#L8546-L8559*
...
*Source: src/compiler/sema_stmt.cpp#L8561-L8570*
...
*Source: src/compiler/sema_stmt.cpp#L8858-L8880; src/compiler/sema_stmt.cpp#L8935-L8938; src/compiler/sema_stmt.cpp#L9050-L9063*
...
*Source: src/compiler/sema_stmt.cpp#L8609-L8683*
...
### `pat.let.refutability-checked` — let-pattern destructuring is refutability-checked

`let <pattern> = expr;` beyond a simple identifier/tuple binding (LET_PAT) requires sema to check the pattern is irrefutable; sema lowers the binding via an internal match.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L286*
...
**Divergence from Rust:** A — guarded nested-variant arms need a catch-all for exhaustiveness (DIVERGENCES.md: finite-enum coverage of guarded arms not yet proven)
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3450-L3480*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3445-L3449*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3358-L3369*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3286-L3294*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3384-L3391*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3278-L3281; src/compiler/mlir_gen_stmt.cpp#L3295-L3306*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3340-L3357; src/compiler/mlir_gen_stmt.cpp#L3374-L3377*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3430-L3444*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3481-L3508*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3392-L3429*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3307-L3326*
...
*Source: src/compiler/mlir_gen_stmt.cpp#L3327-L3383*
...
*Source: src/compiler/sema_impl.hpp#L4164-L4199*
...
**Divergence from Rust:** Logos addition.
...
**Divergence from Rust:** Logos addition: Writ container patterns.
...
**Divergence from Rust:** Logos addition; i24 bound is Writ-specific.

*Source: src/compiler/sema_stmt.cpp#L5312-L5327*

### `pat.writ.literal-pattern-forms` — Writ-literal pattern forms in match arms

Writ literals participate in pattern position with dedicated node kinds mirroring the literal forms: `@null` (PAT_WRIT_NULL), `@true`/`@false` (PAT_WRIT_BOOL), `@<int>`/`@-<int>` (PAT_WRIT_INT, IS_NEG flag), `@"..."` (PAT_WRIT_STR), `@{k: pat, ...}` (PAT_WRIT_MAP, ITEMS=PAT_WRIT_MAP_ENTRY list each with KEY/VALUE), `@[pat, ...]` (PAT_WRIT_ARR, ITEMS=sub-patterns), `@<T>[..]` (PAT_WRIT_TYPED_ARR, TYPE) and `@<K,V>{..}` (PAT_WRIT_TYPED_MAP, TYPE/RET_TYPE?) match on the typed-array/typed-map's declared type rather than on element contents.

*Source: tools/peg_gen_cpp/grammars/logos.peg#L251-L259*
...
**Divergence from Rust:** Logos extension (Writ value patterns); no Rust equivalent.
...
**Divergence from Rust:** Logos addition: Writ data-substrate patterns.
...
**Divergence from Rust:** Writ pattern matching is a Logos addition (no Rust equivalent).
...
**Divergence from Rust:** Logos addition: Writ typed-container patterns.
