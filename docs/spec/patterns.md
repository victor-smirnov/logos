...
### `pat.match.ast-exhaustiveness-nested-enum-payload` — Nested enum-payload exhaustiveness checked on AST patterns

Exhaustiveness for nested enum-payload patterns (`Some(Some(v))`/`Some(None)`/`None`) is decided over the original AST patterns, descending into each variant's single payload, because the LIR-level check skips guarded (desugared) arms.

*Source:* `src/compiler/sema_impl.hpp#L4195-L4200`, `src/compiler/sema_impl.hpp#L4301-L4306`
...
A `ref`/`ref mut` pattern binding over a payload of type P binds a reference whose indirection depth = (number of Ref/MutRef layers sema wrapped on the binding type) − (number of thin-ref layers P already contributes). The binding's value is the address of the payload slot, with depth−1 intermediate slots materialized when match ergonomics flow through nested references (`&&Option<T>`). A fat-ref payload (Ref/MutRef whose repr is not a thin pointer) is excluded from this path.

*Uncertainty:* Depth>1 path is driven by match-ergonomics through nested references; exact user-visible deref count inferred from comments.

*Source:* `src/compiler/mlir_gen_stmt.cpp#L19-L49`, `src/compiler/mlir_gen_stmt.cpp#L109-L160`
...
*Divergence from Rust:* Rust deprecated float patterns; Logos rejects them outright.
...
*Divergence from Rust:* Rust also forbids float patterns (deprecated/removed).
...
### `pat.or.payload-distributes-per-alternative` — Or-pattern inside a variant payload distributes into one arm per alternative

A multi-alternative or-pattern appearing as a variant payload arg (`Some(P|Q)`) is fanned out into one effective arm per alternative (`Some(P) | Some(Q)`); each re-evaluates the arm guard with its own bindings, so a failing guard backtracks to the next alternative.

*Source:* `src/compiler/sema_impl.hpp#L4165-L4170`
...
*Divergence from Rust:* A: no default-binding-mode by-ref propagation for refs in for-loop patterns.
...
*Divergence from Rust:* Subset of Rust for-loop pattern support (richer sub-patterns are a follow-up).
