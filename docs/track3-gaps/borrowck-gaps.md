...
|---|---|---|---|---|---|
| B3-bg-01 | `let` declare-without-init for an immutable binding | Deferred — needs grammar + sema definite-assignment tracker (mark binding as initialised at every assignment site, reject use before all paths assign). Real but bounded slice; ~2-3 days. | Grammar rejects `let v: T;`. Rust accepts when definitely-assigned downstream. | `borrowck-scope-of-deref-issue-4666` (fun1 dropped) | `let v: usize;` ⇒ "syntax error" |
| B3-bg-02 | `let mut` declare-without-init | Deferred — same root cause as B3-bg-01; grammar fails before sema sees it. | Mostly the same. | `lazy-init` | as above |
