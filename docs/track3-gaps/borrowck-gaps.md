...
|---|---|---|---|---|---|
| B3-bg-01 | `let` declare-without-init for an immutable binding | Open | Logos grammar rejects `let v: T;` (no initialiser, no `mut`). Rust accepts this when the binding is definitely-assigned downstream. | `borrowck-scope-of-deref-issue-4666` (fun1 dropped), various | `let v: usize;` ⇒ "syntax error near 'usize'" |
| B3-bg-02 | `let mut` declare-without-init | Open | Mostly the same — but possibly grammar accepts and sema rejects. Confirm. | `lazy-init` | `let mut x: isize; if … { x = 12; } else { x = 10; }` ⇒ "syntax error near 'isize'" |
...
| B3-bg-04 | Addr-of a captured local inside closure body | Open | `&x` inside a closure where `x` is captured by value crashes mlir-gen with `& undefined 'x'`. By-value captures and reads work; capture-by-reference / addr-of-captured isn't wired. | `borrowck-closures-two-imm` (b/c dropped) | `let c = || -> i32 { return get(&x); };` ⇒ "mlir_gen: & undefined 'x'" |
...
| B3-bg-06 | Vec → slice coercion at fn-arg position | Open | Rust passes `Vec<T>` into `fn foo(v: &[T])` via Deref coercion. Logos has `Vec<T>` but no auto-Deref to slice. | `borrowck-mut-vec-as-imm-slice` | `fn want_slice(v: &[isize])` called with `&v: &Vec<isize>` rejected |
| B3-bg-07 | `for i in &v` where `i: &T` and the body does `*i` | Open | The IntoIterator-for-`&Vec` path yields refs whose lifetime is tied to the borrow of `v`; Logos's current iterator borrowing model trips on `*i` reads inside the loop in some cases. Not fully reduced — flagged for triage. | `borrowck-mut-vec-as-imm-slice` (loop unrolled) | tbd |
