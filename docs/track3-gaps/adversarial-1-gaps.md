...
| ADV1-H | Local trait invisible at `dyn` position when name collides with prelude (user `trait Sub` vs ops `Sub`) | **FIXED 2026-06-28** | The predicted 12-site sweep was unnecessary: the impl/vtable ecosystem is bare-keyed + target-disambiguated, so only the NAME-ONLY trait lookups mis-bound. Fix = 3 scope-aware lookups (bare `traits_.find` → `find_trait_iter_scoped`): `lower_trait_def` (sema_decl.cpp), `try_method_on_dyn` (sema_expr.cpp), and the supertrait-closure walk in `trait_vtable_layout` (sema_collect.cpp) — the last also covers a shadowed SUPERTRAIT (`Sub: Add`). No-op for non-colliding names; full ctest clean. Regression: `tests/logos/pass/dyn_trait_shadowing.logos`. | p2 in /tmp/adv1 |
