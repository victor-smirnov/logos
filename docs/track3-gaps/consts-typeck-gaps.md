...
|---|---|---|---|---|---|
| K10-co-01 | Arithmetic expression at array-length position | Open | `[T; 2*4]` rejected — only a single integer literal or generic `const N: i64` is accepted. Top-level `const N: i64 = 2*4;` parses but `[T; N]` not (C6-cc-02 family). | `arithmetic-expr-in-array-len` (literal inlined) | `[0i64; 2i64 * 4i64]` ⇒ "syntax error near '2i64'" |
