...
| C5-cl-03 | `ref` in closure parameter pattern | Deferred — needs grammar + sema to admit `ref` binding-mode in closure-param patterns (alongside the existing pattern-binding modes). Lowering: `\|ref x: T\|` binds `x: &T` and passes by-reference. ~2-3 days. | `\|ref x: T\| ...` rejected. | `issue-5239-2` (rewritten) | `\|ref x: isize\| { *x }` |
