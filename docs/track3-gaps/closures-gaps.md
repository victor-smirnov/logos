...
| C5-cl-07 | Tuple-destructure in closure parameter (`\|(x, y): (T1, T2)\| …`) | Open — Logos's closure_param grammar accepts `IDENT (COLON type_ref)?` and the `ref IDENT: type_ref` variant from C5-cl-03, but not a parenthesised tuple pattern. Workaround: take a single tuple param `t: (T1, T2)` and index via `t.0` / `t.1` inside the body. Closing it = grammar alt `LPAREN IDENT (COMMA IDENT)* RPAREN COLON type_ref` that emits a synth tuple-param + body-prologue `let (x, y) = __synth;` (parallel to C5-cl-03's `ref`-param rewrite). | `binding/fn-pattern-expected-type` (`\|(x, y): (isize, isize)\| { … }`). | `binding/fn-pattern-expected-type` (rewritten via tuple-param workaround) | `\|(x, y): (isize, isize)\| { … }` ⇒ "syntax error near '\|'" |
