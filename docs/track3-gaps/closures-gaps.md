...
| C5-cl-06 | Closure without explicit `-> R` can't return a value | Open — Logos closures default their return type to `void` when no `-> R` annotation is supplied; a `return value;` inside the body then errors with "return type mismatch — expected void, got T". Rust infers the return type from the body. Workaround: always annotate (`\|x: T\| -> R { return val; }`). Fix would let sema infer the closure ret type from its body (mirror what fn-body type-checking already does for `fn` items). | `binding/fn-pattern-expected-type` (rust `\|(x, y): (isize, isize)\| { assert_eq!(x, 1); … }`). | n/a (not imported yet) | `let f = \|x: i32\| { return x + 1; };` ⇒ "return type mismatch — expected void, got {integer}" |
