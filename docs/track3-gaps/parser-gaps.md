...
| P3-pg-04 | `break` / `continue` / `return` as expression in non-stmt position | Open — grammar's `primary_expr` doesn't admit `KW_BREAK` / `KW_CONTINUE` / `KW_RETURN` as expression forms; these are stmt-position only. In Rust they're typed `!`/Never and usable in any expression slot (`int_id(break)`, `let x = if … { … } else { return };`). Fix has two layers: grammar (add the three keywords to expr alts) + Never-type propagation through the surrounding type-check (overlaps with K10-co-04 / consts-typeck-gaps). | `for-loop-while/break-value` (`loop { int_id(break); }` won't parse). | n/a (not imported yet) | `int_id(break)` ⇒ "syntax error near '('" |
