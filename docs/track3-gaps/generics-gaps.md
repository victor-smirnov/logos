...
|---|---|---|---|---|---|
| G3-tg-01 | Turbofish on an enum variant constructor | ✅ Closed (2026-05-11) — added 2 enum_lit alts with explicit `::<TARGS>` between FIELD and the optional payload; sema reads ARGS via either direct-array or `{ITEMS: …}` shape (enum_lit_args sub-production). TYPE_PARAMS currently ignored by sema (inference still kicks in); explicit-arg hint can be wired later if it ever matters. | `Enum::Variant::<T>(arg)` failed to parse. | `match-bot` (un-trimmed); `generic-tag-{local,match,values}` deferred (deeper issues) | `Option::Some::<isize>(3)` now parses |
