# Writ2 C++ Migration — Working Document
...
> `project_writ2_cpp_migration` (terser). Design doc: `writ2-design.md`.

---

## 1. The plan & current status

Migrate the Logos compiler (`logosc`) and the Logos stdlib off **Writ1** onto
**Writ2**. Seven steps:

1. **Audit Writ1 C++ usage in the compiler.** ✅ DONE (see §6.2 for the findings).
2. **Build the C++ Writ2 library (`logos_writ2`).** ✅ **DONE** — data model +
...
3. **Cut `logosc` over to Writ2** (the deep one). ⬜ NEXT. Multi-arena (§6.1) now
   ported. Scope in §6.2.
...
4. **Port the embedded Writ** (Logos `stdlib/lang/writ` + `stdlib/mem/writ`) to
   Writ2. ⬜ (Logos-side; the `lang/writ2` + `mem/writ2` stdlib already exists
   and is the byte-layout spec — see `project_writ2_port_complete`.)
5. **Port the Writ1 infra `persistent` needs** (`std.data.persistent`: fabric
   `WritStatic`, `AnyVal`, `View`) to Writ2. ⬜
6. **Remove Writ1** from Logos + C++. ⬜
7. **Celebrate.** ⬜
...
Headers `include/logos/writ2/`, sources + tests `src/writ2/`. CMake target
...
| `varint.hpp`, `fnv_hash.hpp` | ported verbatim from Writ1 (byte-identical to Logos) |
...
**The two changes from Writ1 (everything else is a port).**
...
1. **`RelativePtr` is SELF-relative** (`int64_t off`; `get() = (uint8_t*)this + off`).
   ...
   Ported from Memoria `arena::RelativePtr`. Writ1's was a base-relative `u32`.
...
2. **The arena is multi-segment, never-move** (MultiChunk: grow = append a chunk,
   ...
   Logos Writ1 arena already had MultiChunk; only the pointer model changed.)

**The byte-layout invariant (do not violate).** Writ2/C++ and Writ2/Logos share
ONE memory layout, byte-for-byte (same wire/disk format; both read the same bytes).
**The Logos writ2 stdlib (`stdlib/lang/writ2/*.logos`) is the layout spec; C++
mirrors it.** Some Logos layouts DIFFER from Writ1 — always match Logos:
- `AnyVal`/`HAny` = **8 bytes** (not Writ1's 4): `word==0` null; `&1==1` Pod
...
- `TinyObjectMap` = `{header,data-ptr}` (NOT inline values + schema_code); header
  bits **cap[52:57], size[58:63]** (SWAPPED vs Writ1).
- `MapEntry` = `{key:AnyVal, val:AnyVal}` = 16B (NOT Writ1's 8B).
- String tag = **130** (Logos `H2_STRING`), not Writ1's `28`. (Flagged: if the
...
so the view must keep it alive. This collapses Writ1's non-owning-view + `Own<>`
...
`ctest -R writ2_exerciser` (label `writ2;cpp`). 8 tests, each returns the first
...
- **Match Logos layouts, not Writ1** (see §3 — tinymap, MapEntry, AnyVal width,
...
- **ExternalRef is a Pod niche, not a tagged object** (writ2 change). It fits the 8B
...
The API + invariants match Writ1 so logosc's call-sites move with a `writ::` →
`writ2::` rename. Two writ2-specific changes:
...
### 6.2 Step 3 — cut `logosc` over to Writ2 (the deep one)

**Audit findings (step 1).** The compiler is woven into Writ1 at three layers:
...
`writ::` → `writ2::` across the compiler and adapt each site to the native
self-relative API; do NOT reintroduce a base/offset facade. End-state is clean
writ2. Incremental by translation unit, gated by the full build + test suite.
- **Phase A — DONE (`c9980a59`).** Additive writ2 API so the rename is near-mechanical
...
- **Per-site mapping:** `av.to_offset()`+`holder` → pass `av` to `as_tinymap/as_array/
  ...
  `binary_encode/clone/from_bytes_copy` → writ2 `binary_encode`/`clone`/`compactify`/
...
- **Phase B — AST producer + readers (the big-bang; do atomically).** The AST is built
  ...
  The writ2 producer surface is READY (`make_doc`/`WritCtr::make_tiny_map/make_array/
  ...
  **Producer codegen recipe** (the ~8 emission points in `codegen.cpp`, all currently
  emitting Writ1 `WritAccess`/base/offset → flip to writ2-native):
  | codegen emits now (Writ1) | flip to (writ2-native) |
  |---|---|
  | `logos::writ::Writ doc_;` (line ~751) | `logos::writ2::WritCtr doc_;` |
  | `make_doc(524288).get()` (~1637) | `*logos::writ2::make_doc(524288)` (MultiChunk) |
  ...
  | `node->set_schema_type_code(writ::schema::ast(c))` (~2207) | `…writ2::schema::ast(c)` |
  ...
  | includes (~585,784) `writ/view.hpp`,`access.hpp`,`schema_codes.hpp` | `writ2/document.hpp`,`view.hpp`,`schema_codes.hpp` |

  **GOTCHA — parser arena mode.** The Writ1 parser runs on a GrowableSingleChunk arena
  with base-relative offsets + `arena_checkpoint`/rollback for backtracking. On writ2
...
- **Phase C — LIR mirror — THE HARD PART (finding, 2026-06-10, branch `writ2-cutover`).**
  ...
  (ExternalRef) re-expressed on writ2. SEPARATE arena from the AST, so it CAN be staged:
  either convert it to self-relative as a dedicated push, OR keep `lir_view`/`lir_mirror`/
  `sema.hpp` mirror types on `logos_writ` (Writ1) while the AST goes writ2 (both libs
...
- **Phase E — link swap:** drop `logos_writ`, link `logos_writ2`; delete Writ1 (step 6).

**Original strategy note (kept for context):** keep the Writ1 **API + collection algorithms** so
...
near find-replace (`writ::` → `writ2::`, drop the `base` arg — self-relative needs
...
- **Exact Logos text grammar** — current `stringify`/`text_parser` are JSON-core; the
  Logos `stdlib/mem/writ2/{stringify,parser}.logos` define the real Writ text
...
- **Logos writ2 stdlib** (`stdlib/lang/writ2/*.logos`) — the BYTE-LAYOUT SPEC.
  `anyval.logos` (HAny niche), `hstring/array/hmap/decimal/typed_value/parameter`.
- **Writ1 C++** (`src/writ`, `include/logos/writ`) — API + algorithms to keep.
- **Design**: `docs/internals/writ2-design.md` (§2 self-rel, §3 segments, §4
