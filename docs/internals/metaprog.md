...
In an AI-coding world, there are two distinct forms of metaprogramming, and Logos's design recognizes both:
...
- **Probabilistic metaprogramming** — AI sessions writing Logos code from human intent. Flexible, semantic, handles novel structure; brittle on combinatorics, leaves gaps, validation explodes at scale.

Both have failure modes at the extremes. *All-deterministic* mechanizes everything that doesn't need to be mechanized — TMP-grade pain just to generate variations a human would write trivially. *All-probabilistic* drowns in N×M validation as combinatorics blow up across (data type × algorithm × storage backend × index strategy).

Memoria's hard-won rule, ported here for AI-era development: **base algorithms and data structures are written by hand in maximally generic, composable form; combinatorial interactions of types and infrastructure are generated by metaprograms.** In a Logos+AI world, the "by hand" role shifts to AI — but the layer split stays.

This calls for a third artifact, sitting between intent and machine:

### `genos` — Algorithmic and Structural Templates
...
{
    target := n
    if target >= total(tree): return OOR

    node := tree.root
    while not is_leaf(node):
        (idx, target) := SUM_shuttle(node.subtree_size, target)
        node := node.children[idx]

    if target >= len(node.keys): return OOR
    return (node.keys[target], node.vals[target])
}
```

A genos body is **executable through a small interpreter** — not lowered to native code, but runnable directly so that a developer or AI agent can validate the intent in isolation: take the genos, run it on test vectors, observe behavior. This is the grounding constraint — without an executable form, semi-formal templates drift; with one, they stay honest.

The interpreter is intentionally minimal:

- **Built-in**: control flow (`if`/`while`/`for`/`return`), arithmetic on primitive types, `:=` assignment, tuple destructure, basic comparisons.
- **Dispatched via hooks**: every unknown name (`total`, `is_leaf`, `SUM_shuttle`, `.children`, `.keys`) is forwarded to a hook table provided by tests or by concrete instantiations.

Hooks let the same genos run against mocked structures (for fast smoke tests), against the real compiled `descent.logos` implementation (as a conformance oracle), or against AI-generated alternatives (each must produce matching outputs on the same test vectors).

### Three-Layer Architecture

| Layer | Author | Artifact | Cardinality | Validation |
|---|---|---|---|---|
| Genos | Human ↔ AI co-curated | Algorithm/structure spec + invariants + reference test vectors | One per family | Interpreter + hooks |
| Deterministic metaprog | AI under human review | `derive_*` / `template` that consumes the genos and emits combinatoric variations | One per family | Test vectors + golden outputs |
| Combinatoric instantiations | Generated | Per-(K × V × CFG × ...) functions and impls | N×M×... — never read | Conformance against the genos |
| Glue / orchestration | AI | API surface, lifecycle, error paths | Small, contextual | Ordinary integration tests |

Compiled native code sits below as ground truth; the architecture is what feeds it.

The cardinality column is the real argument. A genos is one artifact per algorithm family; AI writes it carefully, humans review it carefully. A metaprog is one artifact per family; AI's task there is **shape-preserving** code — write a transformer from genos to combinatoric output, validate against a few golden cases. AI is good at this. The combinatoric output itself is many artifacts that no human will ever read in detail; trying to have AI write them directly puts the validation cost where AI is *worst* — at scale, with subtle drift between cases. The metaprog layer is the firewall: it absorbs the N×M explosion mechanically.

### Validation Asymmetry

The reason the third layer (combinatoric instantiations) cannot be AI-driven directly:

- *Validating one metaprog* = test vectors on a few canonical inputs, plus property "applying it to the genos preserves invariants". Bounded task.
- *Validating N×M independent instantiations* = either a conformance oracle (which is the genos interpreter — back to this architecture), or human reading of every artifact (impossible at N=50).

Without a metaprog layer, the combinatoric tier is either absent (= nothing to use) or accumulates drift case-by-case. There is no middle state that scales.

### Workflow

For a new family of algorithms or data structures:

1. **Write the genos.** Human + AI co-curate the form, invariants, and a small set of reference test vectors. Genos is the spec.
2. **Write or extend the metaprog.** AI writes a `derive_*` / `template` that consumes the genos shape and emits combinatoric instantiations. Bounded surface, easy to review.
3. **Write conformance tests against the genos.** Test vectors run against the interpreter (the genos's own behavior) and against compiled instantiations (the metaprog's output). Discrepancies surface immediately.
4. **Combinatoric instantiations are emergent.** No one reads them. They exist for the compiler.
5. **Glue layer.** AI writes user-facing API surface that wraps the generated artifacts.

A new contributor (human or AI) reading the codebase reads the genos to understand *what* the algorithm is, the metaprog to understand *how it specializes*, and the glue to understand *how it's exposed*. Combinatoric instantiations are invisible to reading.

### Status and Next Steps

The genos concept is **design-stage**, no implementation yet. The current persistent stack uses one half of the architecture today: `#[derive_branch_node]` is a deterministic metaprog that emits aggregate columns and per-op shuttle wrappers; the surrounding algorithms (descent, mutation, fork lifecycle) are hand-written rather than expressed as genos. The genos layer is what completes the picture.

Concrete next steps:

- **Cheap first move**: a `#[genos]` attribute on an ordinary `pub fn` that compiles as the *reference instantiation* for a family. No interpreter, no hooks — just a marker that says "this function is the canonical statement of the algorithm; AI sessions consult it before writing concrete cases". Costs nothing, captures most of the benefit.
- **Then add the interpreter**: minimal interpreter + hook dispatch as described above. Adds executable validation and decouples the genos from any single concrete instantiation.
- **Then conformance harness**: property-based testing where generators drive the same inputs through interpreter, compiled reference instantiation, and any AI-written alternatives. Discrepancies are bugs in *one of the three*; the conformance harness is the arbiter. This is also the substrate for coverage-guided fuzzing later.

The genos layer pairs naturally with the eventual PPL roadmap (coverage-guided adaptive fuzzing): genos defines the algorithm shape and reference behavior, the fuzzer drives inputs, all instantiations must agree. Property testing → fuzz testing → PPL is one continuous axis.
