...
### 2.1 Configurations are Writ documents

The first major design decision: configurations are **not** Logos structs.
They are Writ documents — recursive polymorphic dictionaries, which is
exactly the data class Writ was built for. A Logos struct schema for a
configuration would force structuring decisions onto the type level, where
they become rigid and "swampy" (more on this below). Writ is the right
substrate.

```logos
const PMAP_CFG: WritStatic = @{
    "name": "PMap",
    "streams": [
        @{"key_ty": <type:Varchar>, "val_ty": <type:Writ>, "ordered": true}
    ],
    "cow": true,
    "leaf_kind": "Variable",
...
Why Writ-as-config is structurally right:

- **Writ is already the cross-stage fabric** (compile / runtime / wire /
  disk; see [memory: feat_writ_compile_runtime_fabric]). Configurations
  written in Writ participate in this fabric automatically: serialisable,
...
- **Identity comes from content-hash for free.** Memoria's
  ...
  collapses to `writ_hash(serialize(cfg))`. Stable, cross-build,
  cross-language.
- **Configuration evolution = Writ schema evolution.** TinyObjectMap
  already carries a `schema_type_code`; adding a field to a configuration
  is a Writ schema change, not a separate problem.
- **Inspectability is Hest-native.** Predicates over configurations are
  ordinary Writ-document walks, no special "config inspection" intrinsics.
- **Rich types as values are already Writ's job.** A configuration
...
the language — is *Type as a first-class Writ value*. Today our metaprog
`Type` handle lives in compiler-side data structures, not in the Writ
...
metafn leaf_fixed_part(cfg: WritView, target: Type) -> Vec<Item> {
...
metafn assemble_btree(cfg: WritView) -> Vec<Item> {
...
- Assemblies **chain**: assembly A produces a configuration that drives
  ...
  naturally.
- `(assembly, cfg)` pairs have **stable identity** via Writ content-hash —
...
- `ExprBlob` / `QuoteItemBlob` splice through the Writ blob fabric.
...
- `@{...}` Writ literals as compile-time constants (WritStatic with
  cached identity).
...
- **Type as first-class Writ value.** §5 below. The single most
  load-bearing addition; without it, configurations cannot live in Writ,
...
- **Caching / incrementality of assembly.** An assembly is a pure function
  of its configuration; with Writ content-hash as the key, memoisation is
...
- **Cross-assembly references.** When assembly A produces a configuration
  ...
  Writ-as-config this falls out naturally (content-hash links), but the
  language surface for declaring such links needs to exist.

### 4.4 What collapses under Writ-as-config

The §4.3 list is shorter than it would be if configurations were Logos
structs, because Writ-as-config absorbs several would-be features:

- Configuration evolution → Writ schema versioning.
- Stable identity per (assembly, cfg) → Writ content-hash.
- Cross-build / cross-language compatibility → Writ binary format.
- Configuration serialisation / persistence → Writ is already the disk
  format.
- `Hash` / `Eq` / `Stringify` / `Clone` for configurations → existing
  Writ trait registry.
- Config validators based on shape → Writ schemas
  (`#[writ_schema]`-style annotation, when stabilised).
...
Writ-as-fabric story.

## 5. The bridge: Logos entities as first-class Writ values

For configurations to live in Writ, the metaprog `Type` handle must be a
Writ value. But the right framing is broader: **any addressable Logos
entity** should be embeddable as a Writ value — not just types, and not
just datatype-attributed types. Writ becomes the universal
identity / reflection substrate for the language.

The set of "addressable entities" we want to be able to put in a Writ
...
- **Functions and metafunctions** as values — including ordinary
  ...
  generic-function templates. This is what lets a Writ registry hold
...
- **Items in general** — struct definitions, trait declarations, impl
  ...
  `Vec<Item>`; storing those items in a Writ registry lets later passes
  walk them as data.
- **Other embedded Writ documents.** Writ within Writ: a registry
  whose values are themselves `WritStatic` documents, recursively. The
...
1. **Writ type-codes for each kind of Logos entity.** Hybrid encodings,
   per kind:
   ...
   - Generic instantiations / generic templates: structural — a small
     Writ container with head name + list of arguments / parameters.
     Recursive.
   - Functions / metafunctions: code + fully-qualified path + signature
     ...
     here.
   - Items: structural — Writ mirror of the LIR shape (largely already
     there for our existing `lir_view` / `LExpr` mirror infrastructure).
   - Embedded Writ documents: the existing nested-document mechanism;
     just promote it to a routine, named pattern.
2. **`<type:T>`, `<fn:path>`, `<template:T>`, `<item:expr>` syntax inside
   Writ literals.** A family of `@-quoted` forms, one per kind of
   embeddable entity. Lowering: compiler resolves to the appropriate
   handle, writes the Writ form.
3. **`WritView::as_type() / as_fn() / as_template() / as_item()` etc.**
   in the metaprog API — the inverse, readable from any metafunction.
4. **Standard trait registry entries** — `WritEqual`, `WritHash`,
   `WritStringify`, `WritClone`, `WritRelease` for each new kind,
   through the existing tag-dispatch registry.
5. **WritStatic literals as the canonical input to the metaprog API.**
   Already mostly true (the recent assoc-const WritStatic identity fix
   is a step in this direction); needs to be promoted to a stable
   contract: "Writ is the language in which configurations,
   registries, and cross-pass state for the metaprogramming layer are
   written".

### 5.1 Writ is Logos-specific by design

A guiding principle that follows from this generalisation: **Writ is a
Logos-specific format, and we do not restrict it to a portable
greatest-common-denominator subset**.

Other platforms that consume Writ (the inherited C++ Memoria, future
...
- Cross-platform interop is a constraint on **what gets serialised across
  the boundary**, not on what Writ is allowed to express in-language.
...
## 6. Writ as the home of type-collection algebra

Once Type is a first-class Writ value, the existing Writ container
machinery — `Array<T>`, `Map<K, V>`, SDN paths, schemas, captures, the SQL-
shaped query work in `writ_check` — all of it operates on collections of
types without any new infrastructure. This is a much larger consequence
than just "configurations live in Writ". It is a **category shift in
...
In Logos, with Type as a Writ value:
...
### 6.2 Heterogeneous records of types are just Writ maps
...
LeafKeyStruct`. In Logos with Writ-as-config the same record is a Writ
...
In Logos: a Writ `Map<Type, CodecCfg>` document. Lookup is `codecs[
key_ty]`. New codecs are added by writing a Writ entry. Components from
plugin modules can extend the table; the registry is just a Writ
...
With Type as a Writ value, you can have, at compile time:

```logos
const CONTAINER_REGISTRY: WritStatic = @{
    "containers": [
        @{"target": <type:PMap<UID256, Writ>>, "cfg": ${PMAP_UID_HERMES_CFG}},
        @{"target": <type:PMap<i64, Writ>>,    "cfg": ${PMAP_I64_HERMES_CFG}},
        ...
    ],
    "by_uid": @{
        <type:PMap<UID256, Writ>>.uid(): 0,
        ...
    },
};
```
...
that "Writ can hold a SQL-accessible database" is not hyperbole here:
the existing `writ_check` and typed-path machinery already provides
...
replaces it is "you have data — Writ documents containing types —
manipulate it with normal Logos."

The cost of this is the bridge in §5: Type must be a Writ value, with
...
1. **Type encoding: structural vs hash-only vs hybrid.** A structural
   encoding makes a Writ-serialised configuration self-describing (a
...
4. **The line between configuration and type.** A configuration *contains*
   ...
   configurations are arbitrary Writ documents that happen to be
...
- Memory: `project_persistent_pkg`, `feat_metaprog_inversion`,
  `feat_metaprog_two_layer_hygiene`, `feat_writ_compile_runtime_fabric`,
