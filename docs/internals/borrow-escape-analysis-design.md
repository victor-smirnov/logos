...
Goal: enforce the remaining Writ (and general) borrow-safety properties the
...
[writ-minimal-container-plan.md](writ-minimal-container-plan.md) (its step 3).
...
tracking (line ~2160). So objects accessed via a `&mut`/`*mut` (the whole Writ
...
`HAny` is a value (movable), but a Ref HAny is an absolute pointer into a Writ
