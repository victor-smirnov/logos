...
The manifest is a Hermes document in text (SDN) form. Top-level fields:
...
my-project/
  ...
  .lforge/
    <profile>/                         # debug | release
      ...
      _files/<lib>/<stem>.{o,hermes0}  # per-file artifacts (incremental)
...
`debug` is the default profile; `release` is selected with `--release`. Per-file `.o`/`.hermes0` artifacts make rebuilds incremental: the action runs only if the source's mtime is newer than the output's. Re-archiving runs only if any per-file artifact rebuilt or the archive itself is stale.
...
~/.cache/lforge/
  ...
  build/<sha256-hex>/            # compiled artifacts, one per cache key
    lib<module>.a
    meta.hermes                  # what produced this entry
```
...
```
1. read ./lforge.hermes (lforge.manifest)
