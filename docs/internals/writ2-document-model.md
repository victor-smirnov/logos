# Writ2 document model — status (Writ1→Writ2 port)
...
yet). Each landed L4-green + valgrind-clean. Built on the Writ2 foundation
(never-move arena, HAny, self-describing DSTs, thin self_describing refs).

## The value model

`HAny` (lang.writ2.anyval) — an 8-byte tagged word, the AnyVal analog:
...
(IEEE double). `Writ2::int(v)` / `float(v)` build them (Pod when it fits, else
...
(`obj[-1]`, the Writ1 datatag wire). `container.alloc_tagged(alloc, size, code)`
...
| `HMap` | string-keyed growing hash map (JSON object) | FNV-1a + open addressing + rehash; was Writ1 ObjectMap |
...
| Op | Module | Direction |
|---|---|---|
| stringify | mem.writ2.stringify | doc → JSON-ish text |
| parse | mem.writ2.parser | JSON-ish text → doc |
| hbs_write / hbs_read | mem.writ2.hbs | doc ⇄ bytes (lead-byte + vlen) |
| compactify / clone | lang.writ2.compactify | doc → fresh-arena copy (copying GC, design §6) |
| equal | lang.writ2.equal | structural (deep) equality by value, cross-arena |
...
- **Global type namespace**: Writ2 type names that collide with Writ1's resolve
  ambiguously. Caught `ObjectMap`→`HMap` and `MapEntry`→`HMapEntry` (the latter:
  `sizeof` resolved to the new 16-byte type but `.add` STRIDE to Writ1's 8-byte
...
- **self_describing gate exemption** needs the struct's def imported (`use
  logos.lang.writ2.hstring`) for the compiler to see the flag and allow safe
...
- **Cutover**: retire `stdlib/lang/writ/*` + `stdlib/mem/writ/*` wholesale —
  explicitly a SEPARATE session (and gated on a new C++ Writ2 implementation
  first). Writ2 lives alongside the old Writ until then (design §9 part 7).
