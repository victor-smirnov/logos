...
static void test_document_with_array_root() {
    ...
    arr.slot(3)->set_pointer(s.ptr(), arr.ptr()->slot(3, HermesAccess::base(doc)) ? HermesAccess::base(doc) : HermesAccess::base(doc));
    ...
    LOGOS_ASSERT(*str_slot->as_ptr<ArenaString>(HermesAccess::base(doc)) == "test", "HERMES-DOC-002", "");
...
static void test_compactify_simple() {
    ...
    LOGOS_TRACE("hermes.doc.compactify", "status", "pass",
        "original_used", HermesAccess::arena(doc).total_used(),
        "compact_used", HermesAccess::arena(compact).total_used());
    std::printf("  Compactify (simple): OK (original %zu → compact %zu bytes)\n",
        HermesAccess::arena(doc).total_used(), HermesAccess::arena(compact).total_used());
}
...
static void test_zero_copy_array_round_trip() {
    ...
    auto* larr = HermesAccess::root<ObjectArray>(loaded);
    uint8_t* lb = HermesAccess::base(loaded);
