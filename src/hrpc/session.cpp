...
using logos::writ::binary_encode;
using logos::writ::binary_decode;
...
void Session::handle_return(const MessageHeader& hdr, Writ payload) noexcept {
...
void Session::handle_call_channel_msg(const MessageHeader& hdr, Writ payload) noexcept {
...
void Session::handle_ctx_channel_msg(const MessageHeader& hdr, Writ payload) noexcept {
    std::shared_ptr<PendingCall> pc;
    {
        std::lock_guard lock(pending_calls_mutex_);
        auto it = pending_calls_.find(hdr.call_id);
