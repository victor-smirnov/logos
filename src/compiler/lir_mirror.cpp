...
public:
    LirMirrorEmitter(hermes2::Arena& a, LirMirrorTable& t) : arena_(a), table_(t) {}
    ...
    hermes2::arena_offset_t emit_expr_public (const LExpr& e)    { return emit_expr(e); }
    hermes2::arena_offset_t emit_stmt_public (const LStmt& s)    { return emit_stmt(s); }
    hermes2::arena_offset_t emit_block_public(const LBlock& b)   { return emit_block(b); }
    hermes2::arena_offset_t emit_pat_public  (const Pattern& p)  { return emit_pat(p); }
    hermes2::arena_offset_t emit_hv_public   (const HermesVal& v){ return emit_hv(v); }
    ...
    // for that kind.
    hermes2::arena_offset_t emit_lit_bool_direct(TypeRef ty, bool v) {
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::LitBool));
        put(map_off, ek::LIT_BOOL, put_bool(v));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_lit_int_direct(TypeRef ty, int64_t v) {
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::LitInt));
        put(map_off, ek::LIT_I64, put_i64(v));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_lit_float_direct(TypeRef ty, double v) {
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::LitFloat));
        put(map_off, ek::LIT_F64, put_f64(v));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_lit_str_direct(TypeRef ty, std::string_view v) {
        auto s_av = put_string(v);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::LitStr));
        put(map_off, ek::LIT_STR, s_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_var_ref_direct(TypeRef ty, std::string_view name) {
        auto n_av = put_string(name);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::VarRef));
        put(map_off, ek::NAME, n_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_addr_of_direct(TypeRef ty, std::string_view var_name) {
        auto n_av = put_string(var_name);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::AddrOf));
        put(map_off, ek::NAME, n_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_pack_expand_direct(TypeRef ty, std::string_view var_name) {
        auto n_av = put_string(var_name);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::PackExpand));
        put(map_off, ek::NAME, n_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_size_of_direct(TypeRef ty, TypeRef elem) {
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::SizeOf));
        put(map_off, ek::ELEM_TYPE, type_av(elem));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_align_of_direct(TypeRef ty, TypeRef elem) {
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::AlignOf));
        put(map_off, ek::ELEM_TYPE, type_av(elem));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_generic_ref_direct(TypeRef ty,
                                                    std::string_view name,
                                                    const std::vector<TypeRef>& type_args) {
        auto cn_av = put_string(name);
        auto ta_av = type_array(type_args);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::GenericRef));
        put(map_off, ek::CALLEE,    cn_av);
        put(map_off, ek::TYPE_ARGS, ta_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_type_code_of_direct(TypeRef ty, TypeRef elem) {
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::TypeCodeOf));
        put(map_off, ek::ELEM_TYPE, type_av(elem));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_reflect_of_direct(TypeRef ty, TypeRef elem) {
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::ReflectOf));
        put(map_off, ek::ELEM_TYPE, type_av(elem));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    ...
    hermes2::arena_offset_t emit_deref_direct(TypeRef ty, const lir::LExprPtr& operand) {
        auto o_av = expr_av(operand);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::Deref));
        put(map_off, ek::OPERAND, o_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_cast_direct(TypeRef ty, const lir::LExprPtr& operand,
                                             std::string_view hermes_build_fn) {
        auto o_av = expr_av(operand);
        hermes2::AnyVal hbf_av;
        bool has_hbf = !hermes_build_fn.empty();
        if (has_hbf) hbf_av = put_string(hermes_build_fn);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::Cast));
        put(map_off, ek::OPERAND, o_av);
        if (has_hbf) put(map_off, ek::HERMES_BUILD_FN, hbf_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_try_direct(TypeRef ty, const lir::LExprPtr& inner,
                                            int32_t ok_disc, int32_t err_disc) {
        auto in_av = expr_av(inner);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::Try));
    ...
    hermes2::arena_offset_t emit_slice_lit_direct(TypeRef ty, const lir::LExprPtr& base,
                                                  const lir::LExprPtr& len) {
        auto b_av = expr_av(base);
        auto l_av = expr_av(len);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::SliceLit));
        put(map_off, ek::BASE_PTR, b_av);
        put(map_off, ek::LEN,      l_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_slice_index_direct(TypeRef ty, const lir::LExprPtr& slice,
                                                    const lir::LExprPtr& index) {
        auto s_av = expr_av(slice);
        auto i_av = expr_av(index);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::SliceIndex));
        put(map_off, ek::SLICE, s_av);
        put(map_off, ek::INDEX, i_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_slice_len_direct(TypeRef ty, const lir::LExprPtr& slice) {
        auto s_av = expr_av(slice);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::SliceLen));
        put(map_off, ek::SLICE, s_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_slice_ptr_direct(TypeRef ty, const lir::LExprPtr& slice) {
        auto s_av = expr_av(slice);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::SlicePtr));
        put(map_off, ek::SLICE, s_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_addr_of_temp_direct(TypeRef ty, const lir::LExprPtr& inner,
                                                     bool is_mut) {
        auto in_av = expr_av(inner);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::AddrOfTemp));
        put(map_off, ek::INNER,  in_av);
        put(map_off, ek::IS_MUT, put_bool(is_mut));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_hermes_lit_direct(TypeRef ty,
        ...
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::HermesLit));
    ...
    hermes2::arena_offset_t emit_enum_lit_direct(TypeRef ty,
                                                 std::string_view enum_name,
                                                 std::string_view variant,
                                                 int64_t disc) {
        auto en_av = put_string(enum_name);
        auto vr_av = put_string(variant);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::EnumLit));
    ...
    hermes2::arena_offset_t emit_enum_lit_data_direct(TypeRef ty,
                                                      std::string_view enum_name,
                                                      std::string_view variant,
                                                      int64_t disc,
        ...
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::EnumLitData));
        put(map_off, ek::ENUM_NAME, en_av);
        put(map_off, ek::VARIANT,   vr_av);
        put(map_off, ek::DISC,      put_i64(disc));
    ...
    hermes2::arena_offset_t emit_struct_lit_direct(TypeRef ty,
                                                   std::string_view name,
                                                   const std::vector<std::pair<std::string, lir::LExprPtr>>& fields) {
        auto n_av = put_string(name);
        auto fa   = struct_fields(fields);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::StructLit));
    ...
    hermes2::arena_offset_t emit_call_direct(TypeRef ty,
        ...
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::Call));
        put(map_off, ek::CALLEE,    cn_av);
        put(map_off, ek::TYPE_ARGS, ta_av);
    ...
    hermes2::arena_offset_t emit_method_call_direct(TypeRef ty,
        ...
        auto ta_av   = type_array(type_args);
        auto ar_av   = expr_array(args);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::MethodCall));
    ...
    hermes2::arena_offset_t emit_unary_direct(TypeRef ty, std::string_view op,
                                              const lir::LExprPtr& operand) {
        auto op_av = put_string(op);
        auto o_av  = expr_av(operand);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::Unary));
        put(map_off, ek::OP,      op_av);
        put(map_off, ek::OPERAND, o_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_bin_op_direct(TypeRef ty, std::string_view op,
                                               const lir::LExprPtr& lhs,
                                               const lir::LExprPtr& rhs) {
        auto op_av = put_string(op);
        auto l_av  = expr_av(lhs);
        auto r_av  = expr_av(rhs);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::BinOp));
        put(map_off, ek::OP,  op_av);
    ...
    hermes2::arena_offset_t emit_field_read_direct(TypeRef ty,
                                                   const lir::LExprPtr& receiver,
                                                   std::string_view field) {
        auto r_av = expr_av(receiver);
        auto f_av = put_string(field);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::FieldRead));
        put(map_off, ek::RECEIVER, r_av);
        put(map_off, ek::NAME,     f_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_index_read_direct(TypeRef ty,
                                                   const lir::LExprPtr& receiver,
                                                   const lir::LExprPtr& index) {
        auto r_av = expr_av(receiver);
        auto i_av = expr_av(index);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::IndexRead));
        put(map_off, ek::RECEIVER, r_av);
        put(map_off, ek::INDEX,    i_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_ptr_arith_direct(TypeRef ty, uint8_t op,
                                                  const lir::LExprPtr& ptr,
                                                  const lir::LExprPtr& offset) {
        auto p_av = expr_av(ptr);
        auto o_av = expr_av(offset);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::PtrArith));
    ...
    hermes2::arena_offset_t emit_ptr_diff_direct(TypeRef ty, bool by_byte,
                                                 const lir::LExprPtr& lhs,
                                                 const lir::LExprPtr& rhs) {
        auto l_av = expr_av(lhs);
        auto r_av = expr_av(rhs);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::PtrDiff));
    ...
    hermes2::arena_offset_t emit_if_expr_direct(TypeRef ty,
        ...
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::IfExpr));
    ...
    hermes2::arena_offset_t emit_tuple_lit_direct(TypeRef ty,
                                                  const std::vector<lir::LExprPtr>& elems) {
        auto el_av = expr_array(elems);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::TupleLit));
        put(map_off, ek::ELEMS, el_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_tuple_index_direct(TypeRef ty,
                                                    const lir::LExprPtr& receiver,
                                                    uint32_t index) {
        auto r_av = expr_av(receiver);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::TupleIndex));
        put(map_off, ek::RECEIVER,        r_av);
        put(map_off, ek::TUPLE_INDEX_VAL, put_u32(index));
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_arr_lit_direct(TypeRef ty,
                                                const std::vector<lir::LExprPtr>& elems) {
        auto el_av = expr_array(elems);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::ArrLit));
        put(map_off, ek::ELEMS, el_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_block_expr_direct(TypeRef ty,
                                                   const lir::LBlock* block,
                                                   const lir::LExprPtr& result) {
        auto b_av = block_av_raw(block);
        auto r_av = expr_av(result);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::BlockExpr));
        put(map_off, ek::BLOCK,  b_av);
        put(map_off, ek::RESULT, r_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_closure_call_direct(TypeRef ty,
                                                     const lir::LExprPtr& callee,
                                                     const std::vector<lir::LExprPtr>& args) {
        auto c_av = expr_av(callee);
        auto a_av = expr_array(args);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::ClosureCall));
        put(map_off, ek::CALLEE, c_av);
        put(map_off, ek::ARGS,   a_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_fn_ptr_call_direct(TypeRef ty,
                                                    const lir::LExprPtr& callee,
                                                    const std::vector<lir::LExprPtr>& args) {
        auto c_av = expr_av(callee);
        auto a_av = expr_array(args);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::FnPtrCall));
        put(map_off, ek::CALLEE, c_av);
        put(map_off, ek::ARGS,   a_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_match_expr_direct(TypeRef ty,
                                                   const lir::LExprPtr& scrut,
                                                   const std::vector<lir::EMatchArm>& arms) {
        auto sc_av = expr_av(scrut);
        auto ar_av = expr_arm_array(arms);
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::MatchExpr));
        put(map_off, ek::SCRUT, sc_av);
        put(map_off, ek::ARMS,  ar_av);
        if (ty) put(map_off, ec::TYPE, type_av(ty));
        return map_off;
    }
    hermes2::arena_offset_t emit_format_call_direct(TypeRef ty,
        ...
        auto map_off = make_map(hermes2::schema::lir_expr(lir_schema::expr::Code::FormatCall));
    ...
    hermes2::arena_offset_t emit_closure_box_direct(TypeRef ty,
        ...
        auto cl_av = inner ? closure_av(*inner) : hermes2::AnyVal{};
    ...
    // LStmt::kind.
    void put_line(hermes2::arena_offset_t map_off, uint32_t line) {
        if (line != 0) put(map_off, sc::LINE, put_u32(line));
    }
    hermes2::arena_offset_t emit_let_direct(uint32_t line, std::string_view name,
                                            TypeRef ty, const lir::LExprPtr& value,
                                            bool is_mut) {
        auto name_av = put_string(name);
        auto val_av  = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Let));
    ...
    hermes2::arena_offset_t emit_assign_direct(uint32_t line, std::string_view name,
                                               const lir::LExprPtr& value,
                                               bool drop_old = false) {
        auto name_av = put_string(name);
        auto val_av  = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Assign));
        put(map_off, sk::NAME,  name_av);
        put(map_off, sk::VALUE, val_av);
    ...
    hermes2::arena_offset_t emit_return_direct(uint32_t line, const lir::LExprPtr& value) {
        auto val_av = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Return));
        put(map_off, sk::VALUE, val_av);
        put_line(map_off, line);
        return map_off;
    }
    hermes2::arena_offset_t emit_if_stmt_direct(uint32_t line,
                                                const lir::LExprPtr& cond,
                                                const lir::LBlock* then_blk,
                                                const lir::LBlock* else_blk) {
        auto cond_av = expr_av(cond);
        auto then_av = block_av_raw(then_blk);
        hermes2::AnyVal else_av;
        if (else_blk) else_av = block_av_raw(else_blk);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::If));
    ...
    hermes2::arena_offset_t emit_while_direct(uint32_t line,
                                              const lir::LExprPtr& cond,
                                              const lir::LBlock* body,
                                              std::string_view label) {
        auto cond_av = expr_av(cond);
        auto body_av = block_av_raw(body);
        hermes2::AnyVal label_av;
        if (!label.empty()) label_av = put_string(label);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::While));
        put(map_off, sk::COND,  cond_av);
    ...
    hermes2::arena_offset_t emit_for_direct(uint32_t line,
                                            std::string_view var,
        ...
        hermes2::AnyVal label_av;
        if (!label.empty()) label_av = put_string(label);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::For));
    ...
    hermes2::arena_offset_t emit_loop_direct(uint32_t line,
                                             const lir::LBlock* body,
                                             std::string_view label,
                                             std::string_view break_slot,
                                             TypeRef result_type) {
        auto body_av = block_av_raw(body);
        hermes2::AnyVal label_av, slot_av;
        if (!label.empty())      label_av = put_string(label);
        if (!break_slot.empty()) slot_av  = put_string(break_slot);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Loop));
        put(map_off, sk::BODY,        body_av);
        put(map_off, sk::LABEL,       label_av);
    ...
    hermes2::arena_offset_t emit_break_direct(uint32_t line,
                                              const lir::LExprPtr& value,
                                              std::string_view label) {
        auto val_av = expr_av(value);
        hermes2::AnyVal label_av;
        if (!label.empty()) label_av = put_string(label);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Break));
        put(map_off, sk::VALUE, val_av);
        put(map_off, sk::LABEL, label_av);
        put_line(map_off, line);
        return map_off;
    }
    hermes2::arena_offset_t emit_continue_direct(uint32_t line,
                                                 std::string_view label) {
        hermes2::AnyVal label_av;
        if (!label.empty()) label_av = put_string(label);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Continue));
        put(map_off, sk::LABEL, label_av);
        put_line(map_off, line);
        return map_off;
    }
    hermes2::arena_offset_t emit_block_stmt_direct(uint32_t line,
                                                   const lir::LBlock* body) {
        auto body_av = block_av_raw(body);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Block));
        put(map_off, sk::BODY, body_av);
        put_line(map_off, line);
        return map_off;
    }
    hermes2::arena_offset_t emit_field_write_direct(uint32_t line,
        ...
        auto val_av   = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::FieldWrite));
    ...
    hermes2::arena_offset_t emit_index_write_direct(uint32_t line,
                                                    std::string_view arr,
                                                    const lir::LExprPtr& index,
                                                    const lir::LExprPtr& value) {
        ...
        auto val_av  = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::IndexWrite));
    ...
    hermes2::arena_offset_t emit_field_index_write_direct(uint32_t line,
                                                          std::string_view receiver,
                                                          std::string_view field,
                                                          const lir::LExprPtr& index,
                                                          const lir::LExprPtr& value) {
        ...
        auto idx_av   = expr_av(index);
        auto val_av   = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::FieldIndexWrite));
        put(map_off, sk::RECEIVER, recv_av);
        put(map_off, sk::FIELD,    field_av);
        put(map_off, sk::INDEX,    idx_av);
        put(map_off, sk::VALUE,    val_av);
    ...
    hermes2::arena_offset_t emit_expr_stmt_direct(uint32_t line,
                                                  const lir::LExprPtr& expr) {
        auto expr_avv = expr_av(expr);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::ExprStmt));
        put(map_off, sk::EXPR, expr_avv);
        put_line(map_off, line);
        return map_off;
    }
    hermes2::arena_offset_t emit_match_stmt_direct(uint32_t line,
                                                   const lir::LExprPtr& scrut,
                                                   const std::vector<lir::LMatchArm>& arms) {
        auto scrut_av = expr_av(scrut);
        auto arms_av  = arm_array(arms);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Match));
        put(map_off, sk::SCRUT, scrut_av);
        put(map_off, sk::ARMS,  arms_av);
        put_line(map_off, line);
        return map_off;
    }
    hermes2::arena_offset_t emit_for_each_direct(uint32_t line,
                                                 std::string_view var,
        ...
        auto body_av = block_av_raw(body);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::ForEach));
        put(map_off, sk::VAR,       var_av);
    ...
    hermes2::arena_offset_t emit_deref_write_direct(uint32_t line,
                                                    const lir::LExprPtr& ptr,
                                                    const lir::LExprPtr& value) {
        auto ptr_av = expr_av(ptr);
        auto val_av = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::DerefWrite));
        put(map_off, sk::PTR,   ptr_av);
        put(map_off, sk::VALUE, val_av);
        put_line(map_off, line);
        return map_off;
    }
    hermes2::arena_offset_t emit_drop_direct(uint32_t line,
        ...
        hermes2::AnyVal drop_av;
        if (!drop_fn.empty()) drop_av = put_string(drop_fn);
        hermes2::AnyVal moved_av;
        if (!moved_fields.empty()) moved_av = string_array(moved_fields);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::Drop));
    ...
    hermes2::arena_offset_t emit_deref_field_write_direct(uint32_t line,
                                                          std::string_view receiver,
        ...
        auto field_av = put_string(field);
        auto val_av   = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::DerefFieldWrite));
        put(map_off, sk::RECEIVER,   recv_av);
    ...
    hermes2::arena_offset_t emit_tuple_write_direct(uint32_t line,
                                                    std::string_view receiver,
                                                    uint32_t index,
                                                    const lir::LExprPtr& value,
                                                    TypeRef recv_type) {
        auto recv_av = put_string(receiver);
        auto val_av  = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::TupleWrite));
        put(map_off, sk::RECEIVER,        recv_av);
    ...
    hermes2::arena_offset_t emit_let_else_direct(uint32_t line,
    ...
    hermes2::arena_offset_t emit_chain_field_write_direct(uint32_t line,
                                                          std::string_view receiver,
        ...
        auto val_av    = expr_av(value);
        auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Code::ChainFieldWrite));
        put(map_off, sk::RECEIVER,   recv_av);
    ...
    hermes2::arena_offset_t emit_hv_null_direct() {
        return make_map(hermes2::schema::lir_expr(HV_BASE_DIRECT + 0));
    }
    hermes2::arena_offset_t emit_hv_bool_direct(bool value) {
        auto map_off = make_map(hermes2::schema::lir_expr(HV_BASE_DIRECT + 1));
        put(map_off, hk::BOOL_VALUE, put_bool(value));
        return map_off;
    }
    hermes2::arena_offset_t emit_hv_int_direct(int64_t value) {
        auto map_off = make_map(hermes2::schema::lir_expr(HV_BASE_DIRECT + 2));
        put(map_off, hk::INT_VALUE, put_i64(value));
        return map_off;
    }
    hermes2::arena_offset_t emit_hv_float_direct(double value) {
        auto map_off = make_map(hermes2::schema::lir_expr(HV_BASE_DIRECT + 3));
        put(map_off, hk::FLOAT_VALUE, put_f64(value));
        return map_off;
    }
    hermes2::arena_offset_t emit_hv_str_direct(std::string_view value) {
        auto s_av = put_string(value);
        auto map_off = make_map(hermes2::schema::lir_expr(HV_BASE_DIRECT + 4));
        put(map_off, hk::STR_VALUE, s_av);
        return map_off;
    }
    hermes2::arena_offset_t emit_hv_map_direct(const std::vector<lir::HVMapEntry>& entries,
                                               std::string_view key_type) {
        std::vector<hermes2::AnyVal> key_strs, key_ints, val_avs;
    ...
    hermes2::arena_offset_t emit_hv_array_direct(const std::vector<lir::HermesValPtr>& elements,
    ...
    hermes2::arena_offset_t emit_hv_capture_direct(uint32_t param_index, uint32_t value_index) {
        auto map_off = make_map(hermes2::schema::lir_expr(HV_BASE_DIRECT + 7));
        put(map_off, hk::PARAM_INDEX, put_u32(param_index));
        put(map_off, hk::VALUE_INDEX, put_u32(value_index));
        return map_off;
    }
    hermes2::arena_offset_t emit_hv_type_direct(uint32_t kind, uint64_t uid, std::string_view name) {
        auto name_av = put_string(name);
        auto map_off = make_map(hermes2::schema::lir_expr(HV_BASE_DIRECT + 8));
    ...
    hermes2::arena_offset_t emit_pat_variant_direct(std::string_view enum_name,
                                                    std::string_view variant,
                                                    int64_t disc) {
        auto enum_av    = put_string(enum_name);
        auto variant_av = put_string(variant);
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Variant));
        put(map_off, pk::ENUM_NAME, enum_av);
        put(map_off, pk::VARIANT,   variant_av);
        put(map_off, pk::DISC,      put_i64(disc));
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_int_direct(int64_t value) {
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Int));
        put(map_off, pk::INT_VALUE, put_i64(value));
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_bool_direct(bool value) {
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Bool));
        put(map_off, pk::BOOL_VALUE, put_bool(value));
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_wild_direct(std::string_view name) {
        auto name_av = name.empty() ? hermes2::AnyVal{} : put_string(name);
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Wild));
        put(map_off, pk::NAME, name_av);
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_variant_data_direct(std::string_view enum_name,
                                                         std::string_view variant,
                                                         int64_t disc,
        ...
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::VariantData));
        put(map_off, pk::ENUM_NAME,      enum_av);
        put(map_off, pk::VARIANT,        variant_av);
        put(map_off, pk::DISC,           put_i64(disc));
    ...
    hermes2::arena_offset_t emit_pat_or_direct(const std::vector<lir::Pattern>& alts) {
        auto subs_av = pat_array(alts);
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Or));
        put(map_off, pk::SUBS, subs_av);
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_tuple_direct(const std::vector<std::string>& bindings,
                                                  const std::vector<TypeRef>& binding_types,
                                                  const std::vector<lir::Pattern>& subs) {
        auto bindings_av = string_array(bindings);
        auto btypes_av   = type_array(binding_types);
        auto subs_av     = pat_array(subs);
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Tuple));
        put(map_off, pk::BINDINGS,      bindings_av);
        put(map_off, pk::BINDING_TYPES, btypes_av);
        put(map_off, pk::SUBS,          subs_av);
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_range_direct(int64_t lo, int64_t hi) {
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Range));
        put(map_off, pk::LO, put_i64(lo));
        put(map_off, pk::HI, put_i64(hi));
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_struct_direct(std::string_view struct_name,
                                                   const std::vector<lir::PatFieldBinding>& fields,
                                                   bool has_rest) {
        auto name_av   = put_string(struct_name);
        auto fields_av = field_binding_array(fields);
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Struct));
        put(map_off, pk::STRUCT_NAME, name_av);
        put(map_off, pk::FIELDS,      fields_av);
        put(map_off, pk::HAS_REST,    put_bool(has_rest));
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_slice_direct(const std::vector<lir::Pattern>& prefix,
        ...
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::Slice));
        put(map_off, pk::PREFIX, pre_av);
        put(map_off, pk::REST,   rest_av);
        put(map_off, pk::SUFFIX, suf_av);
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_at_direct(std::string_view name,
                                               const std::vector<lir::Pattern>& sub,
                                               TypeRef type) {
        auto name_av = put_string(name);
        auto sub_av  = pat_array(sub);
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::At));
        put(map_off, pk::NAME, name_av);
        put(map_off, pk::SUB,  sub_av);
        put(map_off, pk::TYPE, type_av(type));
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_ref_bind_direct(std::string_view name,
                                                     bool is_mut,
                                                     TypeRef bind_type) {
        auto name_av = put_string(name);
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::RefBind));
        put(map_off, pk::NAME,      name_av);
        put(map_off, pk::IS_MUT,    put_bool(is_mut));
        put(map_off, pk::BIND_TYPE, type_av(bind_type));
        return map_off;
    }
    hermes2::arena_offset_t emit_pat_ref_pat_direct(const std::vector<lir::Pattern>& inner,
                                                    bool is_mut) {
        auto inner_av = pat_array(inner);
        auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Code::RefPat));
...
private:
    ...
    hermes2::arena_offset_t offset_of(const void* p) const noexcept {
        auto off = static_cast<uint32_t>(
            static_cast<const uint8_t*>(p) - arena_.head().data());
        return hermes2::arena_offset_t{off};
    }

    hermes2::TinyObjectMap* tom_at(hermes2::arena_offset_t off) noexcept {
        return reinterpret_cast<hermes2::TinyObjectMap*>(
            arena_.head().data() + off.value());
    }
    hermes2::ObjectArray* arr_at(hermes2::arena_offset_t off) noexcept {
        return reinterpret_cast<hermes2::ObjectArray*>(
            arena_.head().data() + off.value());
    }
    ...
    hermes2::AnyVal put_u64(uint64_t v) {
        auto av = hermes2::anyval_put<uint64_t>(arena_, v);
        LOGOS_ASSERT(av.has_value(), "LIR-MIRROR-002", "u64 anyval put failed");
        return *av;
    }
    hermes2::AnyVal put_f64(double v) {
        auto av = hermes2::anyval_put<double>(arena_, v);
        LOGOS_ASSERT(av.has_value(), "LIR-MIRROR-002", "f64 anyval put failed");
        return *av;
    }
    hermes2::AnyVal put_bool(bool v) {
        return hermes2::AnyVal::from_value<uint8_t>(
            v ? 1 : 0, hermes2::type_hash::Bool);
    }
    hermes2::AnyVal put_i32(int32_t v) {
        return hermes2::AnyVal::from_value<int32_t>(v);
    }
    hermes2::AnyVal put_u32(uint32_t v) {
        return hermes2::AnyVal::from_value<uint32_t>(v);
    }
    hermes2::AnyVal put_u8(uint8_t v) {
        return hermes2::AnyVal::from_value<uint8_t>(v);
    }

    hermes2::AnyVal type_av(TypeRef t) {
        if (!t) return hermes2::AnyVal{};
    ...
    hermes2::arena_offset_t make_array(size_t n) {
        if (dry_run_) return hermes2::arena_offset_t{};
        auto arr = hermes2::ObjectArray::create(arena_, n == 0 ? 1 : n);
        LOGOS_ASSERT(arr.has_value(), "LIR-MIRROR-003", "ObjectArray alloc failed");
        return offset_of(*arr);
    }
    void array_push(hermes2::arena_offset_t arr_off, hermes2::AnyVal v) {
        if (dry_run_) return;
        auto r = arr_at(arr_off)->push_back(v, arena_);
        LOGOS_ASSERT(r.has_value(), "LIR-MIRROR-003", "ObjectArray push failed");
    }

    hermes2::AnyVal expr_array(const std::vector<lir::LExprPtr>& v) {
    ...
    hermes2::AnyVal expr_arm_array(const std::vector<lir::EMatchArm>& v);
    ...
    struct FieldArrays {
        hermes2::AnyVal names;
        hermes2::AnyVal values;
    };
    FieldArrays struct_fields(
        const std::vector<std::pair<std::string, lir::LExprPtr>>& fields);

    // PatFieldBinding array (for PatStruct).
    hermes2::AnyVal field_binding_array(
        const std::vector<lir::PatFieldBinding>& v);

    // ── map creation + put helpers ─────────────────────────────────────────

    hermes2::arena_offset_t make_map(uint64_t schema_code, uint64_t cap = 8) {
        if (dry_run_) return hermes2::arena_offset_t{};
        auto m = hermes2::TinyObjectMap::create(arena_, cap);
    ...
    void put(hermes2::arena_offset_t map_off,
             const lir_schema::Key& key, hermes2::AnyVal val) {
        if (dry_run_) return;
    ...
    hermes2::arena_offset_t emit_expr(const LExpr& e);
    hermes2::arena_offset_t emit_stmt(const LStmt& s);
    hermes2::arena_offset_t emit_block(const LBlock& b);
    hermes2::arena_offset_t emit_pat(const Pattern& p);
    hermes2::arena_offset_t emit_hv(const HermesVal& v);
    hermes2::arena_offset_t emit_arm(const LMatchArm& a);
    hermes2::arena_offset_t emit_closure(const EClosure& c);
    hermes2::arena_offset_t emit_expr_arm(const lir::EMatchArm& a);
    hermes2::arena_offset_t emit_field_binding(const lir::PatFieldBinding& fb);
};
...
hermes2::arena_offset_t LirMirrorEmitter::emit_block(const LBlock& b) {
    bool backfill_only = false;
    if (b.mirror_offset_ != hermes2::arena_offset_t{}) {
    ...
    auto map_off = make_map(hermes2::schema::lir_stmt(lir_schema::stmt::Count));
...
hermes2::arena_offset_t LirMirrorEmitter::emit_arm(const LMatchArm& a) {
    auto pat_off    = emit_pat(a.pat);
    auto body_off   = emit_block(*a.body);
    hermes2::AnyVal guard_av;
...
hermes2::arena_offset_t LirMirrorEmitter::emit_expr_arm(const lir::EMatchArm& a) {
    auto pat_off    = emit_pat(a.pat);
...
hermes2::AnyVal LirMirrorEmitter::expr_arm_array(
    ...
    const lir::PatFieldBinding& fb)
{
    ...
    auto map_off = make_map(hermes2::schema::lir_pat(lir_schema::pat::Count));
    put(map_off, pk::FIELD_NAME, name_av);
    put(map_off, pk::SUB,        subs_av);
    return map_off;
}

hermes2::AnyVal LirMirrorEmitter::field_binding_array(
    const std::vector<lir::PatFieldBinding>& v)
{
    if (v.empty()) return hermes2::AnyVal{};
    std::vector<hermes2::AnyVal> elems;
    ...
    const std::vector<std::pair<std::string, lir::LExprPtr>>& fields)
{
    ...
    std::vector<hermes2::AnyVal> name_elems, value_elems;
...
hermes2::arena_offset_t LirMirrorEmitter::emit_closure(const EClosure& c) {
    // Body first
    ...
    hermes2::AnyVal param_names_av, param_types_av;
    if (!c.params.empty()) {
        std::vector<hermes2::AnyVal> n_elems, t_elems;
        n_elems.reserve(c.params.size());
        t_elems.reserve(c.params.size());
        for (auto& p : c.params) {
            n_elems.push_back(put_string(p.name));
            t_elems.push_back(p.type ? type_av(p.type) : hermes2::AnyVal{});
        }
    ...
    if (any_mut && c.mut_captures.size() == c.captures.size()) {
        std::vector<hermes2::AnyVal> m_elems;
    ...
    if (c.capture_paths.size() == c.captures.size()) {
        ...
        if (any_narrow) {
            std::vector<hermes2::AnyVal> p_elems;
...
hermes2::arena_offset_t LirMirrorEmitter::emit_hv(const HermesVal& v) {
    ...
    // unreachable.
    LOGOS_ASSERT(v.mirror_offset_ != hermes2::arena_offset_t{},
                 "B6.S35.S6",
...
hermes2::arena_offset_t LirMirrorEmitter::emit_pat(const Pattern& p) {
    ...
    LOGOS_ASSERT(p.mirror_offset_ != hermes2::arena_offset_t{},
                 "B6.S35.S3",
...
hermes2::arena_offset_t LirMirrorEmitter::emit_stmt(const LStmt& s) {
    ...
    LOGOS_ASSERT(s.mirror_offset_ != hermes2::arena_offset_t{},
                 "B6.S35.S5",
...
hermes2::arena_offset_t LirMirrorEmitter::emit_expr(const LExpr& e) {
    ...
    LOGOS_ASSERT(e.mirror_offset_ != hermes2::arena_offset_t{},
                 "B6.S35.S7D",
...
hermes2::arena_offset_t lir_mirror_emit_generic_ref(lir::LProgram& prog, TypeRef ty,
                                                    std::string_view name,
                                                    const std::vector<TypeRef>& type_args) {
    auto& arena = prog.type_pool.arena_or_init();
    LirMirrorEmitter em(arena, *prog.mirror_table, prog.type_pool);
...
hermes2::arena_offset_t lir_mirror_emit_assign(lir::LProgram& prog, uint32_t line, std::string_view name, const lir::LExprPtr& value, bool drop_old) {
    auto& arena = prog.type_pool.arena_or_init();
    LirMirrorEmitter em(arena, *prog.mirror_table, prog.type_pool);
...
void lir_mirror_retype_expr(lir::LProgram& prog,
                            ...
                            TypeRef new_ty) {
    if (expr_off == hermes2::arena_offset_t{}) return;
    auto& arena = prog.type_pool.arena_or_init();
...
void lir_mirror_populate_moved(lir::LProgram& prog, LirMirrorTable& table) {
    ...
    if (prog.expr_pool_) {
        for (auto& uptr : *prog.expr_pool_)
            if (uptr && uptr->mirror_offset_ != hermes2::arena_offset_t{})
