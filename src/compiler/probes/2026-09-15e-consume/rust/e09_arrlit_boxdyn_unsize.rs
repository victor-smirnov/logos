// RUST TWIN of e09_arrlit_boxdyn_unsize.logos (mechanical, tools: l2rs.py of round 2026-09-15e-consume)
...
fn g(p: *mut i64) -> i64 { let b1: Box<D> = Box::new(D { v: 1i64, c: p }); let b2: Box<D> = Box::new(D { v: 10i64, c: p }); { let arr: [Box<dyn Tr>; 2] = [b1, b2]; if arr[1].get() != 10i64 { return 7i64; } } return 0i64; }
fn logos_main() -> i32 {
    let mut n: i64 = 0i64;
    let p: *mut i64 = &mut n;
    let r: i64 = g(p);
