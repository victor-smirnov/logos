...
#
# THE ORACLE IS THE EXIT CODE HERE (every program returns 0 when correct and a
# distinct non-zero when it read the wrong word); a REFUSED/CCFAIL line is the
# compile verdict, printed verbatim. Original note: A destructor accumulator reaches 1011
...
HAND="${HAND:-src/compiler/probes/2026-09-08-fatrepr-fix/hand}"
ROWS="tests/soundness/open"
TMPD=$(mktemp -d); trap 'rm -rf "$TMPD"' EXIT
export LOGOS_VERIFY_LAYOUT="${LOGOS_VERIFY_LAYOUT:-1}"
...
for r in zonemut_fat_ref_struct_field_layout_abort \
         fatslice_field_match_binder_invalid_mlir \
         enum_variant_ctor_arg_no_unsize_coercion \
         method_with_unsized_wrapper_param_not_found; do one "$ROWS/$r.logos"; done
