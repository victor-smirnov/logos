...
HAND="src/compiler/probes/2026-09-06m-placedrop/hand"
...
one() {
    ...
    if grep -q -E "error( \[|:)" "$d/cc.err"; then
        printf '%-46s REFUSED   %s\n' "$(basename "$src" .logos)" \
            "$(grep -m1 -E 'error( \[|:)' "$d/cc.err" | cut -c1-90)"; return
    fi
...
for r in assign_tuple_elem_no_drop_old assign_index_elem_no_drop_old \
         assign_field_path_not_var_rooted_no_drop_old; do one "$ROWS/$r.logos"; done
