...
**Implementation:** `jenny` — Victor Smirnov's Clang 21.1.8 fork, installed at `/opt/jenny-21x`. Integrated into Logos build via `cmake/toolchains/jenny.cmake` (chained through vcpkg).

**Deliverables:**
- [x] `[[clang::green]]` / `__attribute__((green))` function attribute → activates `-fsplit-stack` (segmented stack) codegen for that function
- [x] Two-color call enforcement: calling a green function from a non-green function is a compile-time error
- [x] `-fsplit-stack` infrastructure in LLVM backend (x86-64)
- [x] Integration with CMake build system (`cmake/toolchains/jenny.cmake`)
- [ ] Segmented stack runtime (allocate/extend/shrink stack segments) — needed for reactor, part of Phase 1.2
- [ ] Trampoline generation at green↔red call boundaries — part of Phase 1.2

**Key design:**
- `[[clang::green]]` on a function → LLVM IR `"split-stack"` attribute → segmented stack prologue
- Red functions use system stack (default, full C++ ABI compatibility)
- Cross-color calls detected at compile time; trampolines will be generated by the reactor runtime

**Reference:** jenny git repo: `git@github.com:victor-smirnov/jenny.git`
