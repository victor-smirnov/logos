...
### G149-6 — ✅ FIXED (2026-05-22): `impl Trait for fn(A, B) -> C` is a parse error near `impl`

**FIXED in full** (no-defer directive). The wall was the global per-fn signature mangler embedding the fn-ptr `Self` (=`fn(A,B)->C` with impl TypeVars). Fix: (1) grammar fn_ptr_type impl-target alts; (2) collect_impl + lower_impl_block key the target by arity `$fnptr$N` and clear impl_type_params_ (type-erased → one non-generic emission); (3) Self binds to the resolved FnPtr; (4) FnPtr bound-satisfaction by arity; (5) mono receiver-cname maps `fn(...)->R` → `$fnptr$N`; (6) **the keystone** — `function_signature_key` sets a `g_mangle_erase_fnptr` flag for a `$fnptr$`-prefixed base, and `mangle_type_for_name` gains a FnPtr case that emits the arity-only `$fnptr$<n>` under that flag, so the method symbol (`$fnptr$2__foo__f__ref_$fnptr$2`) is stable across A,B,C and matches the call; (7) **`bare_fn_name` bug** — its free-fn `$`-strip ate the leading `$` of a method type name like `fp.$fnptr$2__foo` (→ `fnptr$2__foo`), so the dispatch lookup missed; guarded the strip to skip a `$` at pos 0 or right after a `.` (also corrects `$tuple$`/`$slice$` method bare-names). Regression `impl_trait_for_fn_ptr` + re-import `fn-type-trait-impl-15444-b151`. Verified general (1-arg + 2-arg arity, turbofish + inferred dispatch, and distinct fn-ptr-VALUE params do NOT collide). 4578/4578. ORIGINAL/INVESTIGATION below:
