...
public:
    ...
    ArrayView   as_array()    const noexcept { return logos::hermes::as_array(av_, holder_); }
    StringView  as_string()   const noexcept { return logos::hermes::as_string(av_, holder_); }
    MapView     as_map()      const noexcept { return logos::hermes::as_map(av_, holder_); }
...
template <>
struct std::formatter<logos::hermes::StringView> : std::formatter<std::string_view> {
    auto format(const logos::hermes::StringView& s, auto& ctx) const {
