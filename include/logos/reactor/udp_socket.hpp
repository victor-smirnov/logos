...
struct UdpEndpoint {
    ...
    UdpEndpoint(const char* host, uint16_t port) {
...
public:
    ...
    static UdpSocket bind_to(const char* host, uint16_t port) {
        int fd = make_socket();
        int one = 1;
    ...
    static UdpSocket connect_to(const char* host, uint16_t port) {
        int fd = make_socket();
        UdpEndpoint ep(host, port);
    ...
    static UdpSocket create() { return UdpSocket{make_socket()}; }
