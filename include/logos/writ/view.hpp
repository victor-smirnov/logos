...
//
// Owning (not the legacy non-owning view + Own<> split): without a borrow checker
...
public:
    ...
    // Construct from a within-arena offset + holder (the legacy `View(offset, holder)`).
    ...
    // null view. The cut-over uses this in place of the legacy `View(av.to_offset(), h)`.
...
public:
    ...
    // (the legacy readers rely on this).
...
// std::string / string_view on the LEFT vs a StringView on the right (the legacy
...
// ── Object — the legacy generic node handle, native {AnyVal, holder} ────────────
