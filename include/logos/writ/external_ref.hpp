...
//
// Writ CHANGE vs legacy: an ExternalRef is now an AnyVal **Pod niche**, not a
...
//
// This is strictly better than the legacy tagged-object form (which only existed
// because the legacy 4-byte AnyVal had a mere 3-byte inline payload):
...
#include <logos/writ/arena_pool.hpp>   // arena_id_t, ArenaPool, global_arena_pool

namespace logos::writ {

class MemHolder;

// The Pod type code identifying an ExternalRef niche (7-bit; matches the legacy
