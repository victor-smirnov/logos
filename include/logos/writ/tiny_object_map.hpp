...
// `put` of a new key into a full map is a no-op (matches WMap<Hu6,WAny>::set).
class TinyObjectMap {
public:
    ...
    // ImportTable, metaprog ExprBlob roots); byte-shared with Logos WMap<Hu6,WAny>.
...
private:
    // Layout matches Logos WMap<Hu6,WAny> byte-for-byte. The `#[zoned2]` pointer
...
// header_(8) + data_(8, self-relative i64) + schema_type_code_(8) = 24 (matches WMap<Hu6,WAny>)
