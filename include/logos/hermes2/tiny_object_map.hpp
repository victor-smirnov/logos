...
// BYTE-IDENTICAL to the Logos stdlib HMap<Hu6, HVal> (stdlib/lang/hermes2/hmap.logos
// "bitmap-indexed spec"):
//
//   { header_ : u64, data_ : self-relative ptr to AnyVal[] }   (16 bytes)
//     header bits[0:51] = presence bitmap, [52:57] = capacity, [58:63] = size
//
// NB: this Logos layout differs from Hermes1's TinyObjectMap (which inlined the
// values and carried a `schema_type_code_` word, with size/cap SWAPPED in the
// header). We follow the Logos layout. The value array is a SEPARATE buffer of
// at-rest AnyVals, kept in key order; O(1) lookup via popcount. FIXED capacity —
// `put` of a new key into a full map is a no-op (matches HMap<Hu6,HVal>::set).
class TinyObjectMap {
public:
...
// header_(8) + data_(8, self-relative i64) = 16 (matches HMap<Hu6,HVal>)
