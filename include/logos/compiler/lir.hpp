...
struct EClosure {
    ...
    // Per-parameter binding slot, parallel to `params` (CL_PARAM_SLOTS).
    std::vector<uint32_t>           param_slots;
...
struct LProgram {
    ...
    // backed marker instantiation (`CtrClass<CFG>`); SEMA (Phase 4a)
    // fills one per DEFERRED diagnostic against a factory-backed marker (an
    // unmet `C: CtrFamily` bound / an unresolved `C::Handle` projection at a
    // create_ctr-style use — the impl is generated by the drain, so the
    // round-0 error is suppressed and re-checked post-drain). Keyed by the
    ...
    struct FactoryDemand {
        std::string base;     // marker base name ("CtrClass")
        ...
        // Phase 4a: true when sema deferred a diagnostic against this demand.
        // The factory MUST close it — a required demand still present after
        // the drain (sema re-deferred in the post-drain round) or with no
        // runnable factory is a hard error; a purely structural marker
