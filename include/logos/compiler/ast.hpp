...
inline constexpr Code WRIT_BLOB          {"WRIT_BLOB",         261}; // sema-internal: pre-serialised Writ static blob (driver splice from metacall WritStatic return); VALUE = raw blob bytes (Varchar). Was 199, which COLLIDED with the parser-produced TEMPLATE_DECL — harmless only because one is an item and the other an expression, i.e. their dispatch switches never met. Moved so the two node kinds are distinguishable by CODE alone.
