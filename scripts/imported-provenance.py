...
rows.sort()
# Default the output path. Measured 2026-09-08: called with no argument the
# script died with IndexError, which is how a re-port round left the table a
# whole round stale — 33 rows out of date, including the seventeen
# `// Modifications:` blocks the round had just written. The table that measures
# whether a port declares its changes must be trivial to refresh.
OUT = sys.argv[1] if len(sys.argv) > 1 else os.path.join(REPO, "tests/imported/PROVENANCE.tsv")
